...
# 5. Symlinks the 'shadow' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ SHADOW Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink shadow into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up shadow command..."

SHADOW_BIN="$SCRIPT_DIR/venv/bin/shadow"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SHADOW_BIN" "$COMMAND_LINK_DIR/shadow"
echo -e "${GREEN}✓${NC} Symlinked shadow → $COMMAND_LINK_DISPLAY_DIR/shadow"
...
# ============================================================================
# Seed bundled skills into ~/.shadow/skills/
# ============================================================================

SHADOW_SKILLS_DIR="${SHADOW_HOME:-$HOME/.shadow}/skills"
mkdir -p "$SHADOW_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.shadow/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SHADOW_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  shadow status        # Check configuration"
if is_termux; then
    echo "  shadow gateway       # Run gateway in foreground"
else
    echo "  shadow gateway install # Install gateway service (messaging + cron)"
fi
echo "  shadow cron list     # View scheduled jobs"
echo "  shadow doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m shadow_cli.main setup
fi
