...
_DEFAULT_CONTAINER_TAG = "yousef shtiwe"
...
def _load_supermemory_config(yousef shtiwe_home: str) -> dict:
    config = _default_config()
    config_path = Path(yousef shtiwe_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, yousef shtiwe_home: str) -> None:
    config_path = Path(yousef shtiwe_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `yousef shtiwe memory setup`.
        # All other options are documented for $YOUSEF SHTIWE_HOME/supermemory.json
    ...
    def save_config(self, values, yousef shtiwe_home):
        ...
        _save_supermemory_config(sanitized, yousef shtiwe_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from yousef shtiwe_constants import get_yousef shtiwe_home
        self._yousef shtiwe_home = kwargs.get("yousef shtiwe_home") or str(get_yousef shtiwe_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._yousef shtiwe_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "yousef shtiwe", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "yousef shtiwe_memory", "target": target, "type": "explicit_memory"},
    ...
    def _tool_store(self, args: dict) -> str:
        ...
        metadata["source"] = "yousef shtiwe_tool"
        try:
