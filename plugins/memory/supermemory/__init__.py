...
def _load_supermemory_config(shadow_home: str) -> dict:
    config = _default_config()
    config_path = Path(shadow_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, shadow_home: str) -> None:
    config_path = Path(shadow_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `shadow memory setup`.
        # All other options are documented for $SHADOW_HOME/supermemory.json
    ...
    def save_config(self, values, shadow_home):
        ...
        _save_supermemory_config(sanitized, shadow_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from shadow_constants import get_shadow_home
        self._shadow_home = kwargs.get("shadow_home") or str(get_shadow_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._shadow_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "shadow", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "shadow_memory", "target": target, "type": "explicit_memory"},
