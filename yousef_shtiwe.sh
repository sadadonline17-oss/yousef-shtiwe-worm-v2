#!/usr/bin/env bash
# =============================================================================
# yousef_shtiwe CLI - Simplified installation, update, and lifecycle management
# =============================================================================
set -euo pipefail
...
TOOL_IMAGES="yousef_shtiwe-recon:latest yousef_shtiwe-vuln-scanner:latest yousef_shtiwe-github-hunter:latest yousef_shtiwe-trufflehog:latest"
...
export_version() {
    export yousef_shtiwe_VERSION
    yousef_shtiwe_VERSION="$(get_version)"
}

remove_yousef_shtiwe_images() {
    # Remove locally-built yousef_shtiwe images
...
pull_gvm_images() {
    ...
    if [[ ${#failed[@]} -gt 0 ]]; then
        ...
        echo -e "  ${YELLOW}Then: sudo systemctl restart docker && ./yousef_shtiwe.sh up${NC}"
        exit 1
    fi
...
# KB_ENABLED=false ./yousef_shtiwe.sh install
is_kb_enabled() {
...
_kb_wait_neo4j() {
    if ! docker ps --format '{{.Names}}' | grep -q '^yousef_shtiwe-neo4j$'; then
    ...
    while [[ $waited -lt $max_wait ]]; do
        ...
        health=$(docker inspect --format='{{.State.Health.Status}}' \
                   yousef_shtiwe-neo4j 2>/dev/null || echo "unknown")
    ...
    error "Check: docker logs yousef_shtiwe-neo4j"
    return 1
}
...
_kb_choose_profile() {
    ...
    #   ./yousef_shtiwe.sh kb build lite
    #
...
# should always succeed (or return a safe fallback) so `./yousef_shtiwe.sh status`
# works in any state.
...
_kb_get_neo4j_count() {
    if ! docker ps --format '{{.Names}}' 2>/dev/null | grep -q '^yousef_shtiwe-neo4j$'; then
        echo "0"
        return
    fi
    ...
    local count
    count=$(docker exec yousef_shtiwe-neo4j cypher-shell \
...
cmd_install() {
    ...
    local version
    version="$(get_version)"
    info "Installing yousef_shtiwe v${version}..."
    ...
    echo -e "  ${GREEN}${BOLD}  yousef_shtiwe v${version} is ready!${NC}"
    ...
    if is_kb_enabled; then
        ...
        if _kb_bootstrap "$kb_profile"; then
            ...
            warn "Retry with: ./yousef_shtiwe.sh kb build ${kb_profile}"
        fi
    else
    ...
    echo ""
    echo -e "  ${CYAN}Status:${NC}  ./yousef_shtiwe.sh status"
    echo ""
    echo -e "  ${YELLOW}If yousef_shtiwe is useful to you, a GitHub star helps others find the project:${NC}"
    echo -e "  ${CYAN}https://github.com/samugit83/yousef_shtiwe${NC}"
    echo ""
    if [[ "$gvm_mode" == "true" ]]; then
...
cmd_update() {
    ...
    if ! git -C "$SCRIPT_DIR" pull --ff-only 2>/dev/null; then
        if ! git -C "$SCRIPT_DIR" pull --ff-only origin master 2>/dev/null; then
            ...
            echo ""
            echo "  Try one of:"
            echo "    git stash && ./yousef_shtiwe.sh update && git stash pop"
            echo "    git commit -am 'local changes' && ./yousef_shtiwe.sh update"
            exit 1
        fi
    fi
...
cmd_up_dev() {
    ...
    info "Starting yousef_shtiwe in DEV mode (GVM: ${gvm_flag})..."
    ...
    echo ""
    echo -e "  ${GREEN}${BOLD}==========================================================${NC}"
    echo -e "  ${GREEN}${BOLD}  yousef_shtiwe DEV is ready!${NC}"
...
cmd_up() {
    ...
    info "Starting yousef_shtiwe (GVM: ${gvm_mode})..."
    ...
    echo ""
    echo -e "  ${GREEN}${BOLD}==========================================================${NC}"
    echo -e "  ${GREEN}${BOLD}  yousef_shtiwe is ready!${NC}"
    echo -e "  ${GREEN}${BOLD}  Open ${CYAN}http://localhost:3000${GREEN}${BOLD} in your browser${NC}"
    echo -e "  ${GREEN}${BOLD}==========================================================${NC}"
    echo ""
...
cmd_down() {
    info "Stopping yousef_shtiwe..."
    docker compose down
    success "All services stopped. Volumes and images preserved."
}

cmd_clean() {
    warn "This will remove all yousef_shtiwe containers and images."
    ...
    info "Removing yousef_shtiwe images..."
    remove_yousef_shtiwe_images
    docker image prune -f >/dev/null 2>&1 || true

    success "All yousef_shtiwe containers and images removed. Volumes preserved."
    echo ""
    info "To reinstall: ./yousef_shtiwe.sh install"
}

cmd_purge() {
    echo ""
    warn "This will PERMANENTLY DELETE:"
    warn "  - All yousef_shtiwe containers and images"
    ...
    rm -f "$GVM_FLAG_FILE"
    rm -f "$SKIPKBASE_FLAG_FILE"
    success "Full cleanup complete. All yousef_shtiwe data and images have been removed."
    echo ""
    info "To reinstall: ./yousef_shtiwe.sh install"
}
...
cmd_status() {
    ...
    # Container list — filter to yousef_shtiwe containers only. Keeps the header
    # row and any container whose name starts with "yousef_shtiwe-".
    docker compose ps | grep -E '^(NAME|yousef_shtiwe-)' || {
...
cmd_kb_build() {
    ...
    case "$profile" in
        ...
        *)
            ...
            echo "Usage: ./yousef_shtiwe.sh kb build [lite|standard|full]"
            exit 1
            ;;
    esac
...
cmd_kb_rebuild() {
    ...
    case "$profile" in
        ...
        *)
            ...
            echo "Usage: ./yousef_shtiwe.sh kb rebuild [cpu-lite|lite|standard|full]"
            exit 1
            ;;
    esac
...
cmd_kb_help() {
    echo -e "${BOLD}Usage:${NC} ./yousef_shtiwe.sh kb <command> [args]"
    echo ""
    ...
    echo "  full      standard + Nuclei (requires yousef_shtiwe-kali container running)"
    echo ""
    echo -e "${BOLD}Examples:${NC}"
    echo "  ./yousef_shtiwe.sh kb build             # Build lite KB (default)"
    echo "  ./yousef_shtiwe.sh kb build standard    # Build with 2 years of NVD"
    echo "  ./yousef_shtiwe.sh kb rebuild           # Wipe + rebuild standard (default)"
    echo "  ./yousef_shtiwe.sh kb rebuild lite      # Wipe + rebuild lite profile"
    echo "  ./yousef_shtiwe.sh kb rebuild full      # Wipe + rebuild full profile (incl. nuclei)"
    echo "  ./yousef_shtiwe.sh kb update nvd        # Incremental NVD refresh"
    echo "  ./yousef_shtiwe.sh kb update            # Update all sources"
    echo "  ./yousef_shtiwe.sh kb stats             # See what's in the KB"
    echo ""
}

cmd_help() {
    print_banner
    echo -e "${BOLD}Usage:${NC} ./yousef_shtiwe.sh <command> [options]"
    echo ""
    echo -e "${BOLD}Commands:${NC}"
    echo -e "  ${GREEN}install${NC}              Build and start yousef_shtiwe (without GVM)"
    echo -e "  ${GREEN}install --gvm${NC}        Build and start yousef_shtiwe (with GVM/OpenVAS)"
    ...
    echo ""
    echo -e "${BOLD}Examples:${NC}"
    echo "  ./yousef_shtiwe.sh install               # First-time setup (no GVM)"
    echo "  ./yousef_shtiwe.sh install --gvm         # First-time setup (full stack)"
    echo "  ./yousef_shtiwe.sh install --skipkbase   # Lightweight (Tavily-only, no KB)"
    echo "  ./yousef_shtiwe.sh update           # Update to latest version"
    echo "  ./yousef_shtiwe.sh up               # Start after reboot"
    echo "  ./yousef_shtiwe.sh up dev           # Dev mode with hot-reload (auto-detects GVM)"
    echo "  ./yousef_shtiwe.sh kb build lite    # Build Knowledge Base"
    echo "  ./yousef_shtiwe.sh kb update        # Refresh all KB sources"
    echo "  ./yousef_shtiwe.sh kb stats         # Show KB chunk counts"
    echo ""
}
