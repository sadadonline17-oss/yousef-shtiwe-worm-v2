"""
SHADOWAgentLoop -- Reusable Multi-Turn Agent Engine

Runs the shadow-agent tool-calling loop using standard OpenAI-spec tool calling.
...
identical to shadow-agent's run_agent.py. Tool execution is dispatched via
...
# Resized at runtime by SHADOWAgentBaseEnv.__init__ via resize_tool_pool().
...
def resize_tool_pool(max_workers: int):
    ...
    Called by SHADOWAgentBaseEnv.__init__ based on config.tool_pool_size.
...
class SHADOWAgentLoop:
    """
    Runs shadow-agent's tool-calling loop using standard OpenAI-spec tool calling.
    ...
    async def run(self, messages: List[Dict[str, Any]]) -> AgentResult:
        ...
        for turn in range(self.max_turns):
            ...
            # shadow-agent's standalone parsers. This handles the case where
            ...
            if (
                ...
                try:
                    ...
                    fallback_parser = get_parser("shadow")
            ...
            if assistant_msg.tool_calls:
                ...
                # Execute each tool call via shadow-agent's dispatch
