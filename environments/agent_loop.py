"""
YOUSEF SHTIWEAgentLoop -- Reusable Multi-Turn Agent Engine

Runs the yousef shtiwe-agent tool-calling loop using standard OpenAI-spec tool calling.
...
identical to yousef shtiwe-agent's run_agent.py. Tool execution is dispatched via
...
# Resized at runtime by YOUSEF SHTIWEAgentBaseEnv.__init__ via resize_tool_pool().
...
def resize_tool_pool(max_workers: int):
    ...
    Called by YOUSEF SHTIWEAgentBaseEnv.__init__ based on config.tool_pool_size.
...
class YOUSEF SHTIWEAgentLoop:
    """
    Runs yousef shtiwe-agent's tool-calling loop using standard OpenAI-spec tool calling.
    ...
    async def run(self, messages: List[Dict[str, Any]]) -> AgentResult:
        ...
        for turn in range(self.max_turns):
            ...
            # yousef shtiwe-agent's standalone parsers. This handles the case where
            ...
            if (
                ...
                try:
                    ...
                    fallback_parser = get_parser("yousef shtiwe")
            ...
            if assistant_msg.tool_calls:
                ...
                # Execute each tool call via yousef shtiwe-agent's dispatch
