"""
YOUSEF SHTIWEAgentBaseEnv -- Abstract Base Environment for YOUSEF SHTIWE-Agent + Atropos

Provides the Atropos integration plumbing that all yousef shtiwe-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via YOUSEF SHTIWEAgentLoop
...
# Ensure the yousef shtiwe-agent repo root is on sys.path so that imports like
...
# Load API keys from yousef shtiwe-agent/.env so all environments can access them
...
# Import yousef shtiwe-agent toolset infrastructure
...
class YOUSEF SHTIWEAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for yousef shtiwe-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of yousef shtiwe toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: yousef shtiwe, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/yousef shtiwe-results/ "
...
class YOUSEF SHTIWEAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for yousef shtiwe-agent Atropos integration.
    ...
    name: Optional[str] = "yousef shtiwe-agent"
    env_config_cls = YOUSEF SHTIWEAgentEnvConfig

    def __init__(
        self,
        config: YOUSEF SHTIWEAgentEnvConfig,
        ...
        # Set terminal environment variables so yousef shtiwe tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY yousef shtiwe-agent tool (terminal, file, web,
