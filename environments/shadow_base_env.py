"""
SHADOWAgentBaseEnv -- Abstract Base Environment for SHADOW-Agent + Atropos

Provides the Atropos integration plumbing that all shadow-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via SHADOWAgentLoop
...
# Ensure the shadow-agent repo root is on sys.path so that imports like
...
# Load API keys from shadow-agent/.env so all environments can access them
...
# Import shadow-agent toolset infrastructure
...
class SHADOWAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for shadow-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of shadow toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: shadow, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/shadow-results/ "
...
class SHADOWAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for shadow-agent Atropos integration.
    ...
    name: Optional[str] = "shadow-agent"
    env_config_cls = SHADOWAgentEnvConfig

    def __init__(
        self,
        config: SHADOWAgentEnvConfig,
        ...
        # Set terminal environment variables so shadow tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY shadow-agent tool (terminal, file, web,
