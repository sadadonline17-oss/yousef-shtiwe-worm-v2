# OpenAI-Compatible API Server for Shadow Agent
...
SSE streaming. By exposing this endpoint, shadow-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  shadow-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.shadow/.env`
2. Run `shadow gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with shadow-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns shadow-agent as a model) |
...
Authorization: Bearer shadow-api-key-here
...
But shadow-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "shadow-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured shadow-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of shadow-agent's core prompt
...
Once implemented, shadow-agent works as a drop-in backend for:
