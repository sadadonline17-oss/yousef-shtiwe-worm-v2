...
Connects SHADOW Agent to WeChat personal accounts via Tencent's iLink Bot API.
...
def _account_dir(shadow_home: str) -> Path:
    path = Path(shadow_home) / "weixin" / "accounts"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _account_file(shadow_home: str, account_id: str) -> Path:
    return _account_dir(shadow_home) / f"{account_id}.json"
...
def save_weixin_account(
    ...
    *,
    ...
    path = _account_file(shadow_home, account_id)
...
def load_weixin_account(shadow_home: str, account_id: str) -> Optional[Dict[str, Any]]:
...
class ContextTokenStore:
    ...
    def __init__(self, shadow_home: str):
        self._root = _account_dir(shadow_home)
...
def _sync_buf_path(shadow_home: str, account_id: str) -> Path:
    return _account_dir(shadow_home) / f"{account_id}.sync.json"


def _load_sync_buf(shadow_home: str, account_id: str) -> str:
    path = _sync_buf_path(shadow_home, account_id)
    if not path.exists():
        return ""
    try:
...
def _save_sync_buf(shadow_home: str, account_id: str, sync_buf: str) -> None:
    path = _sync_buf_path(shadow_home, account_id)
...
class WeixinAdapter(BasePlatformAdapter):
    """Native SHADOW adapter for Weixin personal accounts."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._shadow_home = shadow_home
        self._token_store = ContextTokenStore(shadow_home)
        ...
        if self._account_id and not self._token:
            persisted = load_weixin_account(shadow_home, self._account_id)
            if persisted:
    ...
    async def _poll_loop(self) -> None:
        ...
        sync_buf = _load_sync_buf(self._shadow_home, self._account_id)
        ...
        while self._running:
            try:
                ...
                if new_sync_buf:
                    ...
                    _save_sync_buf(self._shadow_home, self._account_id, sync_buf)
    ...
    async def send(
        ...
        try:
            ...
            for idx, chunk in enumerate(chunks):
                client_id = f"shadow-weixin-{uuid.uuid4().hex}"
    ...
    async def _send_file(self, chat_id: str, path: str, caption: str) -> str:
        ...
        if caption:
            last_message_id = f"shadow-weixin-{uuid.uuid4().hex}"
            await _send_message(
                self._session,
                base_url=self._base_url,
                token=self._token,
...
async def send_weixin_direct(
    *,
    ...
    token_store = ContextTokenStore(str(get_shadow_home()))
