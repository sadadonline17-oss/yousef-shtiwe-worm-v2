...
Checklist for integrating a new messaging platform into the YOUSEF SHTIWE gateway.
...
And add it to the `yousef shtiwe-gateway` composite:

```python
"yousef shtiwe-gateway": {
    "includes": [..., "yousef shtiwe-your-platform"]
}
```
...
## 12. Status Display (`yousef shtiwe_cli/status.py`)
...
## 13. Gateway Setup Wizard (`yousef shtiwe_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ yousef shtiwe_cli/ toolsets.py \
  --include="*.py" -l | sort -u
