...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-YOUSEF SHTIWE-Session-Id header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists yousef shtiwe-agent as an available model
- POST /v1/runs                    — start a run, returns run_id immediately (202)
- GET  /v1/runs/{run_id}/events    — SSE stream of structured lifecycle events
- GET  /health                     — health check

Any OpenAI-compatible frontend (Open WebUI, LobeChat, LibreChat,
AnythingLLM, NextChat, ChatBox, etc.) can connect to yousef shtiwe-agent
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_yousef shtiwe_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        try:
...
def _derive_chat_session_id(
    ...
    the same YOUSEF SHTIWE session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through yousef shtiwe-agent's AIAgent.
    """
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        3. Fallback: "yousef shtiwe-agent"
        """
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``yousef shtiwe sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the yousef shtiwe-api-server default.
        """
        ...
        max_iterations = int(os.getenv("YOUSEF SHTIWE_MAX_ITERATIONS", "90"))
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "yousef shtiwe-agent"})

    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return yousef shtiwe-agent as an available model."""
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # Allow caller to continue an existing session by passing X-YOUSEF SHTIWE-Session-Id.
        ...
        provided_session_id = request.headers.get("X-YOUSEF SHTIWE-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-YOUSEF SHTIWE-Session-Id rejected: "
            ...
            # conversation map to the same YOUSEF SHTIWE session.  The first user
        ...
        if stream:
            ...
            def _on_tool_progress(event_type, name, preview, args, **kwargs):
                ...
                ``event: yousef shtiwe.tool.progress`` line that compliant frontends
        ...
        return web.json_response(response_data, headers={"X-YOUSEF SHTIWE-Session-Id": session_id})
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-YOUSEF SHTIWE-Session-Id"] = session_id
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: yousef shtiwe.tool.progress`` SSE event so
                ...
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    ...
                    await response.write(
                        f"event: yousef shtiwe.tool.progress\ndata: {event_data}\n\n".encode()
                    )
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from yousef shtiwe_cli.auth import has_usable_secret
