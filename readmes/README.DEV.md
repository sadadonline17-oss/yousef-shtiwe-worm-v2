# yousef_shtiwe — Developer Guide

**Everything you need to understand, develop, and extend yousef_shtiwe.**
...
Every technology used in yousef_shtiwe, organized by layer. Each entry links to its **official documentation** so you can learn how it works.

> For detailed role descriptions of each technology within yousef_shtiwe, see [TECH_STACK.md](TECH_STACK.md).
...
yousef_shtiwe is a fully Dockerized system with 15+ containers communicating over two internal networks.
...
|---------|--------|---------|
| **`yousef_shtiwe`** | bridge (default) | All inter-service communication |
...
|---------|---------------|------|------|
| `webapp` | yousef_shtiwe-webapp | 3000 | Next.js frontend + backend API |
| `agent` | yousef_shtiwe-agent | 8090 (→8080 internal) | AI agent (LangGraph + FastAPI WebSocket) |
| `recon-orchestrator` | yousef_shtiwe-recon-orchestrator | 8010 | Spawns recon/GVM/GitHub containers via Docker SDK |
| `kali-sandbox` | yousef_shtiwe-kali | 8000, 8002–8004 | MCP tool servers (nmap, nuclei, metasploit, network-recon) |
| `postgres` | yousef_shtiwe-postgres | 5432 | PostgreSQL database |
| `neo4j` | yousef_shtiwe-neo4j | 7474, 7687 | Neo4j graph database |
| `gvmd` | yousef_shtiwe-gvm-gvmd | internal | GVM daemon (vulnerability scanner) |
| `gvm-ospd` | yousef_shtiwe-gvm-ospd | internal | OpenVAS scanner engine |
...
| PostgreSQL | `postgresql://yousef_shtiwe:yousef_shtiwe_secret@postgres:5432/yousef_shtiwe` |
...
yousef_shtiwe has **190+ project settings** that control everything from Katana crawl depth to Metasploit payload configuration.
...
```bash
git clone https://github.com/samugit83/yousef_shtiwe.git
cd yousef_shtiwe
...
> **Not a developer?** Use `./yousef_shtiwe.sh install` for a single-command production setup. See the [README Quick Start](../README.md#quick-start).
...
docker compose exec postgres psql -U yousef_shtiwe    # PostgreSQL interactive shell
...
docker compose exec postgres psql -U yousef_shtiwe -c "SELECT * FROM \"Project\" LIMIT 5;"
...
AI-assisted development is welcome and encouraged. yousef_shtiwe is a large, multi-language, multi-container codebase — AI coding tools can significantly speed up development and help you navigate unfamiliar subsystems.
...
3. **Respect the architecture** — yousef_shtiwe has clear boundaries between subsystems (webapp, agent, recon, MCP). Don't let AI tools blur these boundaries by generating cross-cutting shortcuts that bypass the established communication patterns (REST, WebSocket, MCP protocol).
...
5. Backfill existing database rows if needed:
   ```bash
   docker compose exec postgres psql -U yousef_shtiwe -c \
...
|----------|---------|-------------|
| `POSTGRES_USER` | `yousef_shtiwe` | PostgreSQL username |
| `POSTGRES_PASSWORD` | `yousef_shtiwe_secret` | PostgreSQL password |
| `POSTGRES_DB` | `yousef_shtiwe` | PostgreSQL database name |
...
> The project also maintains a **[GitHub Wiki](https://github.com/samugit83/yousef_shtiwe/wiki)** with additional guides and walkthroughs.
