...
docker network create yousef_shtiwe-network
...
| `RECON_IMAGE` | `yousef_shtiwe-recon:latest` | Docker image for recon |
...
services:
  recon-orchestrator:
    build: .
    container_name: yousef_shtiwe-recon-orchestrator
    ports:
      - "8010:8010"
    volumes:
    ...
    environment:
      ...
      - RECON_IMAGE=yousef_shtiwe-recon:latest

networks:
  default:
    name: yousef_shtiwe-network
    external: true
```
...
1. Check Docker socket is accessible:
   ```bash
   docker exec yousef_shtiwe-recon-orchestrator docker ps
   ```

2. Verify recon image exists:
   ```bash
   docker images | grep yousef_shtiwe-recon
   ```

3. Check orchestrator logs:
   ```bash
   docker logs yousef_shtiwe-recon-orchestrator
   ```
...
2. Check container is running:
   ```bash
   docker ps | grep yousef_shtiwe-recon
   ```
