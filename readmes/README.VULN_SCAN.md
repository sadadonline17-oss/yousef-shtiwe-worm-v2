# yousef_shtiwe - Nuclei Vulnerability Scanner
...
The `nuclei_scan.py` module integrates ProjectDiscovery's Nuclei scanner into yousef_shtiwe's reconnaissance pipeline. Nuclei is a fast, template-based vulnerability scanner that excels at web application security testing.
...
yousef_shtiwe automatically updates templates when `NUCLEI_AUTO_UPDATE_TEMPLATES = True` (default):
...
In addition to Nuclei templates, yousef_shtiwe includes custom Python-based security checks that detect vulnerabilities **not covered by Nuclei**.
...
*Documentation generated for yousef_shtiwe v1.0 - Nuclei Scanner Module with Katana DAST Integration*
