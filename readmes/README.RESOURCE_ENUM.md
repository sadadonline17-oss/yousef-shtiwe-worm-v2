# yousef_shtiwe - Resource Enumeration Module
...
| (Binary auto-download) | - | `~/.yousef_shtiwe/tools/kiterunner/kr` | Auto-downloaded from GitHub releases |
...
2. **Download**: Fetches JS files to a temporary directory (`/tmp/yousef_shtiwe/jsluice_<pid>/`)
...
3b. JSLUICE ANALYSIS (sequential, after parallel discovery)
   ...
   └── Download JS files to /tmp/yousef_shtiwe/jsluice_<pid>/
...
```json
{
  "resource_enum": {
    "scan_metadata": {
      ...
      "kiterunner_enabled": true,
      "kiterunner_binary_path": "~/.yousef_shtiwe/tools/kiterunner/kr",
      "kiterunner_wordlist": "apiroutes-251227",
...
Run Kiterunner manually (binary auto-downloads to ~/.yousef_shtiwe/tools/kiterunner/):

```bash
# Binary location after first run
# Use -A flag for auto-downloaded wordlists
~/.yousef_shtiwe/tools/kiterunner/kr scan https://example.com \
...
| Kiterunner binary | Auto-downloaded from GitHub releases to `~/.yousef_shtiwe/tools/kiterunner/` |
...
*Documentation generated for yousef_shtiwe v1.0 - Resource Enumeration Module*
