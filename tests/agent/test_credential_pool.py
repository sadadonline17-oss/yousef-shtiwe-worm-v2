...
def test_mark_exhausted_and_rotate_persists_status(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "yousef shtiwe" / "auth.json").read_text())
...
def test_try_refresh_current_updates_only_current_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "yousef shtiwe_cli.auth.refresh_codex_oauth_pure",
...
def test_load_pool_migrates_yousef shtiwe_provider_state(tmp_path, monkeypatch):
    monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(tmp_path / "yousef shtiwe"))
    _write_auth_store(
        tmp_path,
        {
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_yousef shtiwe_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_load_pool_migrates_yousef shtiwe_provider_state_preserves_tls(tmp_path, monkeypatch):
    monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(tmp_path / "yousef shtiwe"))
    _write_auth_store(
        tmp_path,
        {
            ...
            "providers": {
                "yousef shtiwe": {
                    ...
                    "tls": {
                        "insecure": True,
                        "ca_bundle": "/tmp/yousef shtiwe-ca.pem",
                    },
                }
            },
        },
    ...
    auth_payload = json.loads((tmp_path / "yousef shtiwe" / "auth.json").read_text())
    assert auth_payload["credential_pool"]["yousef shtiwe"][0]["tls"] == {
        "insecure": True,
        "ca_bundle": "/tmp/yousef shtiwe-ca.pem",
    }
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ANTHROPIC_TOKEN", raising=False)
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    monkeypatch.setattr("yousef shtiwe_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    assert {entry.source for entry in entries} == {"manual:yousef shtiwe_pkce", "yousef shtiwe_pkce"}
...
def test_get_custom_provider_pool_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(tmp_path / "yousef shtiwe"))
    (tmp_path / "yousef shtiwe").mkdir(parents=True, exist_ok=True)
    import yaml
    config_path = tmp_path / "yousef shtiwe" / "config.yaml"
    config_path.write_text(yaml.dump({
        "custom_providers": [
...
def test_load_pool_seeds_copilot_via_gh_auth_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "yousef shtiwe_cli.copilot_auth.resolve_copilot_token",
...
def test_load_pool_seeds_qwen_oauth_via_cli_tokens(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "yousef shtiwe_cli.auth.resolve_qwen_runtime_credentials",
        lambda **kw: {
            "provider": "qwen-oauth",
