...
class TestSanitizeStructureNonAscii:
    def test_sanitizes_nested_dict_structure(self):
        payload = {
            "default_headers": {
                "X-Title": "YOUSEF SHTIWE │ Agent",
                "User-Agent": "YOUSEF SHTIWE/1.0 🤖",
            }
        }
        assert _sanitize_structure_non_ascii(payload) is True
        assert payload["default_headers"]["X-Title"] == "YOUSEF SHTIWE  Agent"
        assert payload["default_headers"]["User-Agent"] == "YOUSEF SHTIWE/1.0 "
