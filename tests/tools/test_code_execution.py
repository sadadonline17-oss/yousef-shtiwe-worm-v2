...
UDS socket lifecycle, yousef shtiwe_tools generation, timeout enforcement,
...
from tools.code_execution_tool import (
    ...
    execute_code,
    generate_yousef shtiwe_tools_module,
...
class TestYOUSEF SHTIWEToolsGeneration(unittest.TestCase):
    def test_generates_all_allowed_tools(self):
        src = generate_yousef shtiwe_tools_module(list(SANDBOX_ALLOWED_TOOLS))
        for tool in SANDBOX_ALLOWED_TOOLS:
            self.assertIn(f"def {tool}(", src)

    def test_generates_subset(self):
        src = generate_yousef shtiwe_tools_module(["terminal", "web_search"])
    ...
    def test_empty_list_generates_nothing(self):
        src = generate_yousef shtiwe_tools_module([])
    ...
    def test_non_allowed_tools_ignored(self):
        src = generate_yousef shtiwe_tools_module(["vision_analyze", "terminal"])
        self.assertIn("def terminal(", src)
        self.assertNotIn("def vision_analyze(", src)

    def test_rpc_infrastructure_present(self):
        src = generate_yousef shtiwe_tools_module(["terminal"])
        self.assertIn("YOUSEF SHTIWE_RPC_SOCKET", src)
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_yousef shtiwe_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"yousef shtiwe_rpc\")", src)
        self.assertNotIn('os.environ.get("YOUSEF SHTIWE_RPC_DIR", "/tmp/yousef shtiwe_rpc")', src)
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/yousef shtiwe_exec_", mkdir_cmd)
        self.assertIn("YOUSEF SHTIWE_RPC_DIR=/data/data/com.termux/files/usr/tmp/yousef shtiwe_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/yousef shtiwe_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/yousef shtiwe_exec_", mkdir_cmd)
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import yousef shtiwe_constants; print(yousef shtiwe_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("yousef shtiwe_constants.py", result["output"])

    def test_single_tool_call(self):
        """Script calls terminal and prints the result."""
        code = """
from yousef shtiwe_tools import terminal
...
"""
    ...
    def test_multi_tool_chain(self):
        ...
        code = """
from yousef shtiwe_tools import terminal, read_file
r1 = terminal("ls")
...
"""
        ...
        # terminal won't be in yousef shtiwe_tools.py, so import fails
        self.assertEqual(result["status"], "error")
...
"""
    ...
    def test_web_search_tool(self):
        ...
        code = """
from yousef shtiwe_tools import web_search
...
"""
    ...
    def test_json_parse_helper(self):
        ...
        code = r"""
from yousef shtiwe_tools import json_parse
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from yousef shtiwe_tools import shell_quote
...
"""
    ...
    def test_retry_helper_success(self):
        ...
        code = """
from yousef shtiwe_tools import retry
counter = [0]
def flaky():
    counter[0] += 1
...
class TestStubSchemaDrift(unittest.TestCase):
    ...
    def test_generated_module_accepts_all_params(self):
        """The generated yousef shtiwe_tools.py module should accept all current params
        without TypeError when called with keyword arguments."""
        src = generate_yousef shtiwe_tools_module(list(SANDBOX_ALLOWED_TOOLS))

        # Compile the generated module to check for syntax errors
        compile(src, "yousef shtiwe_tools.py", "exec")
...
class TestBuildExecuteCodeSchema(unittest.TestCase):
    ...
    def test_real_scenario_all_sandbox_tools_disabled(self):
        ...
        Scenario: user runs `yousef shtiwe tools code_execution` (only code_execution
    ...
    def test_real_scenario_only_vision_enabled(self):
        """Another real path: user runs `yousef shtiwe tools code_execution,vision`.
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_yousef shtiwe_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("YOUSEF SHTIWE_RPC_SOCKET", child_env)
...
class TestExecuteCodeEdgeCases(unittest.TestCase):
    ...
    def test_none_enabled_tools_uses_all(self):
        ...
        code = (
            "from yousef shtiwe_tools import terminal, web_search, read_file\n"
    ...
    def test_empty_enabled_tools_uses_all(self):
        ...
        code = (
            "from yousef shtiwe_tools import terminal, web_search\n"
            "print('imports ok')\n"
        )
        with patch("model_tools.handle_function_call",
                    return_value=json.dumps({"ok": True})):
