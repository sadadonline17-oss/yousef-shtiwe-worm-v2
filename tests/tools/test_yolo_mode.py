"""Tests for --yolo (SHADOW_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    """When SHADOW_YOLO_MODE is set, all dangerous commands are auto-approved."""
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("SHADOW_SESSION_KEY", "test-session")
        monkeypatch.delenv("SHADOW_YOLO_MODE", raising=False)
        monkeypatch.delenv("SHADOW_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("SHADOW_EXEC_ASK", raising=False)
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With SHADOW_YOLO_MODE, dangerous commands are auto-approved."""
        monkeypatch.setenv("SHADOW_YOLO_MODE", "1")
        monkeypatch.setenv("SHADOW_INTERACTIVE", "1")
        monkeypatch.setenv("SHADOW_SESSION_KEY", "test-session")
    ...
    def test_yolo_mode_not_set_by_default(self):
        """SHADOW_YOLO_MODE should not be set by default."""
        # Clean env check — if it happens to be set in test env, that's fine,
        # we just verify the mechanism exists
        assert os.getenv("SHADOW_YOLO_MODE") is None or True  # no-op, documents intent

    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for SHADOW_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("SHADOW_YOLO_MODE", "")
        monkeypatch.setenv("SHADOW_INTERACTIVE", "1")
        monkeypatch.setenv("SHADOW_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("SHADOW_YOLO_MODE") returns ""
