...
Tests cover:
- managed_yousef shtiwe_tools_enabled() feature flag
...
# managed_yousef shtiwe_tools_enabled
# ---------------------------------------------------------------------------
class TestManagedYousef ShtiweToolsEnabled:
    """Feature flag driven by YOUSEF SHTIWE_ENABLE_Yousef Shtiwe_MANAGED_TOOLS."""

    def test_disabled_by_default(self, monkeypatch):
        monkeypatch.delenv("YOUSEF SHTIWE_ENABLE_Yousef Shtiwe_MANAGED_TOOLS", raising=False)
        assert managed_yousef shtiwe_tools_enabled() is False

    @pytest.mark.parametrize("val", ["1", "true", "True", "yes"])
    def test_enabled_when_truthy(self, monkeypatch, val):
        monkeypatch.setenv("YOUSEF SHTIWE_ENABLE_Yousef Shtiwe_MANAGED_TOOLS", val)
        assert managed_yousef shtiwe_tools_enabled() is True
...
class TestResolveModalBackendState:
    ...
    @staticmethod
    def _resolve(monkeypatch, mode, *, has_direct, managed_ready, yousef shtiwe_enabled=False):
        """Helper to call resolve_modal_backend_state with feature flag control."""
        if yousef shtiwe_enabled:
            monkeypatch.setenv("YOUSEF SHTIWE_ENABLE_Yousef Shtiwe_MANAGED_TOOLS", "1")
        else:
            monkeypatch.setenv("YOUSEF SHTIWE_ENABLE_Yousef Shtiwe_MANAGED_TOOLS", "")
    ...
    def test_auto_prefers_managed_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=True, managed_ready=True, yousef shtiwe_enabled=True)
        assert result["selected_backend"] == "managed"

    def test_auto_falls_back_to_direct(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=True, managed_ready=False, yousef shtiwe_enabled=True)
    ...
    def test_auto_managed_ready_but_yousef shtiwe_disabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=True, managed_ready=True, yousef shtiwe_enabled=False)
        assert result["selected_backend"] == "direct"

    def test_auto_nothing_when_only_managed_and_yousef shtiwe_disabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=False, managed_ready=True, yousef shtiwe_enabled=False)
        assert result["selected_backend"] is None

    # --- direct mode ---

    def test_direct_selects_direct_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=True, managed_ready=True, yousef shtiwe_enabled=True)
        assert result["selected_backend"] == "direct"

    def test_direct_none_when_no_credentials(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=False, managed_ready=True, yousef shtiwe_enabled=True)
        assert result["selected_backend"] is None

    # --- managed mode ---

    def test_managed_selects_managed_when_ready_and_enabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "managed", has_direct=True, managed_ready=True, yousef shtiwe_enabled=True)
        assert result["selected_backend"] == "managed"

    def test_managed_none_when_not_ready(self, monkeypatch):
        result = self._resolve(monkeypatch, "managed", has_direct=True, managed_ready=False, yousef shtiwe_enabled=True)
        assert result["selected_backend"] is None

    def test_managed_blocked_when_yousef shtiwe_disabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "managed", has_direct=True, managed_ready=True, yousef shtiwe_enabled=False)
        assert result["selected_backend"] is None
