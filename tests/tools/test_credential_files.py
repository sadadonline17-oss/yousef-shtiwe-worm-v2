...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        (yousef shtiwe_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(yousef shtiwe_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.yousef shtiwe/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        yousef shtiwe_home = tmp_path / ".yousef shtiwe"
        yousef shtiwe_home.mkdir()
        (yousef shtiwe_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        yousef shtiwe_home = tmp_path / ".yousef shtiwe"
        yousef shtiwe_home.mkdir()
        (yousef shtiwe_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        yousef shtiwe_home = tmp_path / ".yousef shtiwe"
        yousef shtiwe_home.mkdir()
        (yousef shtiwe_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.yousef shtiwe/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        yousef shtiwe_home = tmp_path / ".yousef shtiwe"
        (yousef shtiwe_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"YOUSEF SHTIWE_HOME": str(yousef shtiwe_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.yousef shtiwe")

        assert mounts[0]["container_path"] == "/home/user/.yousef shtiwe/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.yousef shtiwe/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.yousef shtiwe/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape YOUSEF SHTIWE_HOME."""
        monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(tmp_path / ".yousef shtiwe"))
        (tmp_path / ".yousef shtiwe").mkdir()

        # Create a sensitive file one level above yousef shtiwe_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside yousef shtiwe_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside YOUSEF SHTIWE_HOME must still be registered."""
        yousef shtiwe_home = tmp_path / ".yousef shtiwe"
        yousef shtiwe_home.mkdir()
        monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(yousef shtiwe_home))
        (yousef shtiwe_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_yousef shtiwe_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of YOUSEF SHTIWE_HOME must be allowed."""
        yousef shtiwe_home = tmp_path / ".yousef shtiwe"
        yousef shtiwe_home.mkdir()
        subdir = yousef shtiwe_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside YOUSEF SHTIWE_HOME pointing outside must be rejected."""
        yousef shtiwe_home = tmp_path / ".yousef shtiwe"
        yousef shtiwe_home.mkdir()
        monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(yousef shtiwe_home))

        # Create a sensitive file outside yousef shtiwe_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside yousef shtiwe_home pointing outside
        symlink = yousef shtiwe_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes YOUSEF SHTIWE_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, yousef shtiwe_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape YOUSEF SHTIWE_HOME."""
        yousef shtiwe_home = tmp_path / ".yousef shtiwe"
        yousef shtiwe_home.mkdir()
        monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(yousef shtiwe_home))
        ...
        sensitive.write_text("{}")
        self._write_config(yousef shtiwe_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(yousef shtiwe_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside YOUSEF SHTIWE_HOME via config must still mount."""
        yousef shtiwe_home = tmp_path / ".yousef shtiwe"
        yousef shtiwe_home.mkdir()
        monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(yousef shtiwe_home))

        (yousef shtiwe_home / "oauth.json").write_text("{}")
        self._write_config(yousef shtiwe_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        yousef shtiwe_home = tmp_path / ".yousef shtiwe"
        yousef shtiwe_home.mkdir()
        (yousef shtiwe_home / "cache" / "documents").mkdir(parents=True)
        (yousef shtiwe_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(yousef shtiwe_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.yousef shtiwe/cache/documents" in paths
        assert "/root/.yousef shtiwe/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.yousef shtiwe/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        yousef shtiwe_home = tmp_path / ".yousef shtiwe"
        yousef shtiwe_home.mkdir()
        # Use legacy dir name — get_yousef shtiwe_dir prefers old if it exists
        (yousef shtiwe_home / "document_cache").mkdir()
        (yousef shtiwe_home / "image_cache").mkdir()
        monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(yousef shtiwe_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(yousef shtiwe_home / "document_cache") in host_paths
        assert str(yousef shtiwe_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.yousef shtiwe/cache/documents" in container_paths
        assert "/root/.yousef shtiwe/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        yousef shtiwe_home = tmp_path / ".yousef shtiwe"
        doc_dir = yousef shtiwe_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        yousef shtiwe_home = tmp_path / ".yousef shtiwe"
        ss_dir = yousef shtiwe_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.yousef shtiwe/cache/screenshots/session_abc/screen1.png"
