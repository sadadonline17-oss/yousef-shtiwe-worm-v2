...
def _clean_state():
    ...
    saved = {}
    for k in ("SHADOW_INTERACTIVE", "SHADOW_GATEWAY_SESSION", "SHADOW_EXEC_ASK", "SHADOW_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["SHADOW_INTERACTIVE"] = "1"
        session_key = os.getenv("SHADOW_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No SHADOW_INTERACTIVE or SHADOW_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
