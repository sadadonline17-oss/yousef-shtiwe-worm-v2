...
def _clean_state():
    ...
    saved = {}
    for k in ("YOUSEF SHTIWE_INTERACTIVE", "YOUSEF SHTIWE_GATEWAY_SESSION", "YOUSEF SHTIWE_EXEC_ASK", "YOUSEF SHTIWE_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["YOUSEF SHTIWE_INTERACTIVE"] = "1"
        session_key = os.getenv("YOUSEF SHTIWE_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No YOUSEF SHTIWE_INTERACTIVE or YOUSEF SHTIWE_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
