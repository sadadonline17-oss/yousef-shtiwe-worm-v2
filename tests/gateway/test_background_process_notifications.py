...
class TestLoadBackgroundNotificationsMode:
    ...
    def test_defaults_to_all(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(gw, "_yousef shtiwe_home", tmp_path)
        monkeypatch.delenv("YOUSEF SHTIWE_BACKGROUND_NOTIFICATIONS", raising=False)
    ...
    def test_env_var_overrides_config(self, monkeypatch, tmp_path):
        ...
        import gateway.run as gw
        monkeypatch.setattr(gw, "_yousef shtiwe_home", tmp_path)
        monkeypatch.setenv("YOUSEF SHTIWE_BACKGROUND_NOTIFICATIONS", "off")
