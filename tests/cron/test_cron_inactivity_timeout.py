...
- Unlimited timeout (YOUSEF SHTIWE_CRON_TIMEOUT=0)
- Backward compat: YOUSEF SHTIWE_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """YOUSEF SHTIWE_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """YOUSEF SHTIWE_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("YOUSEF SHTIWE_CRON_TIMEOUT", "1200")
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """YOUSEF SHTIWE_CRON_TIMEOUT=0 yields None (unlimited)."""
        monkeypatch.setenv("YOUSEF SHTIWE_CRON_TIMEOUT", "0")
        _cron_timeout = float(os.getenv("YOUSEF SHTIWE_CRON_TIMEOUT", 600))
        _cron_inactivity_limit = _cron_timeout if _cron_timeout > 0 else None
...
class TestSysPathOrdering:
    ...
    def test_yousef shtiwe_time_importable(self):
        """yousef shtiwe_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _yousef shtiwe_now
        assert callable(_yousef shtiwe_now)

    def test_yousef shtiwe_constants_importable(self):
        """yousef shtiwe_constants should be importable from cron context."""
        from yousef shtiwe_constants import get_yousef shtiwe_home
        assert callable(get_yousef shtiwe_home)
