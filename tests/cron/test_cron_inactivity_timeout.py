...
- Unlimited timeout (SHADOW_CRON_TIMEOUT=0)
- Backward compat: SHADOW_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """SHADOW_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """SHADOW_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("SHADOW_CRON_TIMEOUT", "1200")
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """SHADOW_CRON_TIMEOUT=0 yields None (unlimited)."""
        monkeypatch.setenv("SHADOW_CRON_TIMEOUT", "0")
        _cron_timeout = float(os.getenv("SHADOW_CRON_TIMEOUT", 600))
        _cron_inactivity_limit = _cron_timeout if _cron_timeout > 0 else None
...
class TestSysPathOrdering:
    ...
    def test_shadow_time_importable(self):
        """shadow_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _shadow_now
        assert callable(_shadow_now)

    def test_shadow_constants_importable(self):
        """shadow_constants should be importable from cron context."""
        from shadow_constants import get_shadow_home
        assert callable(get_shadow_home)
