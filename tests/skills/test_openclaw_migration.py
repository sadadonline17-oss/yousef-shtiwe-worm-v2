...
"""
    ...
    assert "Tyler Williams > Active Projects: YOUSEF SHTIWE Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `yousef shtiwe cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # yousef shtiwe_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.yousef shtiwe/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "yousef shtiwe_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "YOUSEF SHTIWE prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told YOUSEF SHTIWE to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "YOUSEF SHTIWE config is great"
    assert mod.rebrand_text("openclaw should always respond concisely") == "YOUSEF SHTIWE should always respond concisely"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "YOUSEF SHTIWE uses tools well"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "YOUSEF SHTIWE remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "YOUSEF SHTIWE prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "YOUSEF SHTIWE was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "YOUSEF SHTIWE uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "YOUSEF SHTIWE said to ask YOUSEF SHTIWE about YOUSEF SHTIWE settings"
...
def test_migrate_memory_rebrands_entries(tmp_path):
    ...
    target_root = tmp_path / "yousef shtiwe"
    target_root.mkdir()
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are YOUSEF SHTIWE" in result
