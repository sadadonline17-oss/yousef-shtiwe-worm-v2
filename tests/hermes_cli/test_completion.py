"""Tests for yousef shtiwe_cli/completion.py — shell completion script generation."""
...
from yousef shtiwe_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real yousef shtiwe structure."""
    p = argparse.ArgumentParser(prog="yousef shtiwe")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_yousef shtiwe_completion()" in out
        assert "complete -F _yousef shtiwe_completion yousef shtiwe" in out
...
class TestGenerateZsh:
    def test_contains_compdef_header(self):
        ...
        assert "#compdef yousef shtiwe" in out
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c yousef shtiwe -f" in out
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        out = generate_bash(_make_parser())
        assert "_yousef shtiwe_profiles()" in out
        assert 'profiles_dir="$HOME/.yousef shtiwe/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'yousef shtiwe profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _yousef shtiwe_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_yousef shtiwe_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _yousef shtiwe_profiles"

    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_yousef shtiwe_profiles()" in out
        assert "$HOME/.yousef shtiwe/profiles" in out
    ...
    def test_fish_has_profiles_helper(self):
        out = generate_fish(_make_parser())
        assert "__yousef shtiwe_profiles" in out
        assert "$HOME/.yousef shtiwe/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__yousef shtiwe_profiles" in out
        count = out.count("(__yousef shtiwe_profiles)")
