"""Tests for yousef shtiwe_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestSaveDisabledSkills:
    @patch("yousef shtiwe_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from yousef shtiwe_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("yousef shtiwe_cli.config.load_config")
    ...
    @patch("yousef shtiwe_cli.config.load_config")
    @patch.dict("os.environ", {"YOUSEF SHTIWE_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("YOUSEF SHTIWE_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """YOUSEF SHTIWE_SESSION_PLATFORM should be used when YOUSEF SHTIWE_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_yousef shtiwe_platform_takes_precedence(self, tmp_path, monkeypatch):
        """YOUSEF SHTIWE_PLATFORM should win over YOUSEF SHTIWE_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(tmp_path))
        monkeypatch.setenv("YOUSEF SHTIWE_PLATFORM", "telegram")
        monkeypatch.setenv("YOUSEF SHTIWE_SESSION_PLATFORM", "discord")
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(tmp_path))
        monkeypatch.setenv("YOUSEF SHTIWE_PLATFORM", "telegram")
        monkeypatch.setenv("YOUSEF SHTIWE_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from yousef shtiwe_cli.skills_config import _get_categories
        skills = [
