"""Tests for the YOUSEF SHTIWE plugin system (yousef shtiwe_cli.plugins)."""
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.yousef shtiwe/plugins/ are discovered."""
        plugins_dir = tmp_path / "yousef shtiwe_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(tmp_path / "yousef shtiwe_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("YOUSEF SHTIWE_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".yousef shtiwe" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under yousef shtiwe_plugins.<name>."""
        plugins_dir = tmp_path / "yousef shtiwe_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(tmp_path / "yousef shtiwe_test"))

        # Clean up any prior namespace module
        sys.modules.pop("yousef shtiwe_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "yousef shtiwe_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="yousef shtiwe_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "yousef shtiwe_cli.plugins.invoke_hook",
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import yousef shtiwe_cli.plugins as plugins_mod
...
# in PluginContext (yousef shtiwe_cli/plugins.py).  The tests referenced _plugin_commands,
