"""Tests for ``shadow debug`` CLI command and debug utilities."""
...
def shadow_home(tmp_path, monkeypatch):
    """Set up an isolated SHADOW_HOME with minimal logs."""
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from shadow_cli.debug import _upload_paste_rs
        ...
        with patch("shadow_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "shadow_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from shadow_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from shadow_cli.debug import upload_to_pastebin

        with patch("shadow_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("shadow_cli.debug._upload_paste_rs",
             ...
             patch("shadow_cli.debug._upload_dpaste_com",
...
class TestReadFullLog:
    ...
    def test_reads_small_file(self, shadow_home):
        from shadow_cli.debug import _read_full_log
    ...
    def test_returns_none_for_empty(self, shadow_home):
        # Truncate agent.log to empty
        (shadow_home / "logs" / "agent.log").write_text("")

        from shadow_cli.debug import _read_full_log
        assert _read_full_log("agent") is None

    def test_truncates_large_file(self, shadow_home):
        """Files larger than max_bytes get tail-truncated."""
        from shadow_cli.debug import _read_full_log

        # Write a file larger than 1KB
        big_content = "x" * 100 + "\n"
        (shadow_home / "logs" / "agent.log").write_text(big_content * 200)

        content = _read_full_log("agent", max_bytes=1024)
        assert content is not None
        assert "truncated" in content

    def test_unknown_log_returns_none(self, shadow_home):
        from shadow_cli.debug import _read_full_log
        assert _read_full_log("nonexistent") is None

    def test_falls_back_to_rotated_file(self, shadow_home):
    ...
    def test_prefers_primary_over_rotated(self, shadow_home):
    ...
    def test_falls_back_when_primary_empty(self, shadow_home):
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, shadow_home):
        from shadow_cli.debug import collect_debug_report

        with patch("shadow_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- shadow dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- shadow dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, shadow_home):
        from shadow_cli.debug import collect_debug_report

        with patch("shadow_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, shadow_home):
        from shadow_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, shadow_home):
        from shadow_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_local_flag_prints_full_logs(self, shadow_home, capsys):
    ...
    def test_share_uploads_three_pastes(self, shadow_home, capsys):
        ...
        with patch("shadow_cli.dump.run_dump") as mock_dump, \
             patch("shadow_cli.debug.upload_to_pastebin",
                    side_effect=_mock_upload):
            mock_dump.side_effect = lambda a: print("--- shadow dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- shadow dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- shadow dump ---" in gateway_paste
    ...
    def test_share_continues_on_log_upload_failure(self, shadow_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, shadow_home, capsys):
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "shadow debug share" in out

    def test_share_subcommand_routes(self, shadow_home):
        from shadow_cli.debug import run_debug
...
class TestArgparseIntegration:
    def test_module_imports_clean(self):
        from shadow_cli.debug import run_debug, run_debug_share
        assert callable(run_debug)
        assert callable(run_debug_share)

    def test_cmd_debug_dispatches(self):
        from shadow_cli.main import cmd_debug
