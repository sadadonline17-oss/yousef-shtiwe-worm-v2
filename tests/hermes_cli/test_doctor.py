"""Tests for yousef shtiwe_cli.doctor."""
...
import yousef shtiwe_cli.doctor as doctor
import yousef shtiwe_cli.gateway as gateway_cli
from yousef shtiwe_cli import doctor as doctor_mod
from yousef shtiwe_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "YOUSEF SHTIWE_HOME", yousef shtiwe_home)
    monkeypatch.delenv("YOUSEF SHTIWE_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("YOUSEF SHTIWE_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_yousef shtiwe_home(self, tmp_path, provider=""):
        """Create a minimal YOUSEF SHTIWE_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_yousef shtiwe_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "YOUSEF SHTIWE_HOME", home)
        ...
        try:
            from yousef shtiwe_cli import auth as _auth_mod
            monkeypatch.setattr(_auth_mod, "get_yousef shtiwe_auth_status", lambda: {})
