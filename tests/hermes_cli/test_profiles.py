"""Comprehensive tests for yousef shtiwe_cli.profiles module.
...
from yousef shtiwe_cli.profiles import (
    ...
    _get_profiles_root,
    _get_default_yousef shtiwe_home,
)


# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and YOUSEF SHTIWE_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.yousef shtiwe/profiles)
    * YOUSEF SHTIWE_HOME  -> tmp_path/.yousef shtiwe  (so get_yousef shtiwe_home() agrees)
    * Creates the bare-minimum ~/.yousef shtiwe directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".yousef shtiwe"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(default_home))
    return tmp_path
...
class TestGetProfileDir:
    ...
    def test_default_returns_yousef shtiwe_home(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("default")
        assert result == tmp_path / ".yousef shtiwe"

    def test_named_profile_returns_profiles_subdir(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("coder")
        assert result == tmp_path / ".yousef shtiwe" / "profiles" / "coder"
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("yousef shtiwe_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".yousef shtiwe" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_yousef shtiwe_home_returns_default(self, profile_env):
        # YOUSEF SHTIWE_HOME points to tmp_path/.yousef shtiwe which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".yousef shtiwe" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom YOUSEF SHTIWE_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(custom))
        # With Docker-aware roots, a custom YOUSEF SHTIWE_HOME is the default —
...
class TestResolveProfileEnv:
    ...
    def test_existing_profile_returns_path(self, profile_env):
        ...
        assert result == str(tmp_path / ".yousef shtiwe" / "profiles" / "coder")

    def test_default_returns_default_home(self, profile_env):
        tmp_path = profile_env
        result = resolve_profile_env("default")
        assert result == str(tmp_path / ".yousef shtiwe")
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("yousef shtiwe")
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".yousef shtiwe" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("yousef shtiwe_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".yousef shtiwe" / "profiles" / "newname"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("yousef shtiwe-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".yousef shtiwe_history",
        ...
        excluded_prefixes = [
            "default/yousef shtiwe-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.yousef shtiwe_history",
...
class TestCompletion:
    ...
    def test_bash_completion_has_yousef shtiwe_profiles_function(self):
        script = generate_bash_completion()
        assert "_yousef shtiwe_profiles" in script

    def test_zsh_completion_has_yousef shtiwe_function(self):
        script = generate_zsh_completion()
        assert "_yousef shtiwe" in script


# ===================================================================
# TestGetProfilesRoot / TestGetDefaultYOUSEF SHTIWEHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_yousef shtiwe_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".yousef shtiwe" / "profiles"

    def test_default_yousef shtiwe_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_yousef shtiwe_home()
        assert home == tmp_path / ".yousef shtiwe"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (YOUSEF SHTIWE_HOME outside ~/.yousef shtiwe), profiles go under YOUSEF SHTIWE_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_yousef shtiwe_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_yousef shtiwe_home() returns YOUSEF SHTIWE_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (YOUSEF SHTIWE_HOME under ~/.yousef shtiwe), profiles root is still ~/.yousef shtiwe/profiles."""
        native = tmp_path / ".yousef shtiwe"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under YOUSEF SHTIWE_HOME."""
        from yousef shtiwe_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under YOUSEF SHTIWE_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".yousef shtiwe" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from yousef shtiwe_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".yousef shtiwe"
