"""Tests for yousef shtiwe_cli.gateway."""
...
def test_systemd_install_system_scope_skips_linger_and_uses_systemctl(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "etc" / "systemd" / "system" / "yousef shtiwe-gateway.service"
...
def test_conflicting_systemd_units_warning(monkeypatch, tmp_path, capsys):
    user_unit = tmp_path / "user" / "yousef shtiwe-gateway.service"
    system_unit = tmp_path / "system" / "yousef shtiwe-gateway.service"
    ...
    assert "yousef shtiwe gateway uninstall" in out
...
def test_install_linux_gateway_from_setup_system_choice_without_root_prints_followup(monkeypatch, capsys):
    ...
    assert "sudo yousef shtiwe gateway install --system --run-as-user alice" in out
    assert "sudo yousef shtiwe gateway start --system" in out
