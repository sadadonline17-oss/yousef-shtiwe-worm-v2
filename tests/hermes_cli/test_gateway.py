...
def test_systemd_install_system_scope_skips_linger_and_uses_systemctl(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "etc" / "systemd" / "system" / "shadow-gateway.service"
...
def test_conflicting_systemd_units_warning(monkeypatch, tmp_path, capsys):
    user_unit = tmp_path / "user" / "shadow-gateway.service"
    system_unit = tmp_path / "system" / "shadow-gateway.service"
    ...
    assert "shadow gateway uninstall" in out
...
def test_install_linux_gateway_from_setup_system_choice_without_root_prints_followup(monkeypatch, capsys):
    ...
    assert "sudo shadow gateway install --system --run-as-user alice" in out
    assert "sudo shadow gateway start --system" in out
