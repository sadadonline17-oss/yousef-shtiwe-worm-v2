...
def _make_cli(tmp_path, mcp_servers=None):
    """Create a minimal SHADOWCLI instance with mocked config."""
    import cli as cli_mod
    obj = object.__new__(cli_mod.SHADOWCLI)
...
class TestMCPConfigWatch:
    ...
    def test_no_change_does_not_reload(self, tmp_path):
        ...
        with patch("shadow_cli.config.get_config_path", return_value=cfg_file):
    ...
    def test_missing_config_file_does_not_crash(self, tmp_path):
        ...
        with patch("shadow_cli.config.get_config_path", return_value=missing):
