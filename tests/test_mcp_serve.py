"""
Tests for mcp_serve — YOUSEF SHTIWE MCP server.
...
def _isolate_yousef shtiwe_home(tmp_path, monkeypatch):
    """Redirect YOUSEF SHTIWE_HOME to a temp directory."""
    monkeypatch.setenv("YOUSEF SHTIWE_HOME", str(tmp_path))
    try:
        import yousef shtiwe_constants
        monkeypatch.setattr(yousef shtiwe_constants, "get_yousef shtiwe_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking yousef shtiwe_state schema."""
