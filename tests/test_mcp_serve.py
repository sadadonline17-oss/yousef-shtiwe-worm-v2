"""
Tests for mcp_serve — SHADOW MCP server.
...
def _isolate_shadow_home(tmp_path, monkeypatch):
    """Redirect SHADOW_HOME to a temp directory."""
    monkeypatch.setenv("SHADOW_HOME", str(tmp_path))
    try:
        import shadow_constants
        monkeypatch.setattr(shadow_constants, "get_shadow_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking shadow_state schema."""
