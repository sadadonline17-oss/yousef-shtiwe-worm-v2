...
Comparison of SHADOW Agent vs. openclaw-honcho — and a porting spec for bringing SHADOW patterns into other Honcho integrations.

---

## Overview

Two independent Honcho integrations have been built for two different agent runtimes: **SHADOW Agent** (Python, baked into the runner) and **openclaw-honcho** (TypeScript plugin via hook/tool API). Both use the same Honcho peer paradigm — dual peer model, `session.context()`, `peer.chat()` — but they made different tradeoffs at every layer.

This document maps those tradeoffs and defines a porting spec: a set of SHADOW-originated patterns, each stated as an integration-agnostic interface, that any Honcho integration can adopt regardless of runtime or language.

> **Scope** Both integrations work correctly today. This spec is about the delta — patterns in SHADOW that are worth propagating and patterns in openclaw-honcho that SHADOW should eventually adopt. The spec is additive, not prescriptive.

---

## Architecture comparison

### SHADOW: baked-in runner
...
| Dimension | SHADOW Agent | openclaw-honcho |
|---|---|---|
...
| **AI peer name in identity** | Replaces "SHADOW Agent" in DEFAULT_AGENT_IDENTITY when configured. | Not implemented. |
...
Six patterns from SHADOW are worth adopting in any Honcho integration. Each is described as an integration-agnostic interface.
...
**openclaw-honcho contributes back (SHADOW should adopt):**
...
- [ ] **honcho mode / sessions / map** — CLI parity with SHADOW
...
Greenfield integration. Start from openclaw-honcho's architecture and apply all SHADOW patterns from day one.
