"""Centralized logging setup for YOUSEF SHTIWE Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.yousef shtiwe/logs/`` (profile-aware via ``get_yousef shtiwe_home()``).
...
from yousef shtiwe_constants import get_config_path, get_yousef shtiwe_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_yousef shtiwe_session_injector", False):
    ...
    _session_record_factory._yousef shtiwe_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``yousef shtiwe logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("yousef shtiwe_cli", "cli"),
    "cron": ("cron",),
}
...
def setup_logging(
    *,
    ...
    """Configure the YOUSEF SHTIWE logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    yousef shtiwe_home
        Override for the YOUSEF SHTIWE home directory.  Falls back to
        ``get_yousef shtiwe_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_yousef shtiwe_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._yousef shtiwe_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the yousef shtiwe group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from yousef shtiwe_cli.config import is_managed
