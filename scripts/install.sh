...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.shadow/shadow-agent)"
...
get_shadow_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/shadow" ]; then
        echo "$link_dir/shadow"
    else
        echo "shadow"
    fi
}
...
check_node() {
    ...
    if [ -x "$SHADOW_HOME/node/bin/node" ]; then
        export PATH="$SHADOW_HOME/node/bin:$PATH"
        local found_ver=$("$SHADOW_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (SHADOW-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.shadow/node/..."
    ...
    # Place into ~/.shadow/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$SHADOW_HOME/node"
    mkdir -p "$SHADOW_HOME"
    mv "$extracted_dir" "$SHADOW_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$SHADOW_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$SHADOW_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$SHADOW_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$SHADOW_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$SHADOW_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.shadow/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "SHADOW Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="shadow-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if SHADOW behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up shadow command..."

    if [ "$USE_VENV" = true ]; then
        SHADOW_BIN="$INSTALL_DIR/venv/bin/shadow"
    else
        SHADOW_BIN="$(which shadow 2>/dev/null || echo "")"
        if [ -z "$SHADOW_BIN" ]; then
            log_warn "shadow not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$SHADOW_BIN" ]; then
        log_warn "shadow entry point not found at $SHADOW_BIN"
    ...
    # Create a user-facing shim for the shadow command.
    mkdir -p "$command_link_dir"
    ln -sf "$SHADOW_BIN" "$command_link_dir/shadow"
    log_success "Symlinked shadow → $command_link_display_dir/shadow"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "shadow command ready"
        return 0
    fi
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# SHADOW Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# SHADOW Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so shadow works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.shadow directory structure (config at top level, code in subdir)
    mkdir -p "$SHADOW_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.shadow/.env (top level, easy to find)
    if [ ! -f "$SHADOW_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$SHADOW_HOME/.env"
            log_success "Created ~/.shadow/.env from template"
        else
            touch "$SHADOW_HOME/.env"
            log_success "Created ~/.shadow/.env"
        fi
    else
        log_info "~/.shadow/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.shadow/config.yaml (top level, easy to find)
    if [ ! -f "$SHADOW_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$SHADOW_HOME/config.yaml"
            log_success "Created ~/.shadow/config.yaml from template"
        fi
    else
        log_info "~/.shadow/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$SHADOW_HOME/SOUL.md" ]; then
        cat > "$SHADOW_HOME/SOUL.md" << 'SOUL_EOF'
...
-->
SOUL_EOF
        log_success "Created ~/.shadow/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.shadow/"

    # Seed bundled skills into ~/.shadow/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.shadow/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.shadow/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$SHADOW_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$SHADOW_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.shadow/skills/"
        fi
    fi
}
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — SHADOW itself does not require root access."
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'shadow setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m shadow_cli.main setup < /dev/tty
    else
        python -m shadow_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for SHADOW to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$SHADOW_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'shadow whatsapp' to pair via QR code..."
            echo ""
            read -p "Pair WhatsApp now? [Y/n] " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
                SHADOW_CMD="$(get_shadow_command_path)"
                $SHADOW_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'shadow whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'shadow gateway install' later."
        return 0
    fi
    ...
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $SHADOW_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $SHADOW_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: shadow gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: shadow gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $SHADOW_CMD gateway > "$SHADOW_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.shadow/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: shadow gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: shadow gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.shadow/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.shadow/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.shadow/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.shadow/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.shadow/shadow-agent/"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}shadow${NC}              Start chatting"
    echo -e "   ${GREEN}shadow setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}shadow config${NC}       View/edit configuration"
    echo -e "   ${GREEN}shadow config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}shadow gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}shadow update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'shadow' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'shadow' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
