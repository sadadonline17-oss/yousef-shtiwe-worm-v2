# Contributing to SHADOW Agent

Thank you for contributing to SHADOW Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want SHADOW to work everywhere.
...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `shadow skills browse` (labeled "official") and install it with `shadow skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [SHADOW-OVERLORD Discord](https://discord.gg/SHADOW-OVERLORD). Users can install it with `shadow skills install`.
...
├── cli.py                    # SHADOWCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (shadow-cli, shadow-telegram, etc.)
├── shadow_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── shadow_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.shadow/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (shadow-agent.shadow-overlord.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.shadow/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.shadow/`)
...
| `~/.shadow/sessions/` | JSON session logs |
| `~/.shadow/cron/` | Scheduled job data |
| `~/.shadow/whatsapp/session/` | WhatsApp bridge credentials |
...
Four fields are supported under `metadata.shadow`:
...
The user may skip setup and keep loading the skill. SHADOW only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `shadow setup` or update `~/.shadow/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing SHADOW tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `shadow --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

SHADOW uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.shadow/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `shadow_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `shadow_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

SHADOW runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
- **Discord**: [discord.gg/SHADOW-OVERLORD](https://discord.gg/SHADOW-OVERLORD) — for questions, showcasing projects, and sharing skills
