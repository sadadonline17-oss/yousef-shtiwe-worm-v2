# Contributing to YOUSEF SHTIWE Agent

Thank you for contributing to YOUSEF SHTIWE Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want YOUSEF SHTIWE to work everywhere.
...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `yousef shtiwe skills browse` (labeled "official") and install it with `yousef shtiwe skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [YOUSEF SHTIWE-OVERLORD Discord](https://discord.gg/YOUSEF SHTIWE-OVERLORD). Users can install it with `yousef shtiwe skills install`.
...
├── cli.py                    # YOUSEF SHTIWECLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (yousef shtiwe-cli, yousef shtiwe-telegram, etc.)
├── yousef shtiwe_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── yousef shtiwe_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── config.py                 # Config management, migration, env var definitions
│   ├── setup.py                  # Interactive setup wizard
│   ├── auth.py                   # Provider resolution, OAuth, Yousef Shtiwe Portal
...
│
├── skills/                   # Bundled skills (copied to ~/.yousef shtiwe/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (yousef shtiwe-agent.yousef shtiwe-overlord.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.yousef shtiwe/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.yousef shtiwe/`)

| Path | Purpose |
|------|---------|
| `~/.yousef shtiwe/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.yousef shtiwe/.env` | API keys and secrets |
| `~/.yousef shtiwe/auth.json` | OAuth credentials (Yousef Shtiwe Portal) |
| `~/.yousef shtiwe/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.yousef shtiwe/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.yousef shtiwe/state.db` | SQLite session database |
| `~/.yousef shtiwe/sessions/` | JSON session logs |
| `~/.yousef shtiwe/cron/` | Scheduled job data |
| `~/.yousef shtiwe/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`yousef shtiwe_state.py`) with full-text search and unique session titles. JSON logs go to `~/.yousef shtiwe/sessions/`.
- **Ephemeral injection**: System prompts and prefill messages are injected at API call time, never persisted to the database or logs.
- **Provider abstraction**: The agent works with any OpenAI-compatible API. Provider resolution happens at init time (Yousef Shtiwe Portal OAuth, OpenRouter API key, or custom endpoint).
...
Four fields are supported under `metadata.yousef shtiwe`:
...
The user may skip setup and keep loading the skill. YOUSEF SHTIWE only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `yousef shtiwe setup` or update `~/.yousef shtiwe/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing YOUSEF SHTIWE tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `yousef shtiwe --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

YOUSEF SHTIWE uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.yousef shtiwe/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `yousef shtiwe_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `yousef shtiwe_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

YOUSEF SHTIWE runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
| **Write deny list** | Protected paths (`~/.ssh/authorized_keys`, `/etc/yousef shtiwe`) resolved via `os.path.realpath()` to prevent symlink bypass |
...
- **Discord**: [discord.gg/YOUSEF SHTIWE-OVERLORD](https://discord.gg/YOUSEF SHTIWE-OVERLORD) — for questions, showcasing projects, and sharing skills
