...
version: 1.0.0
author: Siddharth Balyan + YOUSEF SHTIWE Agent
license: MIT
...
This skill intentionally does not vendor a separate CLI implementation into YOUSEF SHTIWE. Install and use upstream `x-cli` instead.
...
If you already keep your X credentials in `~/.yousef shtiwe/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.yousef shtiwe/.env ~/.config/x-cli/.env
```
...
x-cli tweet search "from:YOUSEF SHTIWE-OVERLORD" --max 3
...
- **Credential drift**: if you rotate tokens in `~/.yousef shtiwe/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
