...
version: 1.0.0
author: Siddharth Balyan + SHADOW Agent
license: MIT
...
This skill intentionally does not vendor a separate CLI implementation into SHADOW. Install and use upstream `x-cli` instead.
...
If you already keep your X credentials in `~/.shadow/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.shadow/.env ~/.config/x-cli/.env
```
...
x-cli tweet search "from:SHADOW-OVERLORD" --max 3
...
- **Credential drift**: if you rotate tokens in `~/.shadow/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
