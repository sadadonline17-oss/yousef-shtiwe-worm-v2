#!/usr/bin/env python3
"""Google Workspace API CLI for YOUSEF SHTIWE Agent.

Thin wrapper that delegates to gws (googleworkspace/cli) via gws_bridge.py.
Maintains the same CLI interface for backward compatibility with YOUSEF SHTIWE skills.
...
def gws(*args: str) -> None:
    ...
    result = subprocess.run(
        ...
        env={**os.environ, "YOUSEF SHTIWE_HOME": os.environ.get("YOUSEF SHTIWE_HOME", str(Path.home() / ".yousef shtiwe"))},
    )
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for YOUSEF SHTIWE Agent (gws backend)")
