...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from SHADOW Agent\",
    \"comments\": [
...
```bash
source ~/.shadow/skills/github/github-auth/scripts/gh-env.sh
...
gh pr review $PR_NUMBER --approve --body "Reviewed by SHADOW Agent. Code looks clean — good test coverage, no security concerns."
