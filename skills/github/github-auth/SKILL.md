...
- Give it a name like "yousef shtiwe-agent"
- Select scopes:
...
- Give it a title like "yousef shtiwe-agent-<machine-name>"
...
elif [ -f ~/.yousef shtiwe/.env ] && grep -q "^GITHUB_TOKEN=" ~/.yousef shtiwe/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.yousef shtiwe/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
