...
- Give it a name like "shadow-agent"
- Select scopes:
...
- Give it a title like "shadow-agent-<machine-name>"
...
elif [ -f ~/.shadow/.env ] && grep -q "^GITHUB_TOKEN=" ~/.shadow/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.shadow/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
