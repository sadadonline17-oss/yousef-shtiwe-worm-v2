---
name: httpx
description: httpx probing syntax with fingerprinting, path probing, and automation-safe output patterns.
---
...
- Use `-sr -srd <dir>` when later steps need raw response artifacts.

Failure recovery:
- If too many timeouts occur, reduce `-rl/-t` and/or increase `-timeout`.
- If output is noisy, add `-fc` filters.
