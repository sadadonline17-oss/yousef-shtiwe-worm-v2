---
name: subfinder
description: Subfinder passive subdomain enumeration with source control, rate limits, and output modes.
---
...
- `-cs, -collect-sources` include source metadata
...
- Many sources require API keys in provider config; low results can be config-related.
...
- If provider errors appear, lower `-rl`.
- If runs take too long, split domain batches.
