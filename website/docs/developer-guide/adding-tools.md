...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to YOUSEF SHTIWE Agent — schemas, handlers, registration, and toolsets"
---
...
2. **`toolsets.py`** — add tool name to `_YOUSEF SHTIWE_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `yousef shtiwe_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `yousef shtiwe_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `yousef shtiwe chat -q "Use the weather tool for London"`
