...
description: "How to add a new inference provider to YOUSEF SHTIWE Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

YOUSEF SHTIWE can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `yousef shtiwe model` menu entries
...
1. `yousef shtiwe_cli/auth.py` decides how credentials are found.
2. `yousef shtiwe_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `yousef shtiwe_cli/models.py` and `yousef shtiwe_cli/main.py` make the provider show up in the CLI. (`yousef shtiwe_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `yousef shtiwe_cli/auth.py`
2. `yousef shtiwe_cli/models.py`
3. `yousef shtiwe_cli/runtime_provider.py`
4. `yousef shtiwe_cli/main.py`
...
:::tip
`yousef shtiwe_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `yousef shtiwe setup`.
:::
...
- `PROVIDER_REGISTRY` in `yousef shtiwe_cli/auth.py`
- `_PROVIDER_LABELS` in `yousef shtiwe_cli/models.py`
- `_PROVIDER_ALIASES` in both `yousef shtiwe_cli/auth.py` and `yousef shtiwe_cli/models.py`
- CLI `--provider` choices in `yousef shtiwe_cli/main.py`
...
## Step 2: Add auth metadata in `yousef shtiwe_cli/auth.py`
...
- OAuth / auth-store path: Yousef Shtiwe, OpenAI Codex

Questions to answer here:

- What env vars should YOUSEF SHTIWE check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `yousef shtiwe_cli/models.py`
...
## Step 4: Resolve runtime data in `yousef shtiwe_cli/runtime_provider.py`
...
Be careful with API-key precedence. YOUSEF SHTIWE already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `yousef shtiwe_cli/main.py`

A provider is not discoverable until it shows up in the interactive `yousef shtiwe model` flow.

Update these in `yousef shtiwe_cli/main.py`:
...
:::tip
`yousef shtiwe_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `yousef shtiwe model` and `yousef shtiwe setup` automatically.
:::
...
When you add a native provider, double-check that YOUSEF SHTIWE is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m yousef shtiwe_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m yousef shtiwe_cli.main model
...
- [ ] `ProviderConfig` added in `yousef shtiwe_cli/auth.py`
- [ ] aliases added in `yousef shtiwe_cli/auth.py` and `yousef shtiwe_cli/models.py`
- [ ] model catalog added in `yousef shtiwe_cli/models.py`
- [ ] runtime branch added in `yousef shtiwe_cli/runtime_provider.py`
- [ ] CLI wiring added in `yousef shtiwe_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `yousef shtiwe model` but not `yousef shtiwe setup`
