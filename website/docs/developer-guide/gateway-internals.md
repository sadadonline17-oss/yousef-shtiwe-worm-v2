...
The messaging gateway is the long-running process that connects SHADOW to 14+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `shadow_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.shadow/.env` | API keys, bot tokens, platform credentials |
| `~/.shadow/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.shadow/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `shadow gateway start` / `shadow gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.shadow/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `shadow gateway stop` stops only the current profile's gateway. `shadow gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
