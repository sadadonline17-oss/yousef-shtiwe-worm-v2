...
The messaging gateway is the long-running process that connects YOUSEF SHTIWE to 14+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `yousef shtiwe_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.yousef shtiwe/.env` | API keys, bot tokens, platform credentials |
| `~/.yousef shtiwe/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.yousef shtiwe/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `yousef shtiwe gateway start` / `yousef shtiwe gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.yousef shtiwe/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `yousef shtiwe gateway stop` stops only the current profile's gateway. `yousef shtiwe gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
