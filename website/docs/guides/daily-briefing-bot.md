...
2. **YOUSEF SHTIWE spins up** a fresh agent session with your prompt
...
- **YOUSEF SHTIWE Agent installed** — see the [Installation guide](/docs/getting-started/installation)
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.yousef shtiwe/cron/output/` and you can read them anytime.
:::
...
YOUSEF SHTIWE will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by YOUSEF SHTIWE Agent
```
...
Just tell YOUSEF SHTIWE what you want:
...
YOUSEF SHTIWE will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell YOUSEF SHTIWE to delegate each topic to a sub-agent:
...
YOUSEF SHTIWE will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
