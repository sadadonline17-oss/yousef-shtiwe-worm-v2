...
2. **SHADOW spins up** a fresh agent session with your prompt
...
- **SHADOW Agent installed** — see the [Installation guide](/docs/getting-started/installation)
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.shadow/cron/output/` and you can read them anytime.
:::
...
SHADOW will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by SHADOW Agent
```
...
Just tell SHADOW what you want:
...
SHADOW will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell SHADOW to delegate each topic to a sub-agent:
...
SHADOW will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
