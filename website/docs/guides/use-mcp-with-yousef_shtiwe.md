---
sidebar_position: 6
title: "Use MCP with YOUSEF SHTIWE"
description: "A practical guide to connecting MCP servers to YOUSEF SHTIWE Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with YOUSEF SHTIWE

This guide shows how to actually use MCP with YOUSEF SHTIWE Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native YOUSEF SHTIWE tool
- you want YOUSEF SHTIWE to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect YOUSEF SHTIWE to internal APIs, databases, or company systems without modifying YOUSEF SHTIWE core

Do not use MCP when:
- a built-in YOUSEF SHTIWE tool already solves the job well
...
- YOUSEF SHTIWE remains the agent
- MCP servers contribute tools
- YOUSEF SHTIWE discovers those tools at startup or reload time
...
If you installed YOUSEF SHTIWE with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- YOUSEF SHTIWE banner/status should show MCP integration when configured
- ask YOUSEF SHTIWE what tools it has available
...
There are two categories of MCP-exposed functionality in YOUSEF SHTIWE:
...
2. YOUSEF SHTIWE-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So YOUSEF SHTIWE will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want YOUSEF SHTIWE to reason over a bounded workspace.
...
Now YOUSEF SHTIWE can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing YOUSEF SHTIWE core.
...
Because YOUSEF SHTIWE now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
