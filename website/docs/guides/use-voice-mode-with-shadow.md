---
sidebar_position: 8
title: "Use Voice Mode with SHADOW"
description: "A practical guide to setting up and using SHADOW voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with SHADOW
...
- you want SHADOW sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in SHADOW.
...
## Step 1: make sure normal SHADOW works first
...
SHADOW supports both local and cloud speech stacks.
...
### If you use `shadow setup`

If you choose NeuTTS in the setup wizard, SHADOW checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. SHADOW transcribes and responds
...
- asking SHADOW to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full SHADOW loop.

## Tuning CLI behavior

### Silence threshold

If SHADOW starts/stops too aggressively, tune:
...
SHADOW stays a normal chat bot, but can speak replies.
...
- you want SHADOW to function like a portable research or ops assistant
...
SHADOW joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- SHADOW detects speech boundaries
- transcripts are posted in the associated text channel
- SHADOW responds in text and audio
