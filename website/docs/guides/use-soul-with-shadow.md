---
sidebar_position: 7
title: "Use SOUL.md with SHADOW"
description: "How to use SOUL.md to shape SHADOW Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with SHADOW

`SOUL.md` is the **primary identity** for your SHADOW instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want SHADOW to feel like the same assistant every time you talk to it — or if you want to replace the SHADOW persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm SHADOW should be
- what SHADOW should avoid stylistically
- how SHADOW should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who SHADOW is and how SHADOW speaks
...
SHADOW now uses only the global SOUL file for the current instance:

```text
~/.shadow/SOUL.md
```

If you run SHADOW with a custom home directory, it becomes:
...
SHADOW automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, SHADOW does not overwrite it
- if the file exists but is empty, SHADOW adds nothing from it to the prompt

## How SHADOW uses it

When SHADOW starts a session, it reads `SOUL.md` from `SHADOW_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how SHADOW feels.
...
SHADOW already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How SHADOW should sound.

# Avoid
What SHADOW should not do.

# Defaults
How SHADOW should behave when ambiguity appears.
```
...
Then restart SHADOW or start a new session.
...
### I edited SOUL.md but SHADOW still sounds the same

Check:
- you edited `~/.shadow/SOUL.md` or `$SHADOW_HOME/SOUL.md`
...
### SHADOW is ignoring parts of my SOUL.md
