...
sidebar_position: 7
title: "Docker"
description: "Running SHADOW Agent in Docker and using Docker as a terminal backend"
---

# SHADOW Agent — Docker

There are two distinct ways Docker intersects with SHADOW Agent:

1. **Running SHADOW IN Docker** — the agent itself runs inside a container (this page's primary focus)
...
If this is your first time running SHADOW Agent, create a data directory on the host and start the container interactively to run the setup wizard:
...
The `/opt/data` volume is the single source of truth for all SHADOW state. It maps to your host's `~/.shadow/` directory and contains:

| Path | Contents |
|------|----------|
| `.env` | API keys and secrets |
| `config.yaml` | All SHADOW configuration |
...
:::warning
Never run two SHADOW containers against the same data directory simultaneously — session files and memory stores are not designed for concurrent access.
:::
...
Start with `docker compose up -d` and view logs with `docker compose logs -f shadow`.

## Resource limits

The SHADOW container needs moderate resources. Recommended minimums:
...
- Python 3 with all SHADOW dependencies (`pip install -e ".[all]"`)
...
- Then runs `shadow` with whatever arguments you pass
...
Check logs: `docker logs shadow`. Common causes:
- Missing or invalid `.env` file — run interactively first to complete setup
- Port conflicts if running with exposed ports

### "Permission denied" errors

The container runs as root by default. If your host `~/.shadow/` was created by a non-root user, permissions should work. If you get errors, ensure the data directory is writable:
