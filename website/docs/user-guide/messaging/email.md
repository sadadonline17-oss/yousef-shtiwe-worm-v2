...
sidebar_position: 7
title: "Email"
description: "Set up YOUSEF SHTIWE Agent as an email assistant via IMAP/SMTP"
---

# Email Setup

YOUSEF SHTIWE can receive and reply to emails using standard IMAP and SMTP protocols. Send an email to the agent's address and it replies in-thread — no special client or bot API needed. Works with Gmail, Outlook, Yahoo, Fastmail, or any provider that supports IMAP/SMTP.
...
- **A dedicated email account** for your YOUSEF SHTIWE agent (don't use your personal email)
...
## Step 1: Configure YOUSEF SHTIWE
...
```bash
# Required
EMAIL_ADDRESS=yousef shtiwe@gmail.com
...
Email access follows the same pattern as all other YOUSEF SHTIWE platforms:
...
| **Duplicate replies** | Ensure only one gateway instance is running. Check `yousef shtiwe gateway status`. |
...
- The password is stored in `~/.yousef shtiwe/.env` — protect this file (`chmod 600`)
