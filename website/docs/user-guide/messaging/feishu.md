...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up YOUSEF SHTIWE Agent as a Feishu or Lark bot"
---

# Feishu / Lark Setup

YOUSEF SHTIWE Agent integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; YOUSEF SHTIWE opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | YOUSEF SHTIWE responds to every message. |
| Group chats | YOUSEF SHTIWE responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. YOUSEF SHTIWE will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `yousef shtiwe gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when YOUSEF SHTIWE runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run YOUSEF SHTIWE behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, YOUSEF SHTIWE starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
Add the following to `~/.yousef shtiwe/.env`:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how YOUSEF SHTIWE responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | YOUSEF SHTIWE responds to @mentions from any user in any group. |
| `allowlist` | YOUSEF SHTIWE only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | YOUSEF SHTIWE ignores all group messages entirely. |
...
|---------|---------|---------|
| Quiet period | `YOUSEF SHTIWE_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `YOUSEF SHTIWE_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `YOUSEF SHTIWE_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `YOUSEF SHTIWE_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.yousef shtiwe/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `YOUSEF SHTIWE_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |
...
| `YOUSEF SHTIWE_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `YOUSEF SHTIWE_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `YOUSEF SHTIWE_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `YOUSEF SHTIWE_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `YOUSEF SHTIWE_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `yousef shtiwe gateway setup` |
| `Another local YOUSEF SHTIWE gateway is already using this Feishu app_id` | Only one YOUSEF SHTIWE instance can use the same app_id at a time. Stop the other gateway first. |
...
Feishu / Lark uses the `yousef shtiwe-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
