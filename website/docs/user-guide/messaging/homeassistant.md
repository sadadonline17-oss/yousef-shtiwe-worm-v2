...
description: Control your smart home with YOUSEF SHTIWE Agent via Home Assistant integration.
...
YOUSEF SHTIWE Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "YOUSEF SHTIWE Agent"
5. Copy the token
...
YOUSEF SHTIWE Agent registers four tools for smart home control:
...
Configure which events the agent sees in `~/.yousef shtiwe/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "YOUSEF SHTIWE Agent".
