...
Connect SHADOW to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between SHADOW and the BlueBubbles server
...
Or set environment variables directly in `~/.shadow/.env`:
...
When someone messages your iMessage, SHADOW automatically sends them a pairing code. Approve it with:
```bash
shadow pairing approve bluebubbles <CODE>
```
Use `shadow pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.shadow/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.shadow/.env`):
```bash
...
SHADOW will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → SHADOW
SHADOW → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** SHADOW sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — SHADOW resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `shadow logs gateway` for webhook errors (or `shadow logs -f` to follow in real-time)
