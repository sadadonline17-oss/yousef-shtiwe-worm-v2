...
Connect YOUSEF SHTIWE to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between YOUSEF SHTIWE and the BlueBubbles server
...
Or set environment variables directly in `~/.yousef shtiwe/.env`:
...
When someone messages your iMessage, YOUSEF SHTIWE automatically sends them a pairing code. Approve it with:
```bash
yousef shtiwe pairing approve bluebubbles <CODE>
```
Use `yousef shtiwe pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.yousef shtiwe/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.yousef shtiwe/.env`):
```bash
...
YOUSEF SHTIWE will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → YOUSEF SHTIWE
YOUSEF SHTIWE → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** YOUSEF SHTIWE sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — YOUSEF SHTIWE resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `yousef shtiwe logs gateway` for webhook errors (or `yousef shtiwe logs -f` to follow in real-time)
