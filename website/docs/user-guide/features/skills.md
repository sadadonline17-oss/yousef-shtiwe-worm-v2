...
All skills live in **`~/.yousef shtiwe/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point YOUSEF SHTIWE at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example of a skill-backed slash command with custom behavior. Running `/plan [request]` tells YOUSEF SHTIWE to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.yousef shtiwe/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
yousef shtiwe chat --toolsets skills -q "What skills do you have?"
yousef shtiwe chat --toolsets skills -q "Show me the axolotl skill"
```
...
When a missing value is encountered, YOUSEF SHTIWE asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `yousef shtiwe setup` or `~/.yousef shtiwe/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `yousef shtiwe config migrate` prompts for unconfigured settings, and `yousef shtiwe config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.yousef shtiwe/skills/                  # Single source of truth
...
If you maintain skills outside of YOUSEF SHTIWE — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell YOUSEF SHTIWE to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.yousef shtiwe/config.yaml`:
...
- **Read-only**: External dirs are only scanned for skill discovery. When the agent creates or edits a skill, it always writes to `~/.yousef shtiwe/skills/`.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, YOUSEF SHTIWE ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.yousef shtiwe/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
All four skills appear in your skill index. If you create a new skill called `my-custom-workflow` locally, it yousef shtiwes the external version.
...
```bash
yousef shtiwe skills browse                              # Browse all hub skills (official first)
yousef shtiwe skills browse --source official            # Browse only official optional skills
yousef shtiwe skills search kubernetes                   # Search all sources
yousef shtiwe skills search react --source skills-sh     # Search the skills.sh directory
yousef shtiwe skills search https://mintlify.com/docs --source well-known
yousef shtiwe skills inspect openai/skills/k8s           # Preview before installing
yousef shtiwe skills install openai/skills/k8s           # Install with security scan
...
yousef shtiwe skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
yousef shtiwe skills list --source hub                   # List hub-installed skills
yousef shtiwe skills check                               # Check installed hub skills for upstream updates
yousef shtiwe skills update                              # Reinstall hub skills with upstream changes when needed
yousef shtiwe skills audit                               # Re-scan all hub skills for security
yousef shtiwe skills uninstall k8s                       # Remove a hub skill
yousef shtiwe skills publish skills/my-skill --to github --repo owner/repo
yousef shtiwe skills snapshot export setup.json          # Export skill config
yousef shtiwe skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with YOUSEF SHTIWE. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `yousef shtiwe skills search <query> --source skills-sh`. YOUSEF SHTIWE resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
YOUSEF SHTIWE currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the YOUSEF SHTIWE repository itself and install with builtin trust.
...
This is Vercel's public skills directory. YOUSEF SHTIWE can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
yousef shtiwe skills search https://mintlify.com/docs --source well-known
yousef shtiwe skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
yousef shtiwe skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

YOUSEF SHTIWE can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
yousef shtiwe skills tap add myorg/skills-repo
```
...
- YOUSEF SHTIWE source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

YOUSEF SHTIWE supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

YOUSEF SHTIWE source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

YOUSEF SHTIWE can search and convert agent entries from LobeHub's public catalog into installable YOUSEF SHTIWE skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- YOUSEF SHTIWE source id: `lobehub`
...
`yousef shtiwe skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
```bash
yousef shtiwe skills check          # Report which installed hub skills changed upstream
yousef shtiwe skills update         # Reinstall only the skills with updates available
yousef shtiwe skills update react   # Update one specific installed hub skill
```
