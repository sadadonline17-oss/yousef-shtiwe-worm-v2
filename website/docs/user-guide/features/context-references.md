...
Type `@` followed by a reference to inject content directly into your message. YOUSEF SHTIWE expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- YOUSEF SHTIWE env: `$YOUSEF SHTIWE_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$YOUSEF SHTIWE_HOME/skills/.hub/`
