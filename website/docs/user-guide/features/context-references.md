...
Type `@` followed by a reference to inject content directly into your message. SHADOW expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- SHADOW env: `$SHADOW_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$SHADOW_HOME/skills/.hub/`
