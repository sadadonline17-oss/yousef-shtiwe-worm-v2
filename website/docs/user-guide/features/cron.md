...
Schedule tasks to run automatically with natural language or cron expressions. YOUSEF SHTIWE exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. YOUSEF SHTIWE disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
yousef shtiwe cron create "every 2h" "Check server status"
yousef shtiwe cron create "every 1h" "Summarize new feed items" --skill blogwatcher
yousef shtiwe cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill find-nearby \
  --name "Skill combo"
```
...
Ask YOUSEF SHTIWE normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

YOUSEF SHTIWE will use the unified `cronjob` tool internally.
...
```bash
yousef shtiwe cron edit <job_id> --schedule "every 4h"
yousef shtiwe cron edit <job_id> --prompt "Use the revised task"
yousef shtiwe cron edit <job_id> --skill blogwatcher --skill find-nearby
yousef shtiwe cron edit <job_id> --add-skill find-nearby
yousef shtiwe cron edit <job_id> --remove-skill blogwatcher
yousef shtiwe cron edit <job_id> --clear-skills
```
...
yousef shtiwe cron pause <job_id>
yousef shtiwe cron resume <job_id>
yousef shtiwe cron run <job_id>
yousef shtiwe cron remove <job_id>
yousef shtiwe cron status
yousef shtiwe cron tick
```
...
sudo yousef shtiwe gateway install --system   # Linux: boot-time system service for servers
yousef shtiwe gateway             # Or run in foreground
...
On each tick YOUSEF SHTIWE:

1. loads jobs from `~/.yousef shtiwe/cron/jobs.json`
...
A file lock at `~/.yousef shtiwe/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.yousef shtiwe/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.yousef shtiwe/cron/output/`), but no message is sent to the delivery target.
...
Or set the `YOUSEF SHTIWE_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, YOUSEF SHTIWE skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Jobs are stored in `~/.yousef shtiwe/cron/jobs.json`. Output from job runs is saved to `~/.yousef shtiwe/cron/output/{job_id}/{timestamp}.md`.
