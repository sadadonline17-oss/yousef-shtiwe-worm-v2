...
YOUSEF SHTIWE has two hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.yousef shtiwe/hooks/` | Gateway only | Logging, alerts, webhooks |
...
Each hook is a directory under `~/.yousef shtiwe/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".yousef shtiwe" / "hooks" / "my-hook" / "activity.log"
...
The gateway ships with a built-in `boot-md` hook that looks for `~/.yousef shtiwe/BOOT.md` on every startup. If the file exists, the agent runs its instructions in a background session. No installation needed — just create the file.

**Create `~/.yousef shtiwe/BOOT.md`:**

```markdown
# Startup Checklist

1. Check if any cron jobs failed overnight — run `yousef shtiwe cron list`
...
```yaml
# ~/.yousef shtiwe/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.yousef shtiwe/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.yousef shtiwe/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.yousef shtiwe/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".yousef shtiwe" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.yousef shtiwe/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.yousef shtiwe/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/yousef shtiwe-events"
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.yousef shtiwe/hooks/`
...
```python
# Inject context
return {"context": "Recalled memories:\n- User likes Python\n- Working on yousef shtiwe-agent"}
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is YOUSEF SHTIWE's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.
...
See the **[Build a Plugin guide](/docs/guides/build-a-yousef shtiwe-plugin)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.
