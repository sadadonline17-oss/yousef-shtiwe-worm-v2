...
If the file is empty, whitespace-only, or cannot be read, SHADOW falls back to a built-in default identity ("You are SHADOW Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
