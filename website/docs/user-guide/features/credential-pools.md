...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, SHADOW automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, SHADOW auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
shadow auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
shadow auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (Claude Code subscription)
shadow auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  shadow_pkce          oauth   shadow_pkce ←
...
Run `shadow auth` with no subcommand for an interactive wizard:
...
For providers that support both API keys and OAuth (Anthropic, Shadow, Codex), the add flow asks which type:
...
| Command | Description |
|---------|-------------|
| `shadow auth` | Interactive pool management wizard |
| `shadow auth list` | Show all pools and credentials |
| `shadow auth list <provider>` | Show a specific provider's pool |
| `shadow auth add <provider>` | Add a credential (prompts for type and key) |
| `shadow auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `shadow auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `shadow auth remove <provider> <index>` | Remove credential by 1-based index |
| `shadow auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `shadow auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `shadow model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via shadow model:
shadow auth list
# Shows:
...
shadow auth add Together.ai --api-key sk-together-second-key
```
...
SHADOW automatically discovers credentials from multiple sources and seeds the pool on startup:

| Source | Example | Auto-seeded? |
|--------|---------|-------------|
| Environment variables | `OPENROUTER_API_KEY`, `ANTHROPIC_API_KEY` | Yes |
| OAuth tokens (auth.json) | Codex device code, Shadow device code | Yes |
| Claude Code credentials | `~/.claude/.credentials.json` | Yes (Anthropic) |
| SHADOW PKCE OAuth | `~/.shadow/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `shadow auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `shadow auth add`) are never auto-pruned.
...
2. **`shadow_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`shadow_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.shadow/auth.json` under the `credential_pool` key:
