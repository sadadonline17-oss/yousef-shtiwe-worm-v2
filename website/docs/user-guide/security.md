...
YOUSEF SHTIWE Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, YOUSEF SHTIWE checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.yousef shtiwe/config.yaml`:
...
1. **CLI flag**: Start a session with `yousef shtiwe --yolo` or `yousef shtiwe chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `YOUSEF SHTIWE_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `YOUSEF SHTIWE_YOLO_MODE` environment variable which is checked before every command execution.
...
Configure the timeout in `~/.yousef shtiwe/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.yousef shtiwe/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.yousef shtiwe/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` yousef shtiwe/gateway | Self-termination prevention |
...
The `YOUSEF SHTIWE_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.yousef shtiwe/config.yaml`:
...
:::tip
Use `yousef shtiwe config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, YOUSEF SHTIWE controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.yousef shtiwe/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.yousef shtiwe/.env to allow open access,
...
For more flexible authorization, YOUSEF SHTIWE includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `yousef shtiwe pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.yousef shtiwe/config.yaml`:
...
yousef shtiwe pairing approve telegram ABC12DEF

# Revoke a user's access
yousef shtiwe pairing revoke telegram 123456789

# Clear all pending codes
yousef shtiwe pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.yousef shtiwe/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, YOUSEF SHTIWE applies strict security hardening to every container.
...
Container resources are configurable in `~/.yousef shtiwe/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.yousef shtiwe/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `YOUSEF SHTIWE_HOME`. Skills declare these in frontmatter:
...
When loaded, YOUSEF SHTIWE checks if these files exist in the active profile's `YOUSEF SHTIWE_HOME` and registers them for mounting:
...
Paths are relative to `~/.yousef shtiwe/`. Files are mounted to `/root/.yousef shtiwe/` inside the container.
...
| **terminal** (local) | Blocks explicit YOUSEF SHTIWE infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- YOUSEF SHTIWE infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
YOUSEF SHTIWE integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
4. **Store secrets securely** — keep API keys in `~/.yousef shtiwe/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.yousef shtiwe/logs/` for unauthorized access attempts
10. **Keep updated** — run `yousef shtiwe update` regularly for security patches

### Securing API Keys

```bash
# Set proper permissions on the .env file
chmod 600 ~/.yousef shtiwe/.env
...
terminal:
  ...
  ssh_user: "yousef shtiwe"
  ssh_key: "~/.ssh/yousef shtiwe_agent_key"
```
