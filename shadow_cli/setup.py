"""
Interactive setup wizard for SHADOW Agent.
...
_DOCS_BASE = "https://shadow-agent.shadow-overlord.com/docs"
...
# display_shadow_home imported lazily at call sites (stale-module safety during shadow update)
...
from shadow_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ SHADOW Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure SHADOW using environment variables or config commands:")
    print_info("  shadow config set model.provider custom")
    print_info("  shadow config set model.base_url http://localhost:8080/v1")
    print_info("  shadow config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'shadow setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'shadow setup')")


def _print_setup_summary(config: dict, shadow_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'shadow setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'shadow setup tts'"))
    else:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'shadow setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {shadow_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('shadow setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('shadow setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('shadow setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('shadow setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('shadow setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('shadow config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('shadow config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('shadow config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('shadow', Colors.GREEN)}              Start chatting")
    print(f"   {color('shadow gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('shadow doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `shadow tools` and `shadow setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``shadow model``)
    ...
    provider added to ``shadow model`` is automatically available here.
    ...
    # Delegate to the shared shadow model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: shadow model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from shadow_cli.auth_commands import auth_add_command
            ...
            print_info(
                "SHADOW can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one SHADOW can auto-use for vision,")
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'shadow setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where SHADOW runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using SHADOW via Telegram/Discord, this is where")
        print_info(
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("SHADOW_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `shadow setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("SHADOW_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("SHADOW_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where SHADOW delivers cron job results,")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://shadow-agent.shadow-overlord.com/docs/user-guide/messaging/slack/")
    print()
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where SHADOW delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where SHADOW delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'shadow whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'shadow whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from shadow_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from shadow_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from shadow_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from shadow_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from shadow_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from shadow_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from shadow_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from shadow_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_qqbot():
    ...
    print_info("Connects SHADOW to QQ via the Official QQ Bot API (v2).")
...
def _setup_bluebubbles():
    ...
    print_info("Connects SHADOW to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from shadow_cli.gateway import _PLATFORMS
    qq_platform = next((p for p in _PLATFORMS if p["key"] == "qqbot"), None)
    if qq_platform:
        from shadow_cli.gateway import _setup_standard_platform
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://shadow-agent.shadow-overlord.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://shadow-agent.shadow-overlord.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  shadow config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with SHADOW from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'shadow setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     shadow config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  shadow gateway")
        else:
            from shadow_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   shadow gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   shadow gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `shadow setup tools` and `shadow tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from shadow_cli.config
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_shadow migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_shadow", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and SHADOW.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure SHADOW to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point SHADOW at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point SHADOW at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point SHADOW at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point SHADOW at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to SHADOW equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing SHADOW config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in SHADOW.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ SHADOW's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(shadow_home: Path) -> bool:
    ...
    print_info("SHADOW can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: shadow claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: shadow claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing SHADOW configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing SHADOW config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in SHADOW (use shadow claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      shadow setup           — full or quick (auto-detected)
      shadow setup model     — just model/provider
      shadow setup tts       — just text-to-speech
      shadow setup terminal  — just terminal backend
      shadow setup gateway   — just messaging platforms
      shadow setup tools     — just tool configuration
      shadow setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ SHADOW Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ SHADOW Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your SHADOW Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have SHADOW configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, shadow_home)
            return
        elif choice == 1:
            # Full setup — fall through to run all sections
            pass
        elif choice == 7:
            print_info("Exiting. Run 'shadow setup' again when ready.")
            return
        elif 2 <= choice <= 6:
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, shadow_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        migration_ran = _offer_openclaw_migration(shadow_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up SHADOW?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, shadow_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {shadow_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'shadow config edit'")
...
def _resolve_shadow_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``shadow chat`` in a fresh process."""
    shadow_bin = shutil.which("shadow")
    if shadow_bin:
        return [shadow_bin, "chat"]

    try:
        if importlib.util.find_spec("shadow_cli") is not None:
            return [sys.executable, "-m", "shadow_cli.main", "chat"]
    except Exception:
        pass
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch shadow chat now?", True):
        return

    chat_argv = _resolve_shadow_chat_argv()
    if not chat_argv:
        print_info("Could not relaunch SHADOW automatically. Run 'shadow chat' manually.")
        return

    os.execvp(chat_argv[0], chat_argv)


def _run_first_time_quick_setup(config: dict, shadow_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'shadow setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    shadow setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  shadow setup gateway")
    print()
...
def _run_quick_setup(config: dict, shadow_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'shadow setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect SHADOW to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'shadow setup gateway'.")
