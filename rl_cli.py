...
if tinker_atropos_dir.exists():
    ...
    os.environ['SHADOW_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to shadow-agent directory if submodule not found
...
from shadow_constants import get_shadow_home, OPENROUTER_BASE_URL
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.shadow/config.yaml if not provided)
    ...
    """
    # Load config from ~/.shadow/config.yaml
