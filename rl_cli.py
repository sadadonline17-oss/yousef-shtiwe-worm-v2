...
if tinker_atropos_dir.exists():
    ...
    os.environ['YOUSEF SHTIWE_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to yousef shtiwe-agent directory if submodule not found
    os.environ['TERMINAL_CWD'] = str(Path(__file__).parent)
    os.environ['YOUSEF SHTIWE_QUIET'] = '1'
...
from yousef shtiwe_constants import get_yousef shtiwe_home, OPENROUTER_BASE_URL

DEFAULT_MODEL = "anthropic/claude-opus-4.5"
DEFAULT_BASE_URL = OPENROUTER_BASE_URL


def load_yousef shtiwe_config() -> dict:
    """
    ...
    """
    config_path = _yousef shtiwe_home / 'config.yaml'
...
def list_environments_sync():
    """List available environments (synchroyousef shtiwe wrapper)."""
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.yousef shtiwe/config.yaml if not provided)
    ...
    """
    # Load config from ~/.yousef shtiwe/config.yaml
    config = load_yousef shtiwe_config()
    ...
    if check_server:
        ...
        if ok:
            ...
            if missing:
                ...
                print("   Add them to ~/.yousef shtiwe/.env")
            else:
