...
- **yousef_shtiwe.sh update logic** -- agent container now always rebuilds (not just restarts) when any `agentic/` file changes, since source code is baked into the image without volume mount
...
- **Smart ingestion on install** -- `./yousef_shtiwe.sh install` detects GPU and API key availability. On CPU without an API key, shows an interactive prompt with estimated times per source and lets the user choose quick start (~15 min) or full ingestion (~4 hours). With GPU or API key, ingests all sources automatically
...
- **Makefile for KB management** -- `knowledge_base/Makefile` with targets for build, update, rebuild, stats, and cleanup. All `yousef_shtiwe.sh` KB commands use `MODE=docker` to run inside the agent container
...
- **Wiki documentation** -- new [Recon & Project Presets](https://github.com/samugit83/yousef_shtiwe/wiki/Recon-Presets) wiki page with full guide, 21-preset reference table, and AI generation walkthrough. Updated Creating a Project (16 tabs), Project Settings Reference, Running Reconnaissance, Home, and Sidebar
...
- **Bottom bar visibility** -- PageBottomBar (node type filters, session controls, stats) now hidden for Reverse Shell, yousef_shtiwe Terminal, RoE, and JS Recon views. Only visible for Graph Map, Graph Views, and All Nodes
...
- **Amass Brute Force Wordlist Selector** -- configurable wordlist selection for Amass DNS brute forcing:
  ...
  - **jhaddix all.txt**: Jason Haddix's comprehensive subdomain wordlist (~2.18M entries compiled from certificate transparency, bug bounty findings, DNS datasets) baked into the `yousef_shtiwe-recon` Docker image
...
- **"Remote Shells" renamed to "Reverse Shell"** — tab renamed for clarity to distinguish from the new yousef_shtiwe Terminal tab. The Reverse Shell tab manages agent-opened sessions (meterpreter, netcat, etc.), while yousef_shtiwe Terminal provides direct interactive sandbox access.
...
- **Animated Loading Indicator** — replaced static "Processing..." text in the AI assistant chat with a dynamic loading experience:
  - **yousef_shtiwe eye logo** with randomized heartbeat animation (2–6s random intervals)
...
- **Pentest Report Generation** — generate professional, client-ready penetration testing reports as self-contained HTML files from the `/reports` page. Reports compile all reconnaissance data, vulnerability findings, CVE intelligence, attack chain results, and remediation recommendations into an 11-section document (Cover, Executive Summary, Scope & Methodology, Risk Summary, Findings, Other Vulnerability Details, Attack Surface, CVE Intelligence, GitHub Secrets, Attack Chains, Recommendations, Appendix). Features include:
  ...
  - **Wiki documentation** — new [Pentest Reports](yousef_shtiwe.wiki/20.-Pentest-Reports) wiki page with example report download
...
- **GVM Vulnerability Scanning** — full end-to-end integration of Greenbone Vulnerability Management (GVM/OpenVAS) into the yousef_shtiwe pipeline:
...
- **Frontend Usability** — unified yousef_shtiwe primary crimson color (#d32f2f), styled message containers with ghost icons and gradient backgrounds, improved markdown heading and list spacing
