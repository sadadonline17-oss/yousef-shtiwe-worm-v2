#!/usr/bin/env python3
"""OpenClaw -> YOUSEF SHTIWE migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into YOUSEF SHTIWE Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into YOUSEF SHTIWE.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into YOUSEF SHTIWE memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into YOUSEF SHTIWE memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import YOUSEF SHTIWE-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of YOUSEF SHTIWE-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into YOUSEF SHTIWE command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.yousef shtiwe/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.yousef shtiwe/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into YOUSEF SHTIWE .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into YOUSEF SHTIWE .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into YOUSEF SHTIWE .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into YOUSEF SHTIWE .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into YOUSEF SHTIWE .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into YOUSEF SHTIWE config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into YOUSEF SHTIWE config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into YOUSEF SHTIWE.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into YOUSEF SHTIWE MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into YOUSEF SHTIWE config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'yousef shtiwe cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into YOUSEF SHTIWE config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into YOUSEF SHTIWE session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into YOUSEF SHTIWE custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into YOUSEF SHTIWE config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into YOUSEF SHTIWE config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into YOUSEF SHTIWE config.yaml approvals section.",
    },
    "memory-backend": {
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update YOUSEF SHTIWE config.yaml")
    ensure_parent(path)
    path.write_text(
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with YOUSEF SHTIWE in migrated text so that
...
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'YOUSEF SHTIWE'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'YOUSEF SHTIWE'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'YOUSEF SHTIWE'),
]


def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with YOUSEF SHTIWE."""
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> YOUSEF SHTIWE Migration Report",
        "",
...
class Migrator:
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "YOUSEF SHTIWE config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the YOUSEF SHTIWE
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No YOUSEF SHTIWE-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted YOUSEF SHTIWE-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in ("file", "exec"):
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: yousef shtiwe config set",
                        )
                    continue
        ...
        for oc_key, yousef shtiwe_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and yousef shtiwe_key not in secret_additions:
                secret_additions[yousef shtiwe_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, yousef shtiwe_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and yousef shtiwe_key not in secret_additions:
                        secret_additions[yousef shtiwe_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        yousef shtiwe_config = load_yaml_file(destination)
        current_model = yousef shtiwe_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = yousef shtiwe_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                yousef shtiwe_config["model"] = {"default": model_str}
            dump_yaml_file(destination, yousef shtiwe_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in ("elevenlabs", "openai", "edge", "microsoft"):
            # OpenClaw renamed "edge" to "microsoft"; YOUSEF SHTIWE still uses "edge"
        ...
        yousef shtiwe_config = load_yaml_file(destination)
        existing_tts = yousef shtiwe_config.get("tts", {})
        ...
        if self.execute:
            ...
            yousef shtiwe_config["tts"] = merged_tts
            dump_yaml_file(destination, yousef shtiwe_config)
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct YOUSEF SHTIWE destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected YOUSEF SHTIWE-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected YOUSEF SHTIWE-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        yousef shtiwe_cfg_path = self.target_root / "config.yaml"
        yousef shtiwe_cfg = load_yaml_file(yousef shtiwe_cfg_path)
        existing_mcp = yousef shtiwe_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in YOUSEF SHTIWE config")
                continue

            yousef shtiwe_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                yousef shtiwe_srv["command"] = srv["command"]
                if srv.get("args"):
                    yousef shtiwe_srv["args"] = srv["args"]
                if srv.get("env"):
                    yousef shtiwe_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    yousef shtiwe_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                yousef shtiwe_srv["url"] = srv["url"]
                if srv.get("headers"):
                    yousef shtiwe_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    yousef shtiwe_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                yousef shtiwe_srv["enabled"] = False
            if srv.get("timeout"):
                yousef shtiwe_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                yousef shtiwe_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                yousef shtiwe_srv["tools"] = {}
                if tools_cfg.get("include"):
                    yousef shtiwe_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    yousef shtiwe_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
            sampling = srv.get("sampling")
            if sampling and isinstance(sampling, dict):
                yousef shtiwe_srv["sampling"] = {
                    k: v for k, v in {
            ...
            existing_mcp[name] = yousef shtiwe_srv
            added += 1
        ...
        if added > 0 and self.execute:
            self.maybe_backup(yousef shtiwe_cfg_path)
            yousef shtiwe_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(yousef shtiwe_cfg_path, yousef shtiwe_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'yousef shtiwe cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = yousef shtiwe_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> YOUSEF SHTIWE reasoning_effort
        ...
        if compaction:
            compression = yousef shtiwe_cfg.get("compression") or {}
            ...
            if compaction.get("model"):
                aux = yousef shtiwe_cfg.setdefault("auxiliary", {})
                aux_comp = aux.setdefault("compression", {})
                aux_comp["model"] = compaction["model"]
            yousef shtiwe_cfg["compression"] = compression
            changes = True

        # Map humanDelay
        human_delay = defaults.get("humanDelay") or {}
        if human_delay:
            hd = yousef shtiwe_cfg.get("human_delay") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'yousef shtiwe gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("YOUSEF SHTIWE_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        yousef shtiwe_cfg_path = self.target_root / "config.yaml"
        yousef shtiwe_cfg = load_yaml_file(yousef shtiwe_cfg_path)
        sr = yousef shtiwe_cfg.get("session_reset") or {}
        changes = False
        ...
        if changes:
            yousef shtiwe_cfg["session_reset"] = sr
            if self.execute:
                self.maybe_backup(yousef shtiwe_cfg_path)
                dump_yaml_file(yousef shtiwe_cfg_path, yousef shtiwe_cfg)
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        yousef shtiwe_cfg_path = self.target_root / "config.yaml"
        yousef shtiwe_cfg = load_yaml_file(yousef shtiwe_cfg_path)
        custom_providers = yousef shtiwe_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(yousef shtiwe_cfg_path)
            yousef shtiwe_cfg["custom_providers"] = custom_providers
            dump_yaml_file(yousef shtiwe_cfg_path, yousef shtiwe_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to YOUSEF SHTIWE config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            yousef shtiwe_cfg_path = self.target_root / "config.yaml"
            yousef shtiwe_cfg = load_yaml_file(yousef shtiwe_cfg_path)
            discord_yousef shtiwe = yousef shtiwe_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_yousef shtiwe["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_yousef shtiwe["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                yousef shtiwe_cfg["discord"] = discord_yousef shtiwe
                dump_yaml_file(yousef shtiwe_cfg_path, yousef shtiwe_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        yousef shtiwe_cfg_path = self.target_root / "config.yaml"
        yousef shtiwe_cfg = load_yaml_file(yousef shtiwe_cfg_path)
        browser_yousef shtiwe = yousef shtiwe_cfg.get("browser") or {}
        changed = False

        # Map fields that have YOUSEF SHTIWE equivalents
        if browser.get("cdpUrl"):
            browser_yousef shtiwe["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_yousef shtiwe["headless"] = browser["headless"]
            changed = True

        if changed:
            yousef shtiwe_cfg["browser"] = browser_yousef shtiwe
            if self.execute:
                self.maybe_backup(yousef shtiwe_cfg_path)
                dump_yaml_file(yousef shtiwe_cfg_path, yousef shtiwe_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            yousef shtiwe_mode = mode_map.get(mode, "manual")
            yousef shtiwe_cfg.setdefault("approvals", {})["mode"] = yousef shtiwe_mode
            if self.execute:
                self.maybe_backup(yousef shtiwe_cfg_path)
                dump_yaml_file(yousef shtiwe_cfg_path, yousef shtiwe_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{yousef shtiwe_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> YOUSEF SHTIWE Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in YOUSEF SHTIWE. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing YOUSEF SHTIWE Config Not Overwritten)",
                "",
                "These items already existed in your YOUSEF SHTIWE config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the YOUSEF SHTIWE agent discovers these",
            "directories, it may read/write to them instead of the YOUSEF SHTIWE state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `yousef shtiwe claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `yousef shtiwe claw cleanup` to fix it.",
            "",
            "## YOUSEF SHTIWE-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `yousef shtiwe claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `yousef shtiwe setup` to configure any remaining settings",
            "- Run `yousef shtiwe mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `yousef shtiwe cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `yousef shtiwe cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `yousef shtiwe gateway install` if you need the gateway service",
            "- Review `~/.yousef shtiwe/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into YOUSEF SHTIWE Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=str(Path.home() / ".yousef shtiwe"), help="YOUSEF SHTIWE home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing YOUSEF SHTIWE targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of YOUSEF SHTIWE-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print(f"  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> YOUSEF SHTIWE Migration   [{mode_label:>8s}]   ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.yousef shtiwe/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.yousef shtiwe/config.yaml")
        print("    2. Run: yousef shtiwe mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: yousef shtiwe cron")
