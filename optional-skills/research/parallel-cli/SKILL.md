...
This is an optional third-party workflow, not a YOUSEF SHTIWE core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with YOUSEF SHTIWE native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer YOUSEF SHTIWE native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer YOUSEF SHTIWE native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
Recommended YOUSEF SHTIWE workflow:
...
## Recommended YOUSEF SHTIWE usage patterns
...
- Do not silently choose Parallel when YOUSEF SHTIWE native tools are already sufficient.
