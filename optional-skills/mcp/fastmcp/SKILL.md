...
- smoke-test a server with the FastMCP CLI before wiring it into YOUSEF SHTIWE or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to YOUSEF SHTIWE. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.yousef shtiwe/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.yousef shtiwe/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is YOUSEF SHTIWE integration, either:

- configure the server in `~/.yousef shtiwe/config.yaml` using the `native-mcp` skill, or
...
### YOUSEF SHTIWE cannot see the deployed server

The server-building part may be correct while the YOUSEF SHTIWE config is not. Load the `native-mcp` skill and configure the server in `~/.yousef shtiwe/config.yaml`, then restart YOUSEF SHTIWE.
