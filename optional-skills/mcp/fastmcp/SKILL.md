...
- smoke-test a server with the FastMCP CLI before wiring it into SHADOW or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to SHADOW. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.shadow/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.shadow/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is SHADOW integration, either:

- configure the server in `~/.shadow/config.yaml` using the `native-mcp` skill, or
...
### SHADOW cannot see the deployed server

The server-building part may be correct while the SHADOW config is not. Load the `native-mcp` skill and configure the server in `~/.shadow/config.yaml`, then restart SHADOW.
