#!/bin/bash
# ============================================================================
# YOUSEF SHTIWE Agent Setup Script
# ============================================================================
# Quick setup for developers who cloned the repo manually.
# Uses uv for desktop/server setup and Python's stdlib venv + pip on Termux.
#
# Usage:
#   ./setup-yousef shtiwe.sh
#
# This script:
...
# 5. Symlinks the 'yousef shtiwe' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ YOUSEF SHTIWE Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink yousef shtiwe into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up yousef shtiwe command..."

YOUSEF SHTIWE_BIN="$SCRIPT_DIR/venv/bin/yousef shtiwe"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$YOUSEF SHTIWE_BIN" "$COMMAND_LINK_DIR/yousef shtiwe"
echo -e "${GREEN}✓${NC} Symlinked yousef shtiwe → $COMMAND_LINK_DISPLAY_DIR/yousef shtiwe"
    ...
    if [ -n "$SHELL_CONFIG" ]; then
        ...
        if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
            if ! grep -q '\.local/bin' "$SHELL_CONFIG" 2>/dev/null; then
                echo "" >> "$SHELL_CONFIG"
                echo "# YOUSEF SHTIWE Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
# ============================================================================
# Seed bundled skills into ~/.yousef shtiwe/skills/
# ============================================================================

YOUSEF SHTIWE_SKILLS_DIR="${YOUSEF SHTIWE_HOME:-$HOME/.yousef shtiwe}/skills"
mkdir -p "$YOUSEF SHTIWE_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.yousef shtiwe/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$YOUSEF SHTIWE_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  yousef shtiwe status        # Check configuration"
if is_termux; then
    echo "  yousef shtiwe gateway       # Run gateway in foreground"
else
    echo "  yousef shtiwe gateway install # Install gateway service (messaging + cron)"
fi
echo "  yousef shtiwe cron list     # View scheduled jobs"
echo "  yousef shtiwe doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m yousef shtiwe_cli.main setup
fi
