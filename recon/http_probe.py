"""
yousef_shtiwe - HTTP Probing Module + Banner Grabbing
...
def get_host_path(container_path: str) -> str:
    ...
    /tmp/yousef_shtiwe is mounted to the same path inside and outside, so no translation needed.
    """
    # /tmp/yousef_shtiwe paths are the same inside and outside the container
    if container_path.startswith("/tmp/yousef_shtiwe"):
...
def run_http_probe(recon_data: dict, output_file: Path = None, settings: dict = None) -> dict:
    ...
    # Use /tmp/yousef_shtiwe to avoid spaces in paths (snap Docker issue)
    scan_temp_dir = Path("/tmp/yousef_shtiwe/.httpx_temp")
