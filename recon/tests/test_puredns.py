...
# Use a test-specific temp dir to avoid touching /tmp/yousef_shtiwe or /app/recon
...
def setup_test_env():
    ...
    data_dir = TEST_TMP / "yousef_shtiwe"
...
def test_successful_filtering():
    ...
    # The function writes to /tmp/yousef_shtiwe and reads from /app/recon/data
    # We'll create real files and mock only subprocess.run
    data_dir = Path("/tmp/yousef_shtiwe")
...
def test_command_construction_all_flags():
    ...
    resolver_shared = Path("/tmp/yousef_shtiwe/resolvers.txt")
    ...
    # Cleanup
    for f in Path("/tmp/yousef_shtiwe").glob("puredns_*"):
...
def test_input_file_content():
    ...
    def capture_run(cmd, **kwargs):
        ...
        input_path = Path("/tmp/yousef_shtiwe/puredns_input_example.com.txt")
...
if __name__ == "__main__":
    # Ensure /tmp/yousef_shtiwe exists with resolvers for filesystem-based tests
    Path("/tmp/yousef_shtiwe").mkdir(parents=True, exist_ok=True)
    resolver = Path("/tmp/yousef_shtiwe/resolvers.txt")
