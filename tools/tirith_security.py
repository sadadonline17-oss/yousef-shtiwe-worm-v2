...
it is automatically downloaded from GitHub releases to $SHADOW_HOME/bin/tirith.
...
def _get_shadow_home() -> str:
    """Return the SHADOW home directory, respecting SHADOW_HOME env var."""
    return str(get_shadow_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_shadow_home(), ".tirith-install-failed")
...
def _shadow_bin_dir() -> str:
    """Return $SHADOW_HOME/bin, creating it if needed."""
    d = os.path.join(_get_shadow_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SHADOW_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_shadow_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SHADOW_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SHADOW_HOME/bin/tirith
    ...
    shadow_bin = os.path.join(_shadow_bin_dir(), "tirith")
    if os.path.isfile(shadow_bin) and os.access(shadow_bin, os.X_OK):
        _resolved_path = shadow_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return shadow_bin
...
def ensure_installed(*, log_failures: bool = True):
    ...
    Quick PATH/local checks are synchroshadow; network download runs in a
    ...
    # Explicit path: synchroshadow check only, no download
    if explicit:
        if os.path.isfile(expanded) and os.access(expanded, os.X_OK):
            _resolved_path = expanded
            return expanded
