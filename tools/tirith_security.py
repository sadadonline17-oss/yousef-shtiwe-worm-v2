...
it is automatically downloaded from GitHub releases to $YOUSEF SHTIWE_HOME/bin/tirith.
...
def _get_yousef shtiwe_home() -> str:
    """Return the YOUSEF SHTIWE home directory, respecting YOUSEF SHTIWE_HOME env var."""
    return str(get_yousef shtiwe_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_yousef shtiwe_home(), ".tirith-install-failed")
...
def _yousef shtiwe_bin_dir() -> str:
    """Return $YOUSEF SHTIWE_HOME/bin, creating it if needed."""
    d = os.path.join(_get_yousef shtiwe_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $YOUSEF SHTIWE_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_yousef shtiwe_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $YOUSEF SHTIWE_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $YOUSEF SHTIWE_HOME/bin/tirith
    ...
    yousef shtiwe_bin = os.path.join(_yousef shtiwe_bin_dir(), "tirith")
    if os.path.isfile(yousef shtiwe_bin) and os.access(yousef shtiwe_bin, os.X_OK):
        _resolved_path = yousef shtiwe_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return yousef shtiwe_bin
...
def ensure_installed(*, log_failures: bool = True):
    ...
    Quick PATH/local checks are synchroyousef shtiwe; network download runs in a
    ...
    # Explicit path: synchroyousef shtiwe check only, no download
    if explicit:
        if os.path.isfile(expanded) and os.access(expanded, os.X_OK):
            _resolved_path = expanded
            return expanded
