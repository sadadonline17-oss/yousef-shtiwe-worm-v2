...
def managed_yousef shtiwe_tools_enabled() -> bool:
    """Return True when the hidden Yousef Shtiwe-managed tools feature flag is enabled."""
    return env_var_enabled("YOUSEF SHTIWE_ENABLE_Yousef Shtiwe_MANAGED_TOOLS")
...
def resolve_modal_backend_state(
    ...
    *,
    ...
    managed_mode_blocked = (
        requested_mode == "managed" and not managed_yousef shtiwe_tools_enabled()
    )

    if normalized_mode == "managed":
        selected_backend = "managed" if managed_yousef shtiwe_tools_enabled() and managed_ready else None
    elif normalized_mode == "direct":
        selected_backend = "direct" if has_direct else None
    else:
        selected_backend = "managed" if managed_yousef shtiwe_tools_enabled() and managed_ready else "direct" if has_direct else None
