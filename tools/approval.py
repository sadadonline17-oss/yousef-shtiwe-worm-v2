...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("YOUSEF SHTIWE_SESSION_KEY", default)

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $YOUSEF SHTIWE_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_YOUSEF SHTIWE_ENV_PATH = (
    r'(?:~\/\.yousef shtiwe/|'
    r'(?:\$home|\$\{home\})/\.yousef shtiwe/|'
    r'(?:\$yousef shtiwe_home|\$\{yousef shtiwe_home\})/)'
    r'\.env\b'
)
_SENSITIVE_WRITE_TARGET = (
    r'(?:/etc/|/dev/sd|'
    rf'{_SSH_SENSITIVE_PATH}|'
    rf'{_YOUSEF SHTIWE_ENV_PATH})'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart yousef shtiwe-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart yousef shtiwe-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(yousef shtiwe|gateway|cli\.py)\b', "kill yousef shtiwe/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill yousef shtiwe` but not
    # `kill -9 $(pgrep -f yousef shtiwe)` because the substitution is opaque
...
# =========================================================================
# Blocking gateway approval (mirrors CLI's synchroyousef shtiwe input() flow)
# =========================================================================
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if os.getenv("YOUSEF SHTIWE_YOLO_MODE") or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("YOUSEF SHTIWE_INTERACTIVE")
    is_gateway = os.getenv("YOUSEF SHTIWE_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        return {"approved": True, "message": None}

    if is_gateway or os.getenv("YOUSEF SHTIWE_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("YOUSEF SHTIWE_YOLO_MODE") or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("YOUSEF SHTIWE_INTERACTIVE")
    is_gateway = os.getenv("YOUSEF SHTIWE_GATEWAY_SESSION")
    is_ask = os.getenv("YOUSEF SHTIWE_EXEC_ASK")
    ...
    # responds with /approve or /deny, mirroring the CLI's synchroyousef shtiwe
