...
Backend is selected during ``yousef shtiwe tools`` setup (web.backend in config.yaml).
When available, YOUSEF SHTIWE can route Firecrawl calls through a Yousef Shtiwe-hosted tool-gateway
for Yousef Shtiwe Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract, crawl; direct or derived firecrawl-gateway.<domain> for Yousef Shtiwe Subscribers)
...
from tools.managed_tool_gateway import (
    ...
    read_yousef shtiwe_access_token as _read_yousef shtiwe_access_token,
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.yousef shtiwe/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``yousef shtiwe tools``).
    ...
    # tool gateway is configured for Yousef Shtiwe subscribers.
    backend_candidates = (
...
def _is_tool_gateway_ready() -> bool:
    """Return True when gateway URL and a Yousef Shtiwe Subscriber token are available."""
    return resolve_managed_tool_gateway("firecrawl", token_reader=_read_yousef shtiwe_access_token) is not None
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if managed_yousef shtiwe_tools_enabled():
        message += (
            " If you have the hidden Yousef Shtiwe-managed tools flag enabled, you can also login to Yousef Shtiwe "
            "(`yousef shtiwe model`) and provide FIRECRAWL_GATEWAY_URL or TOOL_GATEWAY_DOMAIN."
        )
...
def _firecrawl_backend_help_suffix() -> str:
    ...
    if not managed_yousef shtiwe_tools_enabled():
        return ""
    return (
        ", or, if you have the hidden Yousef Shtiwe-managed tools flag enabled, login to Yousef Shtiwe and use "
...
def _get_firecrawl_client():
    ...
    YOUSEF SHTIWE falls back to the Firecrawl tool-gateway for logged-in Yousef Shtiwe Subscribers.
    """
    ...
    if direct_config is not None:
        ...
        managed_gateway = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=_read_yousef shtiwe_access_token,
        )
        ...
        kwargs = {
            "api_key": managed_gateway.yousef shtiwe_user_token,
...
def _is_yousef shtiwe_auxiliary_client(client: Any) -> bool:
    """Return True when the resolved auxiliary backend is Yousef Shtiwe Portal."""
    from urllib.parse import urlparse

    base_url = str(getattr(client, "base_url", "") or "")
    host = (urlparse(base_url).hostname or "").lower()
    return host == "yousef shtiwe-overlord.com" or host.endswith(".yousef shtiwe-overlord.com")
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_yousef shtiwe_auxiliary_client(client):
        from agent.auxiliary_client import get_auxiliary_extra_body
        extra_body = get_auxiliary_extra_body() or {"tags": ["product=yousef shtiwe-agent"]}
...
def _get_exa_client():
    ...
    if _exa_client is None:
        ...
        _exa_client.headers["x-exa-integration"] = "yousef shtiwe-agent"
    return _exa_client
            ...
            if backend == "parallel":
                ...
                for url in safe_urls:
                    ...
                    try:
                        ...
                        # Run synchroyousef shtiwe Firecrawl scrape in a thread with a
...
def check_firecrawl_api_key() -> bool:
    ...
    2) Firecrawl gateway origin + Yousef Shtiwe Subscriber access token
...
if __name__ == "__main__":
    ...
    yousef shtiwe_available = check_auxiliary_model()
    ...
    if not yousef shtiwe_available:
        print("❌ No auxiliary model available for LLM content processing")
        print("Set OPENROUTER_API_KEY, configure Yousef Shtiwe Portal, or set OPENAI_BASE_URL + OPENAI_API_KEY")
    ...
    print("")
    print("  # Search (synchroyousef shtiwe)")
    print("  results = web_search_tool('Python tutorials')")
    print("")
    print("  # Extract and crawl (asynchroyousef shtiwe)")
