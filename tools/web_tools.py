...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.shadow/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``shadow tools``).
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    ...
    return host == "shadow-overlord.com" or host.endswith(".shadow-overlord.com")
...
def _get_exa_client():
    ...
    if _exa_client is None:
        ...
        _exa_client.headers["x-exa-integration"] = "shadow-agent"
    return _exa_client
