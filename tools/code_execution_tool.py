...
Lets the LLM write a Python script that calls YOUSEF SHTIWE tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `yousef shtiwe_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `yousef shtiwe_tools.py` with file-based RPC stubs
...
# ---------------------------------------------------------------------------
# yousef shtiwe_tools.py code generator
# ---------------------------------------------------------------------------
...
def generate_yousef shtiwe_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the yousef shtiwe_tools.py stub module.
...
"""Auto-generated YOUSEF SHTIWE tools RPC stubs."""
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["YOUSEF SHTIWE_RPC_SOCKET"])
...
"""Auto-generated YOUSEF SHTIWE tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, time

_RPC_DIR = os.environ.get("YOUSEF SHTIWE_RPC_DIR") or os.path.join(tempfile.gettempdir(), "yousef shtiwe_rpc")
_seq = 0
''' + _COMMON_HELPERS + '''\
...
def _execute_remote(
    ...
    The script and the generated yousef shtiwe_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/yousef shtiwe_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_yousef shtiwe_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/yousef shtiwe_tools.py", tools_src)
        ...
        env_prefix = (
            f"YOUSEF SHTIWE_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("YOUSEF SHTIWE_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    to a subset of YOUSEF SHTIWE tools.
    ...
    # --- Set up temp directory with yousef shtiwe_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="yousef shtiwe_sandbox_")
    # Use /tmp on macOS to avoid the long /var/folders/... path that pushes
    # Unix domain socket paths past the 104-byte macOS AF_UNIX limit.
    # On Linux, tempfile.gettempdir() already returns /tmp.
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()
    sock_path = os.path.join(_sock_tmpdir, f"yousef shtiwe_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated yousef shtiwe_tools module
        # sandbox_tools is already the correct set (intersection with session
        # tools, or SANDBOX_ALLOWED_TOOLS as fallback — see lines above).
        tools_src = generate_yousef shtiwe_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "yousef shtiwe_tools.py"), "w") as f:
            f.write(tools_src)
        ...
        for k, v in os.environ.items():
            ...
            if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
                child_env[k] = v
        child_env["YOUSEF SHTIWE_RPC_SOCKET"] = sock_path
        child_env["PYTHONDONTWRITEBYTECODE"] = "1"
        # Ensure the yousef shtiwe-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.
        _yousef shtiwe_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        child_env["PYTHONPATH"] = _yousef shtiwe_root + (os.pathsep + _existing_pp if _existing_pp else "")
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.
        _tz_name = os.getenv("YOUSEF SHTIWE_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name

        # Per-profile HOME isolation: redirect system tool configs into
        # {YOUSEF SHTIWE_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.yousef shtiwe/.env')).
...
def build_execute_code_schema(enabled_sandbox_tools: set = None) -> dict:
    ...
    When tools are disabled via ``yousef shtiwe tools`` (e.g. web is turned off),
    ...
    description = (
        "Run a Python script that can call YOUSEF SHTIWE tools programmatically. "
        ...
        f"Available via `from yousef shtiwe_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        "Limits: 5-minute timeout, 50KB stdout cap, max 50 tool calls per script. "
        "terminal() is foreground-only (no background or pty).\n\n"
        "Print your final result to stdout. Use Python stdlib (json, re, math, csv, "
        "datetime, collections, etc.) for processing between tool calls.\n\n"
        "Also available (no import needed — built into yousef shtiwe_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from yousef shtiwe_tools import {import_str}` "
