...
# All skills live in ~/.shadow/skills/ (seeded from bundled skills/ on install).
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from SHADOW_HOME/.env."""
...
def _is_gateway_surface() -> bool:
    if os.getenv("SHADOW_GATEWAY_SESSION"):
        return True
    from gateway.session_context import get_session_env
    return bool(get_session_env("SHADOW_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return "Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to ~/.shadow/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.shadow/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("SHADOW_PLATFORM")
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.shadow/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``shadow skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": "No skills found. Skills directory created at ~/.shadow/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    from shadow_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: shadow plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
...
def skill_view(name: str, file_path: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        if ":" in name:
            ...
            from shadow_cli.plugins import discover_plugins, get_plugin_manager
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.shadow/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `shadow skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.shadow.* first (agentskills.io convention), fall back to top-level
        shadow_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            shadow_meta = metadata.get("shadow", {}) or {}

        tags = _parse_tags(shadow_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            shadow_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
