...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── SHADOW internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files.
        from shadow_constants import get_shadow_home as _get_hh
        _shadow_home = _get_hh().resolve()
        _blocked_dirs = [
            _shadow_home / "skills" / ".hub" / "index-cache",
            _shadow_home / "skills" / ".hub",
        ]
        for _blocked in _blocked_dirs:
            try:
                ...
                return json.dumps({
                    "error": (
                        f"Access denied: {path} is an internal SHADOW cache file "
