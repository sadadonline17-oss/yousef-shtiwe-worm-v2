...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── YOUSEF SHTIWE internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files.
        from yousef shtiwe_constants import get_yousef shtiwe_home as _get_hh
        _yousef shtiwe_home = _get_hh().resolve()
        _blocked_dirs = [
            _yousef shtiwe_home / "skills" / ".hub" / "index-cache",
            _yousef shtiwe_home / "skills" / ".hub",
        ]
        for _blocked in _blocked_dirs:
            try:
                ...
                return json.dumps({
                    "error": (
                        f"Access denied: {path} is an internal YOUSEF SHTIWE cache file "
