...
def _resolve_yousef shtiwe_home() -> Path:
    from yousef shtiwe_constants import get_yousef shtiwe_home
    return get_yousef shtiwe_home()


def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.yousef shtiwe",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``YOUSEF SHTIWE_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside YOUSEF SHTIWE_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    yousef shtiwe_home = _resolve_yousef shtiwe_home()

    # Reject absolute paths — they bypass the YOUSEF SHTIWE_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to YOUSEF SHTIWE_HOME)",
            relative_path,
        )
        return False

    host_path = yousef shtiwe_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape YOUSEF SHTIWE_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, yousef shtiwe_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if os.path.isabs(rel):
                        ...
                        continue
                    host_path = yousef shtiwe_home / rel
                    containment_error = validate_within_dir(host_path, yousef shtiwe_home)
                    if containment_error:
                        logger.warning(
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.yousef shtiwe/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="yousef shtiwe-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching yousef shtiwe_constants.get_yousef shtiwe_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_yousef shtiwe_dir()`` for backward compatibility with old directory layouts.
    """
    from yousef shtiwe_constants import get_yousef shtiwe_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_yousef shtiwe_dir(new_subpath, old_name)
        if host_dir.is_dir():
