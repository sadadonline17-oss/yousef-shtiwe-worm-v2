"""
Unified tool configuration for YOUSEF SHTIWE Agent.

`yousef shtiwe tools` and `yousef shtiwe setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.yousef shtiwe/config.yaml under
...
from yousef shtiwe_cli.yousef shtiwe_subscription import (
    apply_yousef shtiwe_managed_defaults,
...
from yousef shtiwe_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _YOUSEF SHTIWE_CORE_TOOLS (available at runtime if enabled),
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from yousef shtiwe_cli.plugins import discover_plugins, get_plugin_toolsets
...
from yousef shtiwe_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            {
                "name": "Yousef Shtiwe Subscription",
                "tag": "Managed OpenAI TTS billed to your subscription",
                "env_vars": [],
                "tts_provider": "openai",
                "requires_yousef shtiwe_auth": True,
                "managed_yousef shtiwe_feature": "tts",
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_yousef shtiwe_auth": True,
                "managed_yousef shtiwe_feature": "web",
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_yousef shtiwe_auth": True,
                "managed_yousef shtiwe_feature": "image_gen",
    ...
    "browser": {
        ...
        "providers": [
            {
                "name": "Yousef Shtiwe Subscription (Browser Use cloud)",
                "tag": "Managed Browser Use billed to your subscription",
                "env_vars": [],
                "browser_provider": "browser-use",
                "requires_yousef shtiwe_auth": True,
                "managed_yousef shtiwe_feature": "browser",
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_yousef shtiwe_home()}/yousef shtiwe-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in yousef shtiwe-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "yousef shtiwe-cli" (which include all _YOUSEF SHTIWE_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "yousef shtiwe-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `yousef shtiwe tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by yousef shtiwe tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (yousef shtiwe-cli, yousef shtiwe-telegram, etc.)
...
def _toolset_has_keys(ts_key: str, config: dict = None) -> bool:
    ...
    if ts_key in {"web", "image_gen", "tts", "browser"}:
        features = get_yousef shtiwe_subscription_features(config)
        feature = features.features.get(ts_key)
        if feature and (feature.available or feature.managed_by_yousef shtiwe):
            return True
...
def _visible_providers(cat: dict, config: dict) -> list[dict]:
    ...
    for provider in cat.get("providers", []):
        if provider.get("managed_yousef shtiwe_feature") and not managed_yousef shtiwe_tools_enabled():
            continue
        if provider.get("requires_yousef shtiwe_auth") and not features.yousef shtiwe_auth_present:
            continue
...
def _is_provider_active(provider: dict, config: dict) -> bool:
    ...
    managed_feature = provider.get("managed_yousef shtiwe_feature")
    if managed_feature:
        features = get_yousef shtiwe_subscription_features(config)
        ...
        if managed_feature == "image_gen":
            return feature.managed_by_yousef shtiwe
        if provider.get("tts_provider"):
            return (
                feature.managed_by_yousef shtiwe
                and config.get("tts", {}).get("provider") == provider["tts_provider"]
            )
        if "browser_provider" in provider:
            current = config.get("browser", {}).get("cloud_provider")
            return feature.managed_by_yousef shtiwe and provider["browser_provider"] == current
        if provider.get("web_backend"):
            current = config.get("web", {}).get("backend")
            return feature.managed_by_yousef shtiwe and current == provider["web_backend"]
        return feature.managed_by_yousef shtiwe
...
def _configure_provider(provider: dict, config: dict):
    ...
    if provider.get("requires_yousef shtiwe_auth"):
        features = get_yousef shtiwe_subscription_features(config)
        if not features.yousef shtiwe_auth_present:
            _print_warning("  Yousef Shtiwe Subscription is only available after logging into Yousef Shtiwe Portal.")
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your Yousef Shtiwe subscription.")
            override_envs = provider.get("override_env_vars", [])
            if any(get_env_value(env_var) for env_var in override_envs):
                _print_warning(
                    "  Direct credentials are still configured and may take precedence until you remove them from ~/.yousef shtiwe/.env."
                )
        return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `yousef shtiwe tools` and `yousef shtiwe setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ YOUSEF SHTIWE Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://yousef shtiwe-agent.yousef shtiwe-overlord.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            auto_configured = apply_yousef shtiwe_managed_defaults(
                config,
            ...
            if managed_yousef shtiwe_tools_enabled():
                for ts_key in sorted(auto_configured):
                    ...
                    print(color(f"  ✓ {label}: using your Yousef Shtiwe subscription defaults", Colors.GREEN))
    ...
    print()
    from yousef shtiwe_constants import display_yousef shtiwe_home
    print(color(f"  Tool configuration saved to {display_yousef shtiwe_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'yousef shtiwe' or gateway restart.", Colors.DIM))
    print()
