"""
Profile management for multiple isolated YOUSEF SHTIWE instances.

Each profile is a fully independent YOUSEF SHTIWE_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.yousef shtiwe/profiles/<name>/`` by default.

The "default" profile is ``~/.yousef shtiwe`` itself — backward compatible,
zero migration needed.

Usage::

    yousef shtiwe profile create coder          # fresh profile + bundled skills
    yousef shtiwe profile create coder --clone  # also copy config, .env, SOUL.md
    yousef shtiwe profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    yousef shtiwe -p coder chat                 # or via flag
    yousef shtiwe profile use coder             # set as sticky default
    yousef shtiwe profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See yousef shtiwe_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Directories/files to exclude when exporting the default (~/.yousef shtiwe) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "yousef shtiwe-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "yousef shtiwe", "default", "test", "tmp", "root", "sudo",
})

# YOUSEF SHTIWE subcommands that cannot be used as profile names/aliases
_YOUSEF SHTIWE_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the yousef shtiwe root, NOT to the current YOUSEF SHTIWE_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where YOUSEF SHTIWE_HOME points outside
    ``~/.yousef shtiwe``, profiles live under ``YOUSEF SHTIWE_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_yousef shtiwe_home() / "profiles"


def _get_default_yousef shtiwe_home() -> Path:
    """Return the default (pre-profile) YOUSEF SHTIWE_HOME path.

    In standard deployments this is ``~/.yousef shtiwe``.
    In Docker/custom deployments where YOUSEF SHTIWE_HOME is outside ``~/.yousef shtiwe``
    (e.g. ``/opt/data``), returns YOUSEF SHTIWE_HOME directly.
    """
    from yousef shtiwe_constants import get_default_yousef shtiwe_root
    return get_default_yousef shtiwe_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_yousef shtiwe_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.yousef shtiwe
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its YOUSEF SHTIWE_HOME directory."""
    if name == "default":
        return _get_default_yousef shtiwe_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, yousef shtiwe subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _YOUSEF SHTIWE_SUBCOMMANDS:
        return f"'{name}' conflicts with a yousef shtiwe subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_yousef shtiwe_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.yousef shtiwe)."
        )
    ...
    if clone_from is not None or clone_all or clone_config:
        if clone_from is None:
            ...
            source_dir = get_yousef shtiwe_home()
        else:
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches YOUSEF SHTIWE_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "YOUSEF SHTIWE_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.yousef shtiwe).\n"
            "To remove everything, use: yousef shtiwe uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set YOUSEF SHTIWE_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("YOUSEF SHTIWE_HOME")
    try:
        os.environ["YOUSEF SHTIWE_HOME"] = str(profile_dir)
        from yousef shtiwe_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["YOUSEF SHTIWE_HOME"] = old_home
        elif "YOUSEF SHTIWE_HOME" in os.environ:
            del os.environ["YOUSEF SHTIWE_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.yousef shtiwe/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: yousef shtiwe profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from YOUSEF SHTIWE_HOME.

    Returns ``"default"`` if YOUSEF SHTIWE_HOME is not set or points to ``~/.yousef shtiwe``.
    Returns the profile name if YOUSEF SHTIWE_HOME points into ``~/.yousef shtiwe/profiles/<name>``.
    Returns ``"custom"`` if YOUSEF SHTIWE_HOME is set to an unrecognized path.
    """
    from yousef shtiwe_constants import get_yousef shtiwe_home
    yousef shtiwe_home = get_yousef shtiwe_home()
    resolved = yousef shtiwe_home.resolve()

    default_resolved = _get_default_yousef shtiwe_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.yousef shtiwe itself — its parent is ~/ and its
        # directory name is ".yousef shtiwe", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: yousef shtiwe profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.yousef shtiwe itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.yousef shtiwe). "
            "Specify a different name: yousef shtiwe profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for yousef shtiwe profile names."""
    return '''# YOUSEF SHTIWE Agent profile completion
# Add to ~/.bashrc: eval "$(yousef shtiwe completion bash)"

_yousef shtiwe_profiles() {
    local profiles_dir="$HOME/.yousef shtiwe/profiles"
...
_yousef shtiwe_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_yousef shtiwe_profiles)" -- "$cur"))
        return
    fi
...
complete -F _yousef shtiwe_completion yousef shtiwe
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for yousef shtiwe profile names."""
    return '''#compdef yousef shtiwe
# YOUSEF SHTIWE Agent profile completion
# Add to ~/.zshrc: eval "$(yousef shtiwe completion zsh)"

_yousef shtiwe() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.yousef shtiwe/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.yousef shtiwe/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a YOUSEF SHTIWE_HOME path string.

    Called early in the CLI entry point, before any yousef shtiwe modules
    are imported, to set the YOUSEF SHTIWE_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: yousef shtiwe profile create {profile_name}"
        )
