"""
Doctor command for yousef shtiwe CLI.

Diagnoses issues with YOUSEF SHTIWE Agent setup.
"""
...
from yousef shtiwe_cli.config import get_project_root, get_yousef shtiwe_home, get_env_path
from yousef shtiwe_constants import display_yousef shtiwe_home

PROJECT_ROOT = get_project_root()
YOUSEF SHTIWE_HOME = get_yousef shtiwe_home()
_DHH = display_yousef shtiwe_home()  # user-facing display path (e.g. ~/.yousef shtiwe or ~/.yousef shtiwe/profiles/coder)

# Load environment variables from ~/.yousef shtiwe/.env so API key checks work
...
from yousef shtiwe_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.yousef shtiwe/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `yousef shtiwe`.
    os.environ.setdefault("YOUSEF SHTIWE_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 YOUSEF SHTIWE Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.yousef shtiwe/.env (primary location for user config)
    env_path = YOUSEF SHTIWE_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'yousef shtiwe setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'yousef shtiwe setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'yousef shtiwe setup' to create one")
                issues.append("Run 'yousef shtiwe setup' to create .env")
    
    # Check ~/.yousef shtiwe/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        try:
            from yousef shtiwe_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'yousef shtiwe setup' to migrate config")
                else:
                    issues.append("Run 'yousef shtiwe doctor --fix' or 'yousef shtiwe setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'yousef shtiwe doctor --fix'")
        except Exception:
            pass
    ...
    try:
        from yousef shtiwe_cli.auth import get_yousef shtiwe_auth_status, get_codex_auth_status

        yousef shtiwe_status = get_yousef shtiwe_auth_status()
        if yousef shtiwe_status.get("logged_in"):
            check_ok("Yousef Shtiwe Portal auth", "(logged in)")
        else:
            check_warn("Yousef Shtiwe Portal auth", "(not logged in)")
    ...
    for subdir_name in expected_subdirs:
        subdir_path = yousef shtiwe_home / subdir_name
    ...
    soul_path = yousef shtiwe_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give YOUSEF SHTIWE a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# YOUSEF SHTIWE Agent Persona\n\n"
                "<!-- Edit this file to customize how YOUSEF SHTIWE communicates. -->\n\n"
                "You are YOUSEF SHTIWE, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = yousef shtiwe_home / "memories"
    ...
    state_db_path = yousef shtiwe_home / "state.db"
    ...
    wal_path = yousef shtiwe_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'yousef shtiwe doctor --fix' to checkpoint")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'yousef shtiwe setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    hub_dir = YOUSEF SHTIWE_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: yousef shtiwe skills list)")
    ...
    try:
        ...
        _mem_cfg_path = YOUSEF SHTIWE_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: yousef shtiwe memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: yousef shtiwe memory setup")
                issues.append("No Honcho API key — run 'yousef shtiwe memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run yousef shtiwe memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: yousef shtiwe memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: yousef shtiwe memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from yousef shtiwe_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "yousef shtiwe -p" in content:
                            _m = _re.search(r"yousef shtiwe -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'yousef shtiwe doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
