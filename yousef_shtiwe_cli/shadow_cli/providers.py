"""
Single source of truth for provider identity in YOUSEF SHTIWE Agent.
...
2. **YOUSEF SHTIWE overlays** — transport type, auth patterns, aggregator flags,
...
# -- YOUSEF SHTIWE overlay ----------------------------------------------------------
# YOUSEF SHTIWE-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class YOUSEF SHTIWEOverlay:
    """YOUSEF SHTIWE-specific provider metadata layered on top of models.dev."""
...
YOUSEF SHTIWE_OVERLAYS: Dict[str, YOUSEF SHTIWEOverlay] = {
    "openrouter": YOUSEF SHTIWEOverlay(
        transport="openai_chat",
        is_aggregator=True,
        extra_env_vars=("OPENAI_API_KEY",),
        base_url_env_var="OPENROUTER_BASE_URL",
    ),
    "yousef shtiwe": YOUSEF SHTIWEOverlay(
        transport="openai_chat",
        auth_type="oauth_device_code",
        base_url_override="https://inference-api.yousef shtiwe-overlord.com/v1",
    ),
    "openai-codex": YOUSEF SHTIWEOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": YOUSEF SHTIWEOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="YOUSEF SHTIWE_QWEN_BASE_URL",
    ),
    "copilot-acp": YOUSEF SHTIWEOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": YOUSEF SHTIWEOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": YOUSEF SHTIWEOverlay(
        transport="anthropic_messages",
        extra_env_vars=("ANTHROPIC_TOKEN", "CLAUDE_CODE_OAUTH_TOKEN"),
    ),
    "zai": YOUSEF SHTIWEOverlay(
        transport="openai_chat",
        extra_env_vars=("GLM_API_KEY", "ZAI_API_KEY", "Z_AI_API_KEY"),
        base_url_env_var="GLM_BASE_URL",
    ),
    "kimi-for-coding": YOUSEF SHTIWEOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "minimax": YOUSEF SHTIWEOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-cn": YOUSEF SHTIWEOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": YOUSEF SHTIWEOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": YOUSEF SHTIWEOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "vercel": YOUSEF SHTIWEOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": YOUSEF SHTIWEOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": YOUSEF SHTIWEOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "kilo": YOUSEF SHTIWEOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="KILOCODE_BASE_URL",
    ),
    "huggingface": YOUSEF SHTIWEOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "xai": YOUSEF SHTIWEOverlay(
        transport="openai_chat",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "xiaomi": YOUSEF SHTIWEOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "arcee": YOUSEF SHTIWEOverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "yousef shtiwe", "user-config"
...
_LABEL_OVERRIDES: Dict[str, str] = {
    "yousef shtiwe": "Yousef Shtiwe Portal",
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. YOUSEF SHTIWE overlays (for providers not in models.dev: yousef shtiwe, openai-codex, etc.)
      2. models.dev catalog + YOUSEF SHTIWE overlay
    ...
    overlay = YOUSEF SHTIWE_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + yousef shtiwe extra
    ...
    if overlay is not None:
        # YOUSEF SHTIWE-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
