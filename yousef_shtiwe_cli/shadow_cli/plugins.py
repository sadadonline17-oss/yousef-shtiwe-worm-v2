"""
YOUSEF SHTIWE Plugin System
====================

Discovers, loads, and manages plugins from three sources:

1. **User plugins**   – ``~/.yousef shtiwe/plugins/<name>/``
2. **Project plugins** – ``./.yousef shtiwe/plugins/<name>/`` (opt-in via
   ``YOUSEF SHTIWE_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``yousef shtiwe_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "yousef shtiwe_agent.plugins"

_NS_PARENT = "yousef shtiwe_plugins"
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``yousef shtiwe honcho ...``).
    ...
    def register_skill(
        ...
        ``~/.yousef shtiwe/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.yousef shtiwe/plugins/)
        user_dir = get_yousef shtiwe_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.yousef shtiwe/plugins/)
        if _env_enabled("YOUSEF SHTIWE_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".yousef shtiwe" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``yousef shtiwe_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``yousef shtiwe tools`` TUI so plugin-provided toolsets appear
