...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "yousef shtiwe", "openai-codex", "qwen-oauth"}
...
def auth_add_command(args) -> None:
    ...
    if not requested_type:
        if provider.startswith(CUSTOM_POOL_PREFIX):
            ...
            requested_type = AUTH_TYPE_OAUTH if provider in {"anthropic", "yousef shtiwe", "openai-codex", "qwen-oauth"} else AUTH_TYPE_API_KEY
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_yousef shtiwe_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:yousef shtiwe_pkce",
    ...
    if provider == "yousef shtiwe":
        creds = auth_mod._yousef shtiwe_device_code_login(
    ...
    raise SystemExit(f"`yousef shtiwe auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    if removed.source.startswith("env:"):
        ...
        if env_var:
            from yousef shtiwe_cli.config import remove_env_value
            cleared = remove_env_value(env_var)
            if cleared:
                print(f"Cleared {env_var} from .env")

    # If this was a singleton-seeded credential (OAuth device_code, yousef shtiwe_pkce),
    # clear the underlying auth store / credential file so it doesn't get
    # re-seeded on the next load_pool() call.
    elif removed.source == "device_code" and provider in ("openai-codex", "yousef shtiwe"):
    ...
    elif removed.source == "yousef shtiwe_pkce" and provider == "anthropic":
        from yousef shtiwe_constants import get_yousef shtiwe_home
        oauth_file = get_yousef shtiwe_home() / ".anthropic_oauth.json"
        if oauth_file.exists():
            oauth_file.unlink()
            print("Cleared YOUSEF SHTIWE Anthropic OAuth credentials")

    elif removed.source == "claude_code" and provider == "anthropic":
        from yousef shtiwe_cli.auth import suppress_credential_source
        ...
        print("Run `yousef shtiwe auth add anthropic` to re-enable if needed.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `yousef shtiwe auth` is called bare."""
