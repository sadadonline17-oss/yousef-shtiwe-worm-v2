#!/usr/bin/env python3
"""
YOUSEF SHTIWE CLI - Main entry point.

Usage:
    yousef shtiwe                     # Interactive chat (default)
    yousef shtiwe chat                # Interactive chat
    yousef shtiwe gateway             # Run gateway in foreground
    yousef shtiwe gateway start       # Start gateway as service
    yousef shtiwe gateway stop        # Stop gateway service
    yousef shtiwe gateway status      # Show gateway status
    yousef shtiwe gateway install     # Install gateway service
    yousef shtiwe gateway uninstall   # Uninstall gateway service
    yousef shtiwe setup               # Interactive setup wizard
    yousef shtiwe logout              # Clear stored authentication
    yousef shtiwe status              # Show status of all components
    yousef shtiwe cron                # Manage cron jobs
    yousef shtiwe cron list           # List cron jobs
    yousef shtiwe cron status         # Check if cron scheduler is running
    yousef shtiwe doctor              # Check configuration and dependencies
    yousef shtiwe honcho setup                    # Configure Honcho AI memory integration
    yousef shtiwe honcho status                   # Show Honcho config and connection status
    yousef shtiwe honcho sessions                 # List directory → session name mappings
    yousef shtiwe honcho map <name>               # Map current directory to a session name
    yousef shtiwe honcho peer                     # Show peer names and dialectic settings
    yousef shtiwe honcho peer --user NAME         # Set user peer name
    yousef shtiwe honcho peer --ai NAME           # Set AI peer name
    yousef shtiwe honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    yousef shtiwe honcho mode                     # Show current memory mode
    yousef shtiwe honcho mode [hybrid|honcho|local]  # Set memory mode
    yousef shtiwe honcho tokens                   # Show token budget settings
    yousef shtiwe honcho tokens --context N       # Set session.context() token cap
    yousef shtiwe honcho tokens --dialectic N     # Set dialectic result char cap
    yousef shtiwe honcho identity                 # Show AI peer identity representation
    yousef shtiwe honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    yousef shtiwe honcho migrate                  # Step-by-step migration guide: OpenClaw native → YOUSEF SHTIWE + Honcho
    yousef shtiwe version             Show version
    yousef shtiwe update              Update to latest version
    yousef shtiwe uninstall           Uninstall YOUSEF SHTIWE Agent
    yousef shtiwe acp                 Run as an ACP server for editor integration
    yousef shtiwe sessions browse     Interactive session picker with search

    yousef shtiwe claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (yousef shtiwe tools, yousef shtiwe setup, yousef shtiwe model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'yousef shtiwe {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any yousef shtiwe module import.
#
# Many modules cache YOUSEF SHTIWE_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("YOUSEF SHTIWE_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.yousef shtiwe/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set YOUSEF SHTIWE_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check active_profile in the yousef shtiwe root
    if profile_name is None:
        try:
            from yousef shtiwe_constants import get_default_yousef shtiwe_root
            active_path = get_default_yousef shtiwe_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set YOUSEF SHTIWE_HOME
    if profile_name is not None:
        try:
            from yousef shtiwe_cli.profiles import resolve_profile_env
            yousef shtiwe_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent yousef shtiwe from starting
            print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
            return
        os.environ["YOUSEF SHTIWE_HOME"] = yousef shtiwe_home
...
# Load .env from ~/.yousef shtiwe/.env first, then project root as dev fallback.
...
load_yousef shtiwe_dotenv(project_env=PROJECT_ROOT / '.env')

# Initialize centralized file logging early — all `yousef shtiwe` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from yousef shtiwe_logging import setup_logging as _setup_logging
...
try:
    from yousef shtiwe_cli.config import load_config as _load_config_early
    from yousef shtiwe_constants import apply_ipv4_preference as _apply_ipv4
...
from yousef shtiwe_cli import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    from yousef shtiwe_cli.config import get_env_path, get_yousef shtiwe_home, load_config
    from yousef shtiwe_cli.auth import get_auth_status

    # Determine whether YOUSEF SHTIWE itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_yousef shtiwe_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Check for Yousef Shtiwe Portal OAuth credentials
    auth_file = get_yousef shtiwe_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if YOUSEF SHTIWE has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants YOUSEF SHTIWE to use their tokens.
    if _has_yousef shtiwe_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, yousef shtiwe_bin
        cli_args: the original CLI arguments (everything after 'yousef shtiwe')
    """
    import shutil
    ...
    yousef shtiwe_bin = container_info["yousef shtiwe_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f'  }}];\n'
                    f"\n"
                    f"Or run: sudo yousef shtiwe {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo yousef shtiwe {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    exec_cmd = (
        ...
        + ["-u", exec_user]
        + env_flags
        + [container_name, yousef shtiwe_bin]
        + cli_args
    )
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'yousef shtiwe sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like YOUSEF SHTIWE isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  yousef shtiwe setup")
        print()

        from yousef shtiwe_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'yousef shtiwe setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from yousef shtiwe_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["YOUSEF SHTIWE_YOLO_MODE"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["YOUSEF SHTIWE_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from yousef shtiwe_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from yousef shtiwe_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with YOUSEF SHTIWE?")
        print()
    ...
    session_dir = get_yousef shtiwe_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: yousef shtiwe gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  yousef shtiwe gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ YOUSEF SHTIWE Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  yousef shtiwe gateway")
        ...
        print()
        print("  Or install as a service: yousef shtiwe gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'yousef shtiwe whatsapp' to try again.")
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``yousef shtiwe model``) and the setup wizard
    ...
    from yousef shtiwe_cli.config import get_compatible_custom_providers, load_config, get_env_value
    ...
    effective_provider = (
        config_provider
        or os.getenv("YOUSEF SHTIWE_INFERENCE_PROVIDER")
        or "auto"
    )
    try:
    ...
    from yousef shtiwe_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    elif selected_provider == "yousef shtiwe":
        _model_flow_yousef shtiwe(config, current_model, args=args)
    ...
    # a leftover OPENAI_BASE_URL in ~/.yousef shtiwe/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.yousef shtiwe/.env if the active provider is not 'custom'.
    ...
    """
    from yousef shtiwe_cli.config import get_env_value, save_env_value, load_config
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from yousef shtiwe_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from yousef shtiwe_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from yousef shtiwe_cli.config import get_env_value, save_env_value
    ...
    from yousef shtiwe_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_yousef shtiwe(config, current_model="", args=None):
    """Yousef Shtiwe Portal provider: ensure logged in, then pick model."""
    from yousef shtiwe_cli.auth import (
        get_provider_auth_state, _prompt_model_selection, _save_model_choice,
        _update_config_for_provider, resolve_yousef shtiwe_runtime_credentials,
        AuthError, format_auth_error,
        _login_yousef shtiwe, PROVIDER_REGISTRY,
    )
    from yousef shtiwe_cli.config import get_env_value, save_config, save_env_value
    from yousef shtiwe_cli.yousef shtiwe_subscription import (
        apply_yousef shtiwe_provider_defaults,
        get_yousef shtiwe_subscription_explainer_lines,
    )
    import argparse

    state = get_provider_auth_state("yousef shtiwe")
    if not state or not state.get("access_token"):
        print("Not logged into Yousef Shtiwe Portal. Starting login...")
        print()
        try:
            ...
            _login_yousef shtiwe(mock_args, PROVIDER_REGISTRY["yousef shtiwe"])
            print()
            for line in get_yousef shtiwe_subscription_explainer_lines():
                print(line)
        except SystemExit:
            ...
            return
        # login_yousef shtiwe already handles model selection + config update
        return
    ...
    from yousef shtiwe_cli.models import (
        _PROVIDER_MODELS, get_pricing_for_provider, filter_yousef shtiwe_free_models,
        check_yousef shtiwe_free_tier, partition_yousef shtiwe_models_by_tier,
    )
    model_ids = _PROVIDER_MODELS.get("yousef shtiwe", [])
    if not model_ids:
        print("No curated models available for Yousef Shtiwe Portal.")
        return

    # Verify credentials are still valid (catches expired sessions early)
    try:
        creds = resolve_yousef shtiwe_runtime_credentials(min_key_ttl_seconds=5 * 60)
    except Exception as exc:
        relogin = isinstance(exc, AuthError) and exc.relogin_required
        msg = format_auth_error(exc) if isinstance(exc, AuthError) else str(exc)
        if relogin:
            print(f"Session expired: {msg}")
            print("Re-authenticating with Yousef Shtiwe Portal...\n")
            try:
                mock_args = argparse.Namespace(
    ...
    # Fetch live pricing (non-blocking — returns empty dict on failure)
    pricing = get_pricing_for_provider("yousef shtiwe")

    # Check if user is on free tier
    free_tier = check_yousef shtiwe_free_tier()
    ...
    model_ids = filter_yousef shtiwe_free_models(model_ids, pricing)
    unavailable_models: list[str] = []
    if free_tier:
        model_ids, unavailable_models = partition_yousef shtiwe_models_by_tier(model_ids, pricing, free_tier=True)

    if not model_ids and not unavailable_models:
        print("No models available for Yousef Shtiwe Portal after filtering.")
        return

    # Resolve portal URL for upgrade links (may differ on staging)
    _yousef shtiwe_portal_url = ""
    try:
        _yousef shtiwe_state = get_provider_auth_state("yousef shtiwe")
        if _yousef shtiwe_state:
            _yousef shtiwe_portal_url = _yousef shtiwe_state.get("portal_base_url", "")
    except Exception:
        pass
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from yousef shtiwe_cli.auth import DEFAULT_Yousef Shtiwe_PORTAL_URL
            _url = (_yousef shtiwe_portal_url or DEFAULT_Yousef Shtiwe_PORTAL_URL).rstrip("/")
    ...
    selected = _prompt_model_selection(
        ...
        unavailable_models=unavailable_models, portal_url=_yousef shtiwe_portal_url,
    )
    if selected:
        _save_model_choice(selected)
        # Reactivate Yousef Shtiwe as the provider and update config
        inference_url = creds.get("base_url", "")
        _update_config_for_provider("yousef shtiwe", inference_url)
        ...
        if isinstance(current_model_cfg, dict):
            ...
            model_cfg = {}
        model_cfg["provider"] = "yousef shtiwe"
        ...
        changed_defaults = apply_yousef shtiwe_provider_defaults(config)
        save_config(config)
        print(f"Default model set to: {selected} (via Yousef Shtiwe Portal)")
        if "tts" in changed_defaults:
            print("TTS provider set to: OpenAI TTS via your Yousef Shtiwe subscription")
        else:
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from yousef shtiwe_cli.codex_models import get_codex_model_ids
    import argparse
    ...
    _codex_token = None
    # Prefer credential pool (where `yousef shtiwe auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from yousef shtiwe_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from yousef shtiwe_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from yousef shtiwe_cli.auth import _save_model_choice, deactivate_provider
    from yousef shtiwe_cli.config import get_env_value, load_config, save_config
    ...
    from yousef shtiwe_cli.models import probe_api_models
    ...
    if probe.get("used_fallback") and probe.get("resolved_base_url"):
        ...
        print(
            ...
            f"YOUSEF SHTIWE will still save it."
        )
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `yousef shtiwe model` to set a model.")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from yousef shtiwe_cli.curses_ui import flush_stdin
        flush_stdin()
        print()
...
def _model_flow_copilot(config, current_model=""):
    ...
    from yousef shtiwe_cli.config import save_env_value, load_config, save_config
    from yousef shtiwe_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from yousef shtiwe_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from yousef shtiwe_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates YOUSEF SHTIWE turns to `copilot --acp`.")
    print("  YOUSEF SHTIWE currently starts its own ACP subprocess for each request.")
    print("  YOUSEF SHTIWE uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set YOUSEF SHTIWE_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from yousef shtiwe_cli.config import get_env_value, save_env_value, load_config, save_config
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from yousef shtiwe_cli.config import get_env_value, save_env_value, load_config, save_config
    from yousef shtiwe_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from yousef shtiwe_constants import display_yousef shtiwe_home as _dhh_fn
            print(f"    YOUSEF SHTIWE will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
    ...
    try:
        ...
        print("    4. Re-run:               yousef shtiwe model")
        print()
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from yousef shtiwe_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'yousef shtiwe model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate YOUSEF SHTIWE CLI with a provider."""
    from yousef shtiwe_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from yousef shtiwe_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from yousef shtiwe_cli.auth_commands import auth_command
    auth_command(args)


def cmd_status(args):
    """Show status of all components."""
    from yousef shtiwe_cli.status import show_status
    show_status(args)
...
def cmd_webhook(args):
    ...
    from yousef shtiwe_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from yousef shtiwe_cli.doctor import run_doctor
    run_doctor(args)


def cmd_dump(args):
    """Dump setup summary for support/debugging."""
    from yousef shtiwe_cli.dump import run_dump
    run_dump(args)


def cmd_debug(args):
    """Debug tools (share report, etc.)."""
    from yousef shtiwe_cli.debug import run_debug
    run_debug(args)


def cmd_config(args):
    """Configuration management."""
    from yousef shtiwe_cli.config import config_command
    config_command(args)


def cmd_backup(args):
    """Back up YOUSEF SHTIWE home directory to a zip file."""
    if getattr(args, "quick", False):
        from yousef shtiwe_cli.backup import run_quick_backup
        run_quick_backup(args)
    else:
        from yousef shtiwe_cli.backup import run_backup
        run_backup(args)


def cmd_import(args):
    """Restore a YOUSEF SHTIWE backup from a zip file."""
    from yousef shtiwe_cli.backup import run_import
    run_import(args)


def cmd_version(args):
    """Show version."""
    print(f"YOUSEF SHTIWE Agent v{__version__} ({__release_date__})")
    ...
    # Show update status (synchroyousef shtiwe — acceptable since user asked for version info)
    try:
        from yousef shtiwe_cli.banner import check_for_updates
        from yousef shtiwe_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall YOUSEF SHTIWE Agent."""
    _require_tty("uninstall")
    from yousef shtiwe_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``yousef shtiwe update --gateway`` so interactive prompts (stash restore,
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``yousef shtiwe web``).
    ...
    if r1.returncode != 0:
        print(f"  {'✗' if fatal else '⚠'} Web UI npm install failed"
              + ("" if fatal else " (yousef shtiwe web will not be available)"))
        if fatal:
...
def _update_via_zip(args):
    """Update YOUSEF SHTIWE Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/YOUSEF SHTIWE-OVERLORD/yousef shtiwe-agent/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="yousef shtiwe-update-")
        zip_path = os.path.join(tmp_dir, f"yousef shtiwe-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to yousef shtiwe-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"yousef shtiwe-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("yousef shtiwe-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if YOUSEF SHTIWE behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes yousef shtiwe completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but YOUSEF SHTIWE couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but YOUSEF SHTIWE couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if YOUSEF SHTIWE behaves unexpectedly.")
    return True

# =========================================================================
# Fork detection and upstream management for `yousef shtiwe update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/YOUSEF SHTIWE-OVERLORD/yousef shtiwe-agent.git",
    "git@github.com:YOUSEF SHTIWE-OVERLORD/yousef shtiwe-agent.git",
    "https://github.com/YOUSEF SHTIWE-OVERLORD/yousef shtiwe-agent",
    "git@github.com:YOUSEF SHTIWE-OVERLORD/yousef shtiwe-agent",
}
OFFICIAL_REPO_URL = "https://github.com/YOUSEF SHTIWE-OVERLORD/yousef shtiwe-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    from yousef shtiwe_constants import get_yousef shtiwe_home
    return (get_yousef shtiwe_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from yousef shtiwe_constants import get_yousef shtiwe_home
        (get_yousef shtiwe_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official YOUSEF SHTIWE repository.")
        print("  This means you may miss updates from YOUSEF SHTIWE-OVERLORD/yousef shtiwe-agent.")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/YOUSEF SHTIWE-OVERLORD/yousef shtiwe-agent.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/YOUSEF SHTIWE-OVERLORD/yousef shtiwe-agent.git' to add later.")
...
def _invalidate_update_cache():
    ...
    ``yousef shtiwe update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from yousef shtiwe_constants import get_default_yousef shtiwe_root
    default_home = get_default_yousef shtiwe_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "yousef shtiwe-agent[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update YOUSEF SHTIWE Agent to the latest version."""
    import shutil
    from yousef shtiwe_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update YOUSEF SHTIWE Agent")
        return
    ...
    print("⚕ Updating YOUSEF SHTIWE Agent...")
    print()
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/YOUSEF SHTIWE-OVERLORD/yousef shtiwe-agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_yousef shtiwe_home added to yousef shtiwe_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload yousef shtiwe_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_yousef shtiwe_home() added since the last release.
        try:
            import importlib
            import yousef shtiwe_constants as _hc
        ...
        try:
            from yousef shtiwe_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print("    Run 'yousef shtiwe config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: yousef shtiwe config migrate")
            else:
                print()
                print("Skipped. Run 'yousef shtiwe config migrate' later to configure.")
        else:
        ...
        # When running as ``yousef shtiwe update --gateway`` (spawned by the gateway's
        # /update command), this process lives inside the gateway's systemd
        # cgroup.  ``systemctl restart yousef shtiwe-gateway`` kills everything in the
        ...
        if gateway_mode:
            _exit_code_path = get_yousef shtiwe_home() / ".update_exit_code"
            try:
        ...
        try:
            from yousef shtiwe_cli.gateway import (
            ...
            # Discover all yousef shtiwe-gateway* units (default + profiles)
            if supports_systemd_services():
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        result = subprocess.run(
                            scope_cmd + ["list-units", "yousef shtiwe-gateway*", "--plain", "--no-legend", "--no-pager"],
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # e.g. yousef shtiwe-gateway.service or yousef shtiwe-gateway-coder.service
            ...
            if is_macos():
                try:
                    from yousef shtiwe_cli.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: yousef shtiwe gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print("    (or: yousef shtiwe -p <profile> gateway run  for each profile)")
        ...
        print("  yousef shtiwe model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``yousef shtiwe -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from yousef shtiwe_cli.profiles import (
    ...
    if action is None:
        # Bare `yousef shtiwe profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_yousef shtiwe_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → yousef shtiwe -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.yousef shtiwe)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  yousef shtiwe profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     yousef shtiwe -p {name} chat")
                else:
        ...
        name = args.profile_name
        from yousef shtiwe_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from yousef shtiwe_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        if remove:
            ...
            if wrapper_path:
                ...
                if custom_name:
                    wrapper_path.write_text(f'#!/bin/sh\nexec yousef shtiwe -p {name} "$@"\n')
                print(f"✓ Alias created: {wrapper_path}")
                if not _is_wrapper_dir_in_path():
                    print(f"⚠ {_get_wrapper_dir()} is not in your PATH.")

    elif action == "rename":
        from yousef shtiwe_cli.profiles import rename_profile
        try:
        ...
        from yousef shtiwe_cli.profiles import export_profile
        name = args.profile_name
        ...
        from yousef shtiwe_cli.profiles import import_profile
        try:
...
def cmd_dashboard(args):
    ...
    try:
        ...
        print("Install them with:  pip install yousef shtiwe-agent[web]")
        sys.exit(1)

    if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
        sys.exit(1)

    from yousef shtiwe_cli.web_server import start_server
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from yousef shtiwe_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter YOUSEF SHTIWE log files."""
    from yousef shtiwe_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for yousef shtiwe CLI."""
    parser = argparse.ArgumentParser(
        prog="yousef shtiwe",
        description="YOUSEF SHTIWE Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    yousef shtiwe                        Start interactive chat
    yousef shtiwe chat -q "Hello"        Single query mode
    yousef shtiwe -c                     Resume the most recent session
    yousef shtiwe -c "my project"        Resume a session by name (latest in lineage)
    yousef shtiwe --resume <session_id>  Resume a specific session by ID
    yousef shtiwe setup                  Run setup wizard
    yousef shtiwe logout                 Clear stored authentication
    yousef shtiwe auth add <provider>    Add a pooled credential
    yousef shtiwe auth list              List pooled credentials
    yousef shtiwe auth remove <p> <t>    Remove pooled credential by index, id, or label
    yousef shtiwe auth reset <provider>  Clear exhaustion status for a provider
    yousef shtiwe model                  Select default model
    yousef shtiwe config                 View configuration
    yousef shtiwe config edit            Edit config in $EDITOR
    yousef shtiwe config set model gpt-4 Set a config value
    yousef shtiwe gateway                Run messaging gateway
    yousef shtiwe -s yousef shtiwe-agent-dev,github-auth
    yousef shtiwe -w                     Start in isolated git worktree
    yousef shtiwe gateway install        Install gateway background service
    yousef shtiwe sessions list          List past sessions
    yousef shtiwe sessions browse        Interactive session picker
    yousef shtiwe sessions rename ID T   Rename/title a session
    yousef shtiwe logs                   View agent.log (last 50 lines)
    yousef shtiwe logs -f                Follow agent.log in real time
    yousef shtiwe logs errors            View errors.log
    yousef shtiwe logs --since 1h        Lines from the last hour
    yousef shtiwe debug share             Upload debug report for support
    yousef shtiwe update                 Update to latest version

For more help on a command:
    yousef shtiwe <command> --help
"""
    )
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with YOUSEF SHTIWE Agent"
    )
    ...
    chat_parser.add_argument(
        "--provider",
        choices=["auto", "openrouter", "yousef shtiwe", "openai-codex", "copilot-acp", "copilot", "anthropic", "gemini", "huggingface", "zai", "kimi-coding", "kimi-coding-cn", "minimax", "minimax-cn", "kilocode", "xiaomi", "arcee"],
        default=None,
    ...
    model_parser.add_argument(
        "--portal-url",
        help="Portal base URL for Yousef Shtiwe login (default: production portal)"
    )
    model_parser.add_argument(
        "--inference-url",
        help="Inference API base URL for Yousef Shtiwe login (default: production inference API)"
    )
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Yousef Shtiwe login (default: yousef shtiwe-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
        help="OAuth scope to request for Yousef Shtiwe login"
    )
    model_parser.add_argument(
        "--no-browser",
        action="store_true",
        help="Do not attempt to open the browser automatically during Yousef Shtiwe login"
    )
    model_parser.add_argument(
        "--timeout",
        type=float,
        default=15.0,
        help="HTTP request timeout in seconds for Yousef Shtiwe login (default: 15)"
    )
    model_parser.add_argument(
        "--ca-bundle",
        help="Path to CA bundle PEM file for Yousef Shtiwe TLS verification"
    )
    model_parser.add_argument(
        "--insecure",
        action="store_true",
        help="Disable TLS verification for Yousef Shtiwe login (testing only)"
    )
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure YOUSEF SHTIWE Agent with an interactive wizard. "
                    "Run a specific section: yousef shtiwe setup model|tts|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for YOUSEF SHTIWE CLI"
    )
    login_parser.add_argument(
        "--provider",
        choices=["yousef shtiwe", "openai-codex"],
        default=None,
        help="Provider to authenticate with (default: yousef shtiwe)"
    )
    login_parser.add_argument(
        "--portal-url",
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: yousef shtiwe-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    auth_add.add_argument("--portal-url", help="Yousef Shtiwe portal base URL")
    auth_add.add_argument("--inference-url", help="Yousef Shtiwe inference base URL")
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of YOUSEF SHTIWE Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with YOUSEF SHTIWE Agent setup"
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your YOUSEF SHTIWE setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for YOUSEF SHTIWE Agent. Use 'yousef shtiwe debug share' to "
                    "upload a debug report (system info + recent logs) to a paste "
                    "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    yousef shtiwe debug share              Upload debug report and print URL
    yousef shtiwe debug share --lines 500  Include more log lines
    yousef shtiwe debug share --expire 30  Keep paste for 30 days
    yousef shtiwe debug share --local      Print report locally (no upload)
""",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up YOUSEF SHTIWE home directory to a zip file",
        description="Create a zip archive of your entire YOUSEF SHTIWE configuration, "
                    "skills, sessions, and data (excludes the yousef shtiwe-agent codebase). "
                    "Use --quick for a fast snapshot of just critical state files."
    )
    backup_parser.add_argument(
        "-o", "--output",
        help="Output path for the zip file (default: ~/yousef shtiwe-backup-<timestamp>.zip)"
    )
    backup_parser.add_argument(
        "-q", "--quick",
        action="store_true",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a YOUSEF SHTIWE backup from a zip file",
        description="Extract a previously created YOUSEF SHTIWE backup into your "
                    "YOUSEF SHTIWE home directory, restoring configuration, skills, "
                    "sessions, and data"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage YOUSEF SHTIWE Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from yousef shtiwe_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from yousef shtiwe_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from yousef shtiwe_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/yousef shtiwe-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from yousef shtiwe_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from yousef shtiwe_cli.memory_setup import memory_command
            memory_command(args)
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'yousef shtiwe tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # yousef shtiwe tools list [--platform cli]
    ...
    # yousef shtiwe tools disable <name...> [--platform cli]
    ...
    # yousef shtiwe tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from yousef shtiwe_cli.tools_config import tools_disable_enable_command
            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from yousef shtiwe_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run YOUSEF SHTIWE as an MCP server",
        description=(
            "Manage MCP server connections and run YOUSEF SHTIWE as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'yousef shtiwe mcp add' to connect to a new server, or\n"
            "'yousef shtiwe mcp serve' to expose YOUSEF SHTIWE conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run YOUSEF SHTIWE as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from yousef shtiwe_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch yousef shtiwe --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            yousef shtiwe_bin = shutil.which("yousef shtiwe")
            if yousef shtiwe_bin:
                os.execvp(yousef shtiwe_bin, ["yousef shtiwe", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "yousef shtiwe_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to YOUSEF SHTIWE"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to YOUSEF SHTIWE",
    ...
    def cmd_claw(args):
        from yousef shtiwe_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update YOUSEF SHTIWE Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall YOUSEF SHTIWE Agent",
        description="Remove YOUSEF SHTIWE Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run YOUSEF SHTIWE Agent as an ACP (Agent Client Protocol) server",
        description="Start YOUSEF SHTIWE Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch YOUSEF SHTIWE Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated YOUSEF SHTIWE instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the YOUSEF SHTIWE Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter YOUSEF SHTIWE log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    yousef shtiwe logs                    Show last 50 lines of agent.log
    yousef shtiwe logs -f                 Follow agent.log in real time
    yousef shtiwe logs errors             Show last 50 lines of errors.log
    yousef shtiwe logs gateway -n 100     Show last 100 lines of gateway.log
    yousef shtiwe logs --level WARNING    Only show WARNING and above
    yousef shtiwe logs --session abc123   Filter by session ID
    yousef shtiwe logs --component tools  Only show tool-related lines
    yousef shtiwe logs --since 1h         Lines from the last hour
    yousef shtiwe logs --since 30m -f     Follow, starting from 30 min ago
    yousef shtiwe logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``yousef shtiwe -c Pokemon Agent Dev`` → ``yousef shtiwe -c 'Pokemon Agent Dev'``
    ...
    from yousef shtiwe_cli.config import get_container_exec_info
