"""
Gateway subcommand for yousef shtiwe CLI.

Handles: yousef shtiwe gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_yousef shtiwe_home is imported lazily at call sites to avoid ImportError
# when yousef shtiwe_constants is cached from a pre-update version during `yousef shtiwe update`.
from yousef shtiwe_cli.setup import (
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "yousef shtiwe-gateway*",
...
def _request_gateway_self_restart(pid: int) -> bool:
    """Ask a running gateway ancestor to restart itself asynchroyousef shtiwely."""
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``yousef shtiwe update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            YOUSEF SHTIWE profile are returned.
    """
    _exclude = exclude_pids or set()
    pids = [pid for pid in _get_service_pids() if pid not in _exclude]
    patterns = [
        "yousef shtiwe_cli.main gateway",
        "yousef shtiwe_cli.main --profile",
        "yousef shtiwe_cli.main -p",
        "yousef shtiwe_cli/main.py gateway",
        "yousef shtiwe_cli/main.py --profile",
        "yousef shtiwe_cli/main.py -p",
        "yousef shtiwe gateway",
        "gateway/run.py",
    ]
    current_home = str(get_yousef shtiwe_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"YOUSEF SHTIWE_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "YOUSEF SHTIWE_HOME=" in command and f"YOUSEF SHTIWE_HOME={current_home}" not in command:
            return False
        return True
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (YOUSEF SHTIWE_HOME-scoped).
...
from yousef shtiwe_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "yousef shtiwe-gateway"
SERVICE_DESCRIPTION = "YOUSEF SHTIWE Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current YOUSEF SHTIWE_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (YOUSEF SHTIWE_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from yousef shtiwe_constants import get_default_yousef shtiwe_root
    home = get_yousef shtiwe_home().resolve()
    default = get_default_yousef shtiwe_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary YOUSEF SHTIWE_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(yousef shtiwe_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when YOUSEF SHTIWE_HOME is a named profile.

    For ``~/.yousef shtiwe/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        yousef shtiwe_home: Optional explicit YOUSEF SHTIWE_HOME path. Defaults to the current
            ``get_yousef shtiwe_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from yousef shtiwe_constants import get_default_yousef shtiwe_root
    home = Path(yousef shtiwe_home or str(get_yousef shtiwe_home())).resolve()
    default = get_default_yousef shtiwe_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this YOUSEF SHTIWE_HOME.

    Default ``~/.yousef shtiwe`` returns ``yousef shtiwe-gateway`` (backward compatible).
    Profile ``~/.yousef shtiwe/profiles/coder`` returns ``yousef shtiwe-gateway-coder``.
    Any other YOUSEF SHTIWE_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    yousef shtiwe gateway uninstall")
    print_info("    sudo yousef shtiwe gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so YOUSEF SHTIWE can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo yousef shtiwe gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo yousef shtiwe gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo yousef shtiwe gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode YOUSEF SHTIWE often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.yousef shtiwe`` → ``ai.yousef shtiwe.gateway.plist`` (backward compatible).
    Profile ``~/.yousef shtiwe/profiles/coder`` → ``ai.yousef shtiwe.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.yousef shtiwe.gateway-{suffix}" if suffix else "ai.yousef shtiwe.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.yousef shtiwe/yousef shtiwe-agent  -> /home/alice/.yousef shtiwe/yousef shtiwe-agent
      /opt/yousef shtiwe                 -> /opt/yousef shtiwe  (kept as-is)
...
def _yousef shtiwe_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current YOUSEF SHTIWE_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_yousef shtiwe_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.yousef shtiwe                    → /home/alice/.yousef shtiwe
      /root/.yousef shtiwe/profiles/coder     → /home/alice/.yousef shtiwe/profiles/coder
      /opt/custom-yousef shtiwe               → /opt/custom-yousef shtiwe  (kept as-is)
    """
    current_yousef shtiwe = get_yousef shtiwe_home().resolve()
    current_default = (Path.home() / ".yousef shtiwe").resolve()
    target_default = Path(target_home_dir) / ".yousef shtiwe"

    # Default ~/.yousef shtiwe → remap to target user's default
    if current_yousef shtiwe == current_default:
        return str(target_default)

    # Profile or subdir of ~/.yousef shtiwe → preserve the relative structure
    try:
        relative = current_yousef shtiwe.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.yousef shtiwe) — keep as-is
        return str(current_yousef shtiwe)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        yousef shtiwe_home = _yousef shtiwe_home_for_target_user(home_dir)
        profile_arg = _profile_arg(yousef shtiwe_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m yousef shtiwe_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="YOUSEF SHTIWE_HOME={yousef shtiwe_home}"
Restart=on-failure
RestartSec=30
...
"""
    ...
    yousef shtiwe_home = str(get_yousef shtiwe_home().resolve())
    profile_arg = _profile_arg(yousef shtiwe_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current YOUSEF SHTIWE install")
    return True
...
def _get_restart_drain_timeout() -> float:
    ...
    raw = os.getenv("YOUSEF SHTIWE_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}yousef shtiwe gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}yousef shtiwe gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}yousef shtiwe gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}yousef shtiwe gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}yousef shtiwe gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.yousef shtiwe.gateway-{suffix}" if suffix else "ai.yousef shtiwe.gateway"
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>yousef shtiwe_cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>YOUSEF SHTIWE_HOME</key>
        <string>{yousef shtiwe_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current YOUSEF SHTIWE install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  yousef shtiwe gateway status             # Check status")
...
def launchd_stop():
    ...
    # `yousef shtiwe gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def _wait_for_gateway_exit(timeout: float = 10.0, force_after: float | None = 5.0) -> bool:
    ...
    YOUSEF SHTIWE_HOME directories.
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current YOUSEF SHTIWE install")
    else:
        print("⚠ Service definition is stale relative to the current YOUSEF SHTIWE install")
        print("  Run: yousef shtiwe gateway start")
    ...
    if deep:
        log_file = get_yousef shtiwe_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ YOUSEF SHTIWE Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and YOUSEF SHTIWE will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @yousef shtiwe:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. yousef shtiwe) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where YOUSEF SHTIWE delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your YOUSEF SHTIWE agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address YOUSEF SHTIWE will use (e.g., yousef shtiwe@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. YOUSEF SHTIWE connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: yousef shtiwe pairing generate bluebubbles",
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_yousef shtiwe_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'yousef shtiwe pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: yousef shtiwe pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'yousef shtiwe gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from yousef shtiwe_cli.main import cmd_whatsapp
    import argparse
...
def _setup_weixin():
    ...
    print()
    print_info("  1. YOUSEF SHTIWE will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. YOUSEF SHTIWE will store the returned account_id/token in ~/.yousef shtiwe/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `yousef shtiwe gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_yousef shtiwe_home())))
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `yousef shtiwe pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `yousef shtiwe pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"YOUSEF SHTIWEAgent\"")
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: yousef shtiwe gateway")
                except subprocess.CalledProcessError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    ...
                    print_info("  Or run in foreground:  yousef shtiwe gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: yousef shtiwe gateway run")
                print_info("  For persistence:   tmux new -s yousef shtiwe 'yousef shtiwe gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: yousef shtiwe gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup yousef shtiwe gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: yousef shtiwe gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'yousef shtiwe gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: yousef shtiwe gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: yousef shtiwe gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s yousef shtiwe 'yousef shtiwe gateway run'")
                print()
            ...
            print()
            print("  yousef shtiwe gateway run                              # direct foreground")
            print("  tmux new -s yousef shtiwe 'yousef shtiwe gateway run'         # persistent via tmux")
            print("  nohup yousef shtiwe gateway run > ~/.yousef shtiwe/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: yousef shtiwe gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: yousef shtiwe gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: yousef shtiwe gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: yousef shtiwe gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if supports_systemd_services():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print("  Then restart the gateway:")
                    print("    yousef shtiwe gateway restart")
                    return
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: yousef shtiwe gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  yousef shtiwe gateway install")
                    print("  sudo yousef shtiwe gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  yousef shtiwe gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup yousef shtiwe gateway run > ~/.yousef shtiwe/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s yousef shtiwe 'yousef shtiwe gateway run'         # persistent via tmux")
                    print("  nohup yousef shtiwe gateway run > ~/.yousef shtiwe/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  yousef shtiwe gateway install  # Install as user service")
                    print("  sudo yousef shtiwe gateway install --system  # Install as boot-time system service")
