"""yousef shtiwe webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.yousef shtiwe/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _yousef shtiwe_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
    _dhh = display_yousef shtiwe_home()
    return f"""
  ...
  Then start the gateway: yousef shtiwe gateway run
"""
...
def webhook_command(args):
    """Entry point for 'yousef shtiwe webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: yousef shtiwe webhook {subscribe|list|remove|test}")
        print("Run 'yousef shtiwe webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (yousef shtiwe gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: yousef shtiwe webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from yousef shtiwe webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (yousef shtiwe gateway run)")
