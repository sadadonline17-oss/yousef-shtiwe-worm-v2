...
- **Aggregators** (OpenRouter, Yousef Shtiwe, AI Gateway, Kilo Code) need
...
# slug used by aggregator APIs (OpenRouter, Yousef Shtiwe, etc.).
#
...
def _normalize_provider_alias(provider_name: str) -> str:
    """Resolve provider aliases to YOUSEF SHTIWE' canonical ids."""
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        ...
        target_provider: The canonical YOUSEF SHTIWE provider id, e.g.
            ``"openrouter"``, ``"anthropic"``, ``"copilot"``,
            ``"deepseek"``, ``"custom"``.  Should already be normalised
            via ``yousef shtiwe_cli.models.normalize_provider()``.
