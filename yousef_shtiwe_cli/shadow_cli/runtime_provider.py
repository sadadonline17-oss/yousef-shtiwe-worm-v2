...
from yousef shtiwe_cli import auth as auth_mod
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from yousef shtiwe_cli.models import copilot_model_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if configured_mode and _provider_supports_explicit_api_mode(provider, configured_provider):
            ...
            from yousef shtiwe_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = os.getenv("YOUSEF SHTIWE_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if isinstance(custom_providers, dict):
        logger.warning(
            ...
            "Run 'yousef shtiwe doctor' for details."
        )
        return None
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "yousef shtiwe":
        state = auth_mod.get_provider_auth_state("yousef shtiwe") or {}
        base_url = (
            explicit_base_url
            or str(state.get("inference_base_url") or auth_mod.DEFAULT_Yousef Shtiwe_INFERENCE_URL).strip().rstrip("/")
        )
...
def resolve_runtime_provider(
    *,
    ...
    if pool and pool.has_credentials():
        ...
        # For Yousef Shtiwe, the pool entry's runtime_api_key is the agent_key — a
        # short-lived inference credential (~30 min TTL).  The pool doesn't
        # refresh it during selection (that would trigger network calls in
        # non-runtime contexts like `yousef shtiwe auth list`).  If the key is
        # expired, clear pool_api_key so we fall through to
        # resolve_yousef shtiwe_runtime_credentials() which handles refresh + mint.
        if provider == "yousef shtiwe" and entry is not None and pool_api_key:
            min_ttl = max(60, int(os.getenv("YOUSEF SHTIWE_Yousef Shtiwe_MIN_KEY_TTL_SECONDS", "1800")))
            yousef shtiwe_state = {
                "agent_key": getattr(entry, "agent_key", None),
                "agent_key_expires_at": getattr(entry, "agent_key_expires_at", None),
            }
            if not _agent_key_is_usable(yousef shtiwe_state, min_ttl):
                logger.debug("Yousef Shtiwe pool entry agent_key expired/missing, falling through to runtime resolution")
                pool_api_key = ""
    ...
    if provider == "yousef shtiwe":
        try:
            ...
            if requested_provider != "auto":
                raise
            # Auto-detected Yousef Shtiwe but credentials are stale/revoked —
            # fall through to env-var providers (e.g. OpenRouter).
            logger.info("Auto-detected Yousef Shtiwe provider but credentials failed; "
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "yousef shtiwe-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
