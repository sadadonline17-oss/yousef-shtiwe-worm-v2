"""
Configuration management for YOUSEF SHTIWE Agent.

Config files are stored in ~/.yousef shtiwe/ for easy access:
- ~/.yousef shtiwe/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.yousef shtiwe/.env         - API keys and secrets

This module provides:
- yousef shtiwe config          - Show current configuration
- yousef shtiwe config edit     - Open config in editor
- yousef shtiwe config set      - Set a specific value
- yousef shtiwe config wizard   - Re-run setup wizard
"""
...
from tools.tool_backend_helpers import managed_yousef shtiwe_tools_enabled as _managed_yousef shtiwe_tools_enabled
...
from yousef shtiwe_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("YOUSEF SHTIWE_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_yousef shtiwe_home() / ".managed"
...
def is_managed() -> bool:
    """Check if YOUSEF SHTIWE is running in package-manager-managed mode.

    Two signals: the YOUSEF SHTIWE_MANAGED env var (set by the systemd service),
    or a .managed marker file in YOUSEF SHTIWE_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade yousef shtiwe-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "yousef shtiwe update"


def format_managed_message(action: str = "modify this YOUSEF SHTIWE installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("YOUSEF SHTIWE_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this YOUSEF SHTIWE installation is managed by NixOS "
            f"(YOUSEF SHTIWE_MANAGED={env_hint}).\n"
            "Edit services.yousef shtiwe-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this YOUSEF SHTIWE installation is managed by Homebrew "
            f"(YOUSEF SHTIWE_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade yousef shtiwe-agent"
        )

    return (
        f"Cannot {action}: this YOUSEF SHTIWE installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall YOUSEF SHTIWE."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from YOUSEF SHTIWE_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, yousef shtiwe_bin
    or None if container mode is not active, we're already inside the
    container, or YOUSEF SHTIWE_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("YOUSEF SHTIWE_DEV") == "1":
        return None
    ...
    container_mode_file = get_yousef shtiwe_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "yousef shtiwe-agent")
    exec_user = info.get("exec_user", "yousef shtiwe")
    yousef shtiwe_bin = info.get("yousef shtiwe_bin", "/data/current-package/bin/yousef shtiwe")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "yousef shtiwe_bin": yousef shtiwe_bin,
    }
...
# Re-export from yousef shtiwe_constants — canonical definition lives there.
from yousef shtiwe_constants import get_yousef shtiwe_home  # noqa: F811,E402

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_yousef shtiwe_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_yousef shtiwe_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the yousef shtiwe group can
    share state with the gateway service.

    The mode can be overridden via the YOUSEF SHTIWE_HOME_MODE environment variable
    (e.g. YOUSEF SHTIWE_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse YOUSEF SHTIWE_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("YOUSEF SHTIWE_HOME_MODE", "").strip()
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into YOUSEF SHTIWE_HOME if the user doesn't have one yet."""
...
def ensure_yousef shtiwe_home():
    """Ensure ~/.yousef shtiwe directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_yousef shtiwe_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_yousef shtiwe_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"YOUSEF SHTIWE_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when YOUSEF SHTIWE
    ...
    "browser": {
        ...
        "camofox": {
            # When true, YOUSEF SHTIWE sends a stable profile-scoped userId to Camofox
    ...
    "auxiliary": {
        "vision": {
            "provider": "auto",    # auto | openrouter | yousef shtiwe | codex | custom
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.yousef shtiwe/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # configured providers (OpenRouter, Yousef Shtiwe, Z.ai, Kimi, etc.) are supported.
    "delegation": {
    ...
    # always goes to ~/.yousef shtiwe/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for yousef shtiwe-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *YOUSEF SHTIWE Agent*" header.
    ...
    # Logging — controls file logging to ~/.yousef shtiwe/logs/.
...
# selection step (Yousef Shtiwe Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "Yousef Shtiwe_BASE_URL": {
        "description": "Yousef Shtiwe Portal base URL override",
        "prompt": "Yousef Shtiwe Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for Yousef Shtiwe Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Yousef Shtiwe Subscribers only, used to derive vendor hosts, e.g. yousef shtiwe-overlord.com -> firecrawl-gateway.yousef shtiwe-overlord.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for Yousef Shtiwe Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Yousef Shtiwe Subscriber access token for tool-gateway requests (optional; otherwise read from the YOUSEF SHTIWE auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @yousef shtiwe:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. YOUSEF SHTIWE_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'yousef shtiwe-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote YOUSEF SHTIWE API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote YOUSEF SHTIWE API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote YOUSEF SHTIWE API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # YOUSEF SHTIWE_TOOL_PROGRESS and YOUSEF SHTIWE_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "YOUSEF SHTIWE_TOOL_PROGRESS": {
    ...
    "YOUSEF SHTIWE_TOOL_PROGRESS_MODE": {
        "description": "(deprecated) Use display.tool_progress in config.yaml instead",
    ...
    "YOUSEF SHTIWE_PREFILL_MESSAGES_FILE": {
    ...
    "YOUSEF SHTIWE_EPHEMERAL_SYSTEM_PROMPT": {
...
if not _managed_yousef shtiwe_tools_enabled():
    for _hidden_var in (
        "FIRECRAWL_GATEWAY_URL",
        "TOOL_GATEWAY_DOMAIN",
        "TOOL_GATEWAY_SCHEME",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.yousef shtiwe.config`` entries, then checks
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'yousef shtiwe setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — YOUSEF SHTIWE won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'yousef shtiwe doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("YOUSEF SHTIWE_TOOL_PROGRESS")
            old_mode = get_env_value("YOUSEF SHTIWE_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from YOUSEF SHTIWE_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from YOUSEF SHTIWE_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("YOUSEF SHTIWE_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from YOUSEF SHTIWE_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: yousef shtiwe config set <key> <value>")
    ...
    # metadata.yousef shtiwe.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.yousef shtiwe/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.yousef shtiwe/config.yaml."""
    import copy
...
#   openai-codex (OAuth — yousef shtiwe auth) — OpenAI Codex
#   yousef shtiwe         (OAuth — yousef shtiwe auth) — Yousef Shtiwe Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.yousef shtiwe/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.yousef shtiwe/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real YOUSEF SHTIWE env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.yousef shtiwe/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.yousef shtiwe/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.yousef shtiwe/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.yousef shtiwe/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to YOUSEF SHTIWE — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known YOUSEF SHTIWE vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.yousef shtiwe/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ YOUSEF SHTIWE Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  yousef shtiwe config edit     # Edit config file", Colors.DIM))
    print(color("  yousef shtiwe config set <key> <value>", Colors.DIM))
    print(color("  yousef shtiwe setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: yousef shtiwe config set <key> <value>")
            print()
            print("Examples:")
            print("  yousef shtiwe config set model anthropic/claude-sonnet-4")
            print("  yousef shtiwe config set terminal.backend docker")
            print("  yousef shtiwe config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'yousef shtiwe config migrate' to add them")
        ...
        print("  yousef shtiwe config           Show current configuration")
        print("  yousef shtiwe config edit      Open config in editor")
        print("  yousef shtiwe config set <key> <value>   Set a config value")
        print("  yousef shtiwe config check     Check for missing/outdated config")
        print("  yousef shtiwe config migrate   Update config with new options")
        print("  yousef shtiwe config path      Show config file path")
        print("  yousef shtiwe config env-path  Show .env file path")
        sys.exit(1)
