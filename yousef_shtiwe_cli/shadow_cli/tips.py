...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or yousef shtiwe -c.",
    ...
    "Ctrl+Z suspends YOUSEF SHTIWE to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "yousef shtiwe -c resumes your most recent CLI session. yousef shtiwe -c \"project name\" resumes by title.",
    "yousef shtiwe -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "yousef shtiwe -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "yousef shtiwe chat -t web,terminal enables only specific toolsets for a focused session.",
    "yousef shtiwe chat -s github-pr-workflow preloads a skill at launch.",
    "yousef shtiwe chat -q \"query\" runs a single non-interactive query and exits.",
    "yousef shtiwe chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "yousef shtiwe chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "yousef shtiwe --yolo bypasses all dangerous command approval prompts for the entire session.",
    "yousef shtiwe chat --source telegram tags the session for filtering in yousef shtiwe sessions list.",
    "yousef shtiwe -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "yousef shtiwe doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "yousef shtiwe dump outputs a compact setup summary — great for bug reports.",
    "yousef shtiwe config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "yousef shtiwe config edit opens config.yaml in your default editor.",
    "yousef shtiwe config check scans for missing or stale configuration options.",
    "yousef shtiwe sessions browse opens an interactive session picker with search.",
    "yousef shtiwe sessions stats shows session counts by platform and database size.",
    "yousef shtiwe sessions prune --older-than 30 cleans up old sessions.",
    "yousef shtiwe skills search react --source skills-sh searches the skills.sh public directory.",
    "yousef shtiwe skills check scans installed hub skills for upstream updates.",
    "yousef shtiwe skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "yousef shtiwe skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "yousef shtiwe mcp add github --command npx adds MCP servers from the command line.",
    "yousef shtiwe mcp serve runs YOUSEF SHTIWE itself as an MCP server for other agents.",
    "yousef shtiwe auth add lets you add multiple API keys for credential pool rotation.",
    "yousef shtiwe completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "yousef shtiwe logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "yousef shtiwe backup creates a zip backup of your entire YOUSEF SHTIWE home directory.",
    "yousef shtiwe profile create coder creates an isolated profile that becomes its own command.",
    "yousef shtiwe profile create work --clone copies your current config and keys to a new profile.",
    "yousef shtiwe update syncs new bundled skills to ALL profiles automatically.",
    "yousef shtiwe gateway install sets up YOUSEF SHTIWE as a system service (systemd/launchd).",
    "yousef shtiwe memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "yousef shtiwe webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as yousef shtiwe -w).",
    ...
    "YOUSEF SHTIWE_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call YOUSEF SHTIWE tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'yousef shtiwe profile create coder' creates the 'coder' command.",
    "yousef shtiwe profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, YOUSEF SHTIWE prints a resume command with session ID and stats.",
    "yousef shtiwe sessions export backup.jsonl exports all sessions for backup or analysis.",
    "yousef shtiwe -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "yousef shtiwe skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .yousef shtiwe/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: yousef shtiwe cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "YOUSEF SHTIWE runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "yousef shtiwe gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to YOUSEF SHTIWE via a local macOS server.",
    ...
    "Context files (.yousef shtiwe.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.yousef shtiwe/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "YOUSEF SHTIWE loads project context from .yousef shtiwe.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — yousef shtiwe tools can toggle them per platform.",
    ...
    "Checkpoints use yousef shtiwe repos in ~/.yousef shtiwe/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "yousef shtiwe chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "yousef shtiwe acp runs YOUSEF SHTIWE as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "YOUSEF SHTIWE_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "yousef shtiwe login supports OAuth-based auth for Yousef Shtiwe and OpenAI Codex providers.",
    ...
    "yousef shtiwe status --deep runs deeper diagnostic checks across all components.",

    # --- Hidden Gems & Power-User Tricks ---
    "BOOT.md at ~/.yousef shtiwe/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.yousef shtiwe/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make YOUSEF SHTIWE your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.yousef shtiwe/SOUL.md to customize.",
    ...
    "Before a gateway session reset, YOUSEF SHTIWE auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.yousef shtiwe/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at YOUSEF SHTIWE_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "YOUSEF SHTIWE_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in YOUSEF SHTIWE_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.yousef shtiwe/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'yousef shtiwe profile create ls' would be rejected.",
    "yousef shtiwe profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "yousef shtiwe claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "YOUSEF SHTIWE_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via yousef shtiwe tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.yousef shtiwe/.env.",
    "Skills from trusted repos (YOUSEF SHTIWE-OVERLORD) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.yousef shtiwe/skills/.hub/quarantine/ holds skills pending security review.",
]
