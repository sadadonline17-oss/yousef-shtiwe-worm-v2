"""
Dump command for yousef shtiwe CLI.

Outputs a compact, plain-text summary of the user's YOUSEF SHTIWE setup
...
from yousef shtiwe_cli.config import get_yousef shtiwe_home, get_env_path, get_project_root, load_config
...
def _count_skills(yousef shtiwe_home: Path) -> int:
...
def _cron_summary(yousef shtiwe_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = yousef shtiwe_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines = []
    lines.append("--- yousef shtiwe dump ---")
    ...
    lines.append(f"yousef shtiwe_home:      {display_yousef shtiwe_home()}")
    ...
    api_keys = [
        ...
        ("Yousef Shtiwe_API_KEY", "yousef shtiwe"),
        ("GLM_API_KEY", "glm/zai"),
        ("ZAI_API_KEY", "zai"),
        ("KIMI_API_KEY", "kimi"),
    ...
    toolsets = config.get("toolsets", ["yousef shtiwe-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(yousef shtiwe_home)}")
    lines.append(f"  skills:             {_count_skills(yousef shtiwe_home)}")
