"""
Interactive setup wizard for YOUSEF SHTIWE Agent.
...
Config files are stored in ~/.yousef shtiwe/ for easy access.
"""
...
_DOCS_BASE = "https://yousef shtiwe-agent.yousef shtiwe-overlord.com/docs"
...
# display_yousef shtiwe_home imported lazily at call sites (stale-module safety during yousef shtiwe update)

from yousef shtiwe_cli.colors import Colors, color
...
from yousef shtiwe_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ YOUSEF SHTIWE Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure YOUSEF SHTIWE using environment variables or config commands:")
    print_info("  yousef shtiwe config set model.provider custom")
    print_info("  yousef shtiwe config set model.base_url http://localhost:8080/v1")
    print_info("  yousef shtiwe config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'yousef shtiwe setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0) -> int:
    ...
    from yousef shtiwe_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from yousef shtiwe_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'yousef shtiwe setup')")


def _print_setup_summary(config: dict, yousef shtiwe_home):
    ...
    tool_status = []
    subscription_features = get_yousef shtiwe_subscription_features(config)
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'yousef shtiwe setup' to configure"))
    ...
    if subscription_features.web.managed_by_yousef shtiwe:
        tool_status.append(("Web Search & Extract (Yousef Shtiwe subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_yousef shtiwe:
        tool_status.append(("Browser Automation (Yousef Shtiwe Browser Use)", True, None))
    ...
    if subscription_features.image_gen.managed_by_yousef shtiwe:
        tool_status.append(("Image Generation (Yousef Shtiwe subscription)", True, None))
    ...
    if subscription_features.tts.managed_by_yousef shtiwe:
        tool_status.append(("Text-to-Speech (OpenAI via Yousef Shtiwe subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'yousef shtiwe setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_yousef shtiwe:
        tool_status.append(("Modal Execution (Yousef Shtiwe subscription)", True, None))
    elif config.get("terminal", {}).get("backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'yousef shtiwe setup terminal'"))
    elif managed_yousef shtiwe_tools_enabled() and subscription_features.yousef shtiwe_auth_present:
        tool_status.append(("Modal Execution (optional via Yousef Shtiwe subscription)", True, None))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'yousef shtiwe setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {yousef shtiwe_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('yousef shtiwe setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('yousef shtiwe setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('yousef shtiwe setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('yousef shtiwe setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('yousef shtiwe setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('yousef shtiwe config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('yousef shtiwe config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('yousef shtiwe config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('yousef shtiwe', Colors.GREEN)}              Start chatting")
    print(f"   {color('yousef shtiwe gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('yousef shtiwe doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `yousef shtiwe tools` and `yousef shtiwe setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``yousef shtiwe model``)
    ...
    provider added to ``yousef shtiwe model`` is automatically available here.
    ...
    # Delegate to the shared yousef shtiwe model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: yousef shtiwe model")
    ...
    yousef shtiwe_subscription_selected = selected_provider == "yousef shtiwe"
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from yousef shtiwe_cli.auth_commands import auth_add_command
            ...
            print_info(
                "YOUSEF SHTIWE can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        _prov_names = {
            "yousef shtiwe-api": "Yousef Shtiwe Portal API key",
        ...
        print_info("doesn't currently provide one YOUSEF SHTIWE can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'yousef shtiwe setup' or configure AUXILIARY_VISION_* settings")


    if selected_provider == "yousef shtiwe" and yousef shtiwe_subscription_selected:
        ...
        if "tts" in changed_defaults:
            print_success("TTS provider set to: OpenAI TTS via your Yousef Shtiwe subscription")
        else:
            print_info(f"Keeping your existing TTS provider: {current_tts}")

    save_config(config)

    if not quick and selected_provider != "yousef shtiwe":
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_yousef shtiwe_tools_enabled() and subscription_features.yousef shtiwe_auth_present:
        choices.append("Yousef Shtiwe Subscription (managed OpenAI TTS, billed to your subscription)")
        providers.append("yousef shtiwe-openai")
    choices.extend(
        [
    ...
    selected_via_yousef shtiwe = selected == "yousef shtiwe-openai"
    if selected == "yousef shtiwe-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed Yousef Shtiwe gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.yousef shtiwe/.env."
            )
    ...
    elif selected == "openai" and not selected_via_yousef shtiwe:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'yousef shtiwe setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where YOUSEF SHTIWE runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using YOUSEF SHTIWE via Telegram/Discord, this is where")
        print_info(
        ...
        managed_modal_available = bool(
            ...
            and
            get_yousef shtiwe_subscription_features(config).yousef shtiwe_auth_present
        ...
        if managed_modal_available:
            modal_choices = [
                "Use my Yousef Shtiwe subscription",
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed Yousef Shtiwe gateway and bill to your subscription.")
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("YOUSEF SHTIWE_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `yousef shtiwe setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("YOUSEF SHTIWE_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("YOUSEF SHTIWE_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where YOUSEF SHTIWE delivers cron job results,")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://yousef shtiwe-agent.yousef shtiwe-overlord.com/docs/user-guide/messaging/slack/")
    print()
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where YOUSEF SHTIWE delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where YOUSEF SHTIWE delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'yousef shtiwe whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'yousef shtiwe whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from yousef shtiwe_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from yousef shtiwe_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from yousef shtiwe_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from yousef shtiwe_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from yousef shtiwe_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from yousef shtiwe_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from yousef shtiwe_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from yousef shtiwe_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_qqbot():
    ...
    print_info("Connects YOUSEF SHTIWE to QQ via the Official QQ Bot API (v2).")
...
def _setup_bluebubbles():
    ...
    print_info("Connects YOUSEF SHTIWE to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from yousef shtiwe_cli.gateway import _PLATFORMS
    qq_platform = next((p for p in _PLATFORMS if p["key"] == "qqbot"), None)
    if qq_platform:
        from yousef shtiwe_cli.gateway import _setup_standard_platform
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://yousef shtiwe-agent.yousef shtiwe-overlord.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://yousef shtiwe-agent.yousef shtiwe-overlord.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  yousef shtiwe config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with YOUSEF SHTIWE from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'yousef shtiwe setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     yousef shtiwe config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: yousef shtiwe gateway install")
            else:
                print_info("  You can install later: yousef shtiwe gateway install")
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo yousef shtiwe gateway install --system")
                print_info("  Or run in foreground:  yousef shtiwe gateway")
        else:
            from yousef shtiwe_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   yousef shtiwe gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   yousef shtiwe gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `yousef shtiwe setup tools` and `yousef shtiwe tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from yousef shtiwe_cli.config
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_yousef shtiwe.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_yousef shtiwe migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_yousef shtiwe", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and YOUSEF SHTIWE.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure YOUSEF SHTIWE to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point YOUSEF SHTIWE at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point YOUSEF SHTIWE at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point YOUSEF SHTIWE at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point YOUSEF SHTIWE at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to YOUSEF SHTIWE equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing YOUSEF SHTIWE config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in YOUSEF SHTIWE.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ YOUSEF SHTIWE's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(yousef shtiwe_home: Path) -> bool:
    ...
    print_info("YOUSEF SHTIWE can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: yousef shtiwe claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=yousef shtiwe_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: yousef shtiwe claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing YOUSEF SHTIWE configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing YOUSEF SHTIWE config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in YOUSEF SHTIWE (use yousef shtiwe claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      yousef shtiwe setup           — full or quick (auto-detected)
      yousef shtiwe setup model     — just model/provider
      yousef shtiwe setup tts       — just text-to-speech
      yousef shtiwe setup terminal  — just terminal backend
      yousef shtiwe setup gateway   — just messaging platforms
      yousef shtiwe setup tools     — just tool configuration
      yousef shtiwe setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ YOUSEF SHTIWE Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ YOUSEF SHTIWE Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your YOUSEF SHTIWE Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have YOUSEF SHTIWE configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, yousef shtiwe_home)
            return
        elif choice == 1:
            # Full setup — fall through to run all sections
            pass
        elif choice == 7:
            print_info("Exiting. Run 'yousef shtiwe setup' again when ready.")
            return
        elif 2 <= choice <= 6:
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, yousef shtiwe_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        migration_ran = _offer_openclaw_migration(yousef shtiwe_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up YOUSEF SHTIWE?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, yousef shtiwe_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {yousef shtiwe_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'yousef shtiwe config edit'")
...
def _resolve_yousef shtiwe_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``yousef shtiwe chat`` in a fresh process."""
    yousef shtiwe_bin = shutil.which("yousef shtiwe")
    if yousef shtiwe_bin:
        return [yousef shtiwe_bin, "chat"]

    try:
        if importlib.util.find_spec("yousef shtiwe_cli") is not None:
            return [sys.executable, "-m", "yousef shtiwe_cli.main", "chat"]
    except Exception:
        pass
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch yousef shtiwe chat now?", True):
        return

    chat_argv = _resolve_yousef shtiwe_chat_argv()
    if not chat_argv:
        print_info("Could not relaunch YOUSEF SHTIWE automatically. Run 'yousef shtiwe chat' manually.")
        return

    os.execvp(chat_argv[0], chat_argv)


def _run_first_time_quick_setup(config: dict, yousef shtiwe_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'yousef shtiwe setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    yousef shtiwe setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  yousef shtiwe setup gateway")
    print()
...
def _run_quick_setup(config: dict, yousef shtiwe_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'yousef shtiwe setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect YOUSEF SHTIWE to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'yousef shtiwe setup gateway'.")
