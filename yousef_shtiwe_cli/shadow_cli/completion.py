"""Shell completion script generation for yousef shtiwe CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_yousef shtiwe_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# YOUSEF SHTIWE Agent bash completion
# Add to ~/.bashrc:
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef yousef shtiwe
# YOUSEF SHTIWE Agent zsh completion
# Add to ~/.zshrc:
...
_yousef shtiwe() {{
    ...
    _arguments -C \\
        ...
        '(-p --profile){{-p,--profile}}[Profile name]:profile:_yousef shtiwe_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'yousef shtiwe command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# YOUSEF SHTIWE Agent fish completion",
        "# Add to your config:",
        "#   yousef shtiwe completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __yousef shtiwe_profiles",
        "    echo default",
        "    if test -d $HOME/.yousef shtiwe/profiles",
        "        ls $HOME/.yousef shtiwe/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c yousef shtiwe -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c yousef shtiwe -f -s p -l profile"
        " -d 'Profile name' -xa '(__yousef shtiwe_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__yousef shtiwe_profiles)' -d 'Profile name'"
                )
