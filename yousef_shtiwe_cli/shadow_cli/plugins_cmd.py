...
Plugins are installed from Git repositories into ``~/.yousef shtiwe/plugins/``.
...
def _plugins_dir() -> Path:
    ...
    plugins = get_yousef shtiwe_home() / "plugins"
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from yousef shtiwe_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_yousef shtiwe_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_yousef shtiwe_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_yousef shtiwe_home()}/.env later)[/dim]")
...
def cmd_install(identifier: str, force: bool = False) -> None:
    ...
    if not (target / "plugin.yaml").exists() and not (target / "__init__.py").exists():
        console.print(
            ...
            f"or __init__.py. It may not be a valid YOUSEF SHTIWE plugin."
        )
    ...
    console.print("[dim]  yousef shtiwe gateway restart[/dim]")
    console.print()
