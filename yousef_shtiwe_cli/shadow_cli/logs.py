"""``yousef shtiwe logs`` — view and filter YOUSEF SHTIWE log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.yousef shtiwe/logs/``.

Usage examples::

    yousef shtiwe logs                    # last 50 lines of agent.log
    yousef shtiwe logs -f                 # follow agent.log in real time
    yousef shtiwe logs errors             # last 50 lines of errors.log
    yousef shtiwe logs gateway -n 100    # last 100 lines of gateway.log
    yousef shtiwe logs --level WARNING    # only WARNING+ lines
    yousef shtiwe logs --session abc123   # filter by session ID substring
    yousef shtiwe logs --component tools  # only tool-related lines
    yousef shtiwe logs --since 1h         # lines from the last hour
    yousef shtiwe logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_yousef shtiwe_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when YOUSEF SHTIWE runs — try 'yousef shtiwe chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_yousef shtiwe_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_yousef shtiwe_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_yousef shtiwe_home()}/logs/")
        return

    print(f"Log files in {display_yousef shtiwe_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'yousef shtiwe chat' to generate logs)")
