"""yousef shtiwe claw — OpenClaw migration commands.
...
Usage:
    ...
    yousef shtiwe claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    yousef shtiwe claw cleanup              # Archive leftover OpenClaw directories
    yousef shtiwe claw cleanup --dry-run    # Preview what would be archived
"""
...
from yousef shtiwe_cli.config import get_yousef shtiwe_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "YOUSEF SHTIWE may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a YOUSEF SHTIWE gateway is running with connected platforms.
    ...
    print()
    print_error(
        "YOUSEF SHTIWE gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'yousef shtiwe stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_yousef shtiwe.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_yousef shtiwe", script_path)
...
def claw_command(args):
    """Route yousef shtiwe claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: yousef shtiwe claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to YOUSEF SHTIWE")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'yousef shtiwe claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → YOUSEF SHTIWE migration."""
    ...
    print(
        color(
            "│          ⚕ YOUSEF SHTIWE — OpenClaw Migration                 │",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: yousef shtiwe claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {yousef shtiwe_home}")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: yousef shtiwe claw migrate --yes")
            return
    ...
    # 'yousef shtiwe claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    print(
        color(
            "│          ⚕ YOUSEF SHTIWE — OpenClaw Cleanup                   │",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: yousef shtiwe claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: yousef shtiwe claw cleanup --yes")
        else:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  yousef shtiwe claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  yousef shtiwe claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  yousef shtiwe config set OPENROUTER_API_KEY sk-or-v1-...")
