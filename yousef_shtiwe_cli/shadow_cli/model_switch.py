...
- ``yousef shtiwe_cli.providers``        -- canonical provider identity + overlays
- ``yousef shtiwe_cli.model_normalize``  -- per-provider name formatting
...
from yousef shtiwe_cli.providers import (
...
_YOUSEF SHTIWE_MODEL_WARNING = (
    "YOUSEF SHTIWE-OVERLORD YOUSEF SHTIWE 3 & 4 models are NOT agentic and are not designed "
    "for use with YOUSEF SHTIWE Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real YOUSEF SHTIWE-OVERLORD YOUSEF SHTIWE 3 / YOUSEF SHTIWE 4 chat families.
# The previous substring check (`"yousef shtiwe" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``yousef shtiwe-brain:qwen3-14b-ctx16k`` that just
# happen to carry "yousef shtiwe" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   YOUSEF SHTIWE-OVERLORD/YOUSEF SHTIWE-3-Llama-3.1-70B, yousef shtiwe-4-405b, openrouter/yousef shtiwe3:70b
# Negative examples it must NOT match:
#   yousef shtiwe-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_Yousef Shtiwe_YOUSEF SHTIWE_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])yousef shtiwe[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_yousef shtiwe_yousef shtiwe_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Yousef Shtiwe YOUSEF SHTIWE 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_Yousef Shtiwe_YOUSEF SHTIWE_NON_AGENTIC_RE.search(model_name))


def _check_yousef shtiwe_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Yousef Shtiwe YOUSEF SHTIWE 3/4 chat model."""
    if is_yousef shtiwe_yousef shtiwe_non_agentic(model_name):
        return _YOUSEF SHTIWE_MODEL_WARNING
    return ""
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "yousef shtiwe")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "yousef shtiwe")
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'yousef shtiwe model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'yousef shtiwe doctor' — config issues detected:"
    ...
    yousef shtiwe_warn = _check_yousef shtiwe_model_warning(new_model)
    if yousef shtiwe_warn:
        warnings.append(yousef shtiwe_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from yousef shtiwe_cli/models.py (OPENROUTER_MODELS,
    ...
    from yousef shtiwe_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # Build curated model lists keyed by yousef shtiwe provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "yousef shtiwe" shares OpenRouter's curated list if not separately defined
    if "yousef shtiwe" not in curated:
        curated["yousef shtiwe"] = curated["openrouter"]

    # --- 1. Check YOUSEF SHTIWE-mapped providers ---
    for yousef shtiwe_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(yousef shtiwe_id)
        ...
        model_ids = curated.get(yousef shtiwe_id, [])
        total = len(model_ids)
        top = model_ids[:max_models]

        slug = yousef shtiwe_id
    ...
    # --- 2. Check YOUSEF SHTIWE-only providers (yousef shtiwe, openai-codex, copilot, opencode-go) ---
    from yousef shtiwe_cli.providers import YOUSEF SHTIWE_OVERLAYS
    from yousef shtiwe_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → YOUSEF SHTIWE provider ID.
    # YOUSEF SHTIWE_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use YOUSEF SHTIWE IDs ("copilot").
    _mdev_to_yousef shtiwe = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in YOUSEF SHTIWE_OVERLAYS.items():
        if pid in seen_slugs:
            continue

        # Resolve YOUSEF SHTIWE slug — e.g. "github-copilot" → "copilot"
        yousef shtiwe_slug = _mdev_to_yousef shtiwe.get(pid, pid)
        if yousef shtiwe_slug in seen_slugs:
            continue
        ...
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, yousef shtiwe_slug):
        ...
        if not has_creds:
            try:
                from yousef shtiwe_cli.auth import _load_auth_store
                store = _load_auth_store()
                providers_store = store.get("providers", {})
                pool_store = store.get("credential_pool", {})
                if store and (
                    pid in providers_store or yousef shtiwe_slug in providers_store
                    or pid in pool_store or yousef shtiwe_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(yousef shtiwe_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", yousef shtiwe_slug, exc)
        ...
        # configured.
        if not has_creds and yousef shtiwe_slug == "anthropic":
            try:
                ...
                if (yousef shtiwe_creds and yousef shtiwe_creds.get("accessToken")) or \
        ...
        # Use curated list — look up by YOUSEF SHTIWE slug, fall back to overlay key
        model_ids = curated.get(yousef shtiwe_slug, []) or curated.get(pid, [])
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": yousef shtiwe_slug,
            "name": get_label(yousef shtiwe_slug),
            "is_current": yousef shtiwe_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "yousef shtiwe",
        })
        seen_slugs.add(pid)
        seen_slugs.add(yousef shtiwe_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or YOUSEF SHTIWE_OVERLAYS (keeps /model in sync
    # with `yousef shtiwe model`).
    try:
        from yousef shtiwe_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
