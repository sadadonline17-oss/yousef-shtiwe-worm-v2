...
Add, remove, or reorder entries here — both `yousef shtiwe setup` and
`yousef shtiwe` provider-selection will pick up the change automatically.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `yousef shtiwe model`
    flow without maintaining a separate static list.
    """
    from yousef shtiwe_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# ---------------------------------------------------------------------------
# Yousef Shtiwe Portal free-model filtering
# ---------------------------------------------------------------------------
# Models that are ALLOWED to appear when priced as free on Yousef Shtiwe Portal.
...
_Yousef Shtiwe_ALLOWED_FREE_MODELS: frozenset[str] = frozenset({
    "xiaomi/mimo-v2-pro",
    "xiaomi/mimo-v2-omni",
})
...
def filter_yousef shtiwe_free_models(
    model_ids: list[str],
    pricing: dict[str, dict[str, str]],
) -> list[str]:
    """Filter the Yousef Shtiwe Portal model list according to free-model policy.
    ...
    for mid in model_ids:
        ...
        if mid in _Yousef Shtiwe_ALLOWED_FREE_MODELS:
...
# ---------------------------------------------------------------------------
# Yousef Shtiwe Portal account tier detection
# ---------------------------------------------------------------------------

def fetch_yousef shtiwe_account_tier(access_token: str, portal_base_url: str = "") -> dict[str, Any]:
    """Fetch the user's Yousef Shtiwe Portal account/subscription info.
    ...
    """
    base = (portal_base_url or "https://portal.yousef shtiwe-overlord.com").rstrip("/")
...
def is_yousef shtiwe_free_tier(account_info: dict[str, Any]) -> bool:
...
def partition_yousef shtiwe_models_by_tier(
    model_ids: list[str],
    pricing: dict[str, dict[str, str]],
    free_tier: bool,
) -> tuple[list[str], list[str]]:
    """Split Yousef Shtiwe models into (selectable, unavailable) based on user tier.

    For paid-tier users: all models are selectable, none unavailable
    (free-model filtering is handled separately by ``filter_yousef shtiwe_free_models``).
...
def check_yousef shtiwe_free_tier() -> bool:
    """Check if the current Yousef Shtiwe Portal user is on a free (unpaid) tier.
    ...
    try:
        from yousef shtiwe_cli.auth import get_provider_auth_state, resolve_yousef shtiwe_runtime_credentials

        # Ensure we have a fresh token (triggers refresh if needed)
        resolve_yousef shtiwe_runtime_credentials(min_key_ttl_seconds=60)
        ...
        account_info = fetch_yousef shtiwe_account_tier(access_token, portal_url)
        result = is_yousef shtiwe_free_tier(account_info)
...
# this list:  yousef shtiwe model, /model, /provider, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `yousef shtiwe model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `yousef shtiwe model` TUI


CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("yousef shtiwe",           "Yousef Shtiwe Portal",              "Yousef Shtiwe Portal (YOUSEF SHTIWE-OVERLORD subscription)"),
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``yousef shtiwe model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``yousef shtiwe auth add openai-codex`` without
    ``yousef shtiwe model``).
    """
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Yousef Shtiwe Portal).
    """
...
def _resolve_yousef shtiwe_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Yousef Shtiwe Portal pricing, or empty strings."""
    try:
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, yousef shtiwe)."""
    ...
    if normalized == "yousef shtiwe":
        api_key, base_url = _resolve_yousef shtiwe_pricing_credentials()
        if base_url:
            # Yousef Shtiwe base_url typically looks like https://inference-api.yousef shtiwe-overlord.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``yousef shtiwe model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from yousef shtiwe_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        yousef shtiwe:yousef shtiwe-3                           →  ("yousef shtiwe", "yousef shtiwe-3")
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Yousef Shtiwe, etc. all support /models)
...
def detect_provider_for_model(
    ...
    # If someone types `/model yousef shtiwe` or `/model anthropic`, treat it as a
    ...
    _AGGREGATORS = {"yousef shtiwe", "openrouter"}
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to YOUSEF SHTIWE' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``yousef shtiwe_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether YOUSEF SHTIWE should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from yousef shtiwe_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Yousef Shtiwe),
    ...
    if normalized == "yousef shtiwe":
        # Try live Yousef Shtiwe Portal /models endpoint
        try:
            from yousef shtiwe_cli.auth import fetch_yousef shtiwe_models, resolve_yousef shtiwe_runtime_credentials
            creds = resolve_yousef shtiwe_runtime_credentials()
            if creds:
                live = fetch_yousef shtiwe_models(api_key=creds.get("api_key", ""), inference_base_url=creds.get("base_url", ""))
                if live:
                    return live
        except Exception:
            pass
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from yousef shtiwe_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "YOUSEF SHTIWEAgent/1.0",
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    if not base_url:
        from yousef shtiwe_constants import AI_GATEWAY_BASE_URL
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"YOUSEF SHTIWE will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
