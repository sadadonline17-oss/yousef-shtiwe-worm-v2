"""Helpers for Yousef Shtiwe subscription managed-tool capabilities."""
...
from yousef shtiwe_cli.config import get_env_value, load_config
...
_DEFAULT_PLATFORM_TOOLSETS = {
    "cli": "yousef shtiwe-cli",
}


@dataclass(frozen=True)
class Yousef ShtiweFeatureState:
    key: str
    label: str
    included_by_default: bool
    available: bool
    active: bool
    managed_by_yousef shtiwe: bool
...
@dataclass(frozen=True)
class Yousef ShtiweSubscriptionFeatures:
    subscribed: bool
    yousef shtiwe_auth_present: bool
    provider_is_yousef shtiwe: bool
    features: Dict[str, Yousef ShtiweFeatureState]

    @property
    def web(self) -> Yousef ShtiweFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> Yousef ShtiweFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> Yousef ShtiweFeatureState:
        return self.features["tts"]

    @property
    def browser(self) -> Yousef ShtiweFeatureState:
        return self.features["browser"]

    @property
    def modal(self) -> Yousef ShtiweFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[Yousef ShtiweFeatureState]:
...
def get_yousef shtiwe_subscription_features(
    config: Optional[Dict[str, object]] = None,
) -> Yousef ShtiweSubscriptionFeatures:
    if config is None:
        config = load_config() or {}
    config = dict(config)
    model_cfg = _model_config_dict(config)
    provider_is_yousef shtiwe = str(model_cfg.get("provider") or "").strip().lower() == "yousef shtiwe"

    try:
        yousef shtiwe_status = get_yousef shtiwe_auth_status()
    except Exception:
        yousef shtiwe_status = {}

    managed_tools_flag = managed_yousef shtiwe_tools_enabled()
    yousef shtiwe_auth_present = bool(yousef shtiwe_status.get("logged_in"))
    subscribed = provider_is_yousef shtiwe or yousef shtiwe_auth_present
    ...
    managed_web_available = managed_tools_flag and yousef shtiwe_auth_present and is_managed_tool_gateway_ready("firecrawl")
    managed_image_available = managed_tools_flag and yousef shtiwe_auth_present and is_managed_tool_gateway_ready("fal-queue")
    managed_tts_available = managed_tools_flag and yousef shtiwe_auth_present and is_managed_tool_gateway_ready("openai-audio")
    managed_browser_available = managed_tools_flag and yousef shtiwe_auth_present and is_managed_tool_gateway_ready("browser-use")
    managed_modal_available = managed_tools_flag and yousef shtiwe_auth_present and is_managed_tool_gateway_ready("modal")
    ...
    features = {
        "web": Yousef ShtiweFeatureState(
            key="web",
            label="Web tools",
            included_by_default=True,
            available=web_available,
            active=web_active,
            managed_by_yousef shtiwe=web_managed,
        ...
        "image_gen": Yousef ShtiweFeatureState(
            key="image_gen",
            label="Image generation",
            included_by_default=True,
            available=image_available,
            active=image_active,
            managed_by_yousef shtiwe=image_managed,
            direct_override=image_active and not image_managed,
            toolset_enabled=image_tool_enabled,
            current_provider="FAL" if direct_fal else ("Yousef Shtiwe Subscription" if image_managed else ""),
            explicit_configured=direct_fal,
        ),
        "tts": Yousef ShtiweFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
            available=tts_available,
            active=tts_active,
            managed_by_yousef shtiwe=tts_managed,
        ...
        "browser": Yousef ShtiweFeatureState(
            key="browser",
            label="Browser automation",
            included_by_default=True,
            available=browser_available,
            active=browser_active,
            managed_by_yousef shtiwe=browser_managed,
        ...
        "modal": Yousef ShtiweFeatureState(
            key="modal",
            label="Modal execution",
            included_by_default=False,
            available=modal_available,
            active=modal_active,
            managed_by_yousef shtiwe=modal_managed,
    ...
    return Yousef ShtiweSubscriptionFeatures(
        subscribed=subscribed,
        yousef shtiwe_auth_present=yousef shtiwe_auth_present,
        provider_is_yousef shtiwe=provider_is_yousef shtiwe,
        features=features,
    )


def get_yousef shtiwe_subscription_explainer_lines() -> list[str]:
    if not managed_yousef shtiwe_tools_enabled():
        return []

    return [
        "Yousef Shtiwe subscription enables managed web tools, image generation, OpenAI TTS, and browser automation by default.",
        "Those managed tools bill to your Yousef Shtiwe subscription. Modal execution is optional and can bill to your subscription too.",
        "Change these later with: yousef shtiwe setup tools, yousef shtiwe setup terminal, or yousef shtiwe status.",
    ]


def apply_yousef shtiwe_provider_defaults(config: Dict[str, object]) -> set[str]:
    """Apply provider-level Yousef Shtiwe defaults shared by `yousef shtiwe setup` and `yousef shtiwe model`."""
    if not managed_yousef shtiwe_tools_enabled():
        return set()

    features = get_yousef shtiwe_subscription_features(config)
    if not features.provider_is_yousef shtiwe:
        return set()
...
def apply_yousef shtiwe_managed_defaults(
