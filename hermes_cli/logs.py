"""``shadow logs`` — view and filter SHADOW log files.
...
Usage examples::
    ...
    shadow logs                    # last 50 lines of agent.log
    shadow logs -f                 # follow agent.log in real time
    shadow logs errors             # last 50 lines of errors.log
    shadow logs gateway -n 100    # last 100 lines of gateway.log
    shadow logs --level WARNING    # only WARNING+ lines
    shadow logs --session abc123   # filter by session ID substring
    shadow logs --component tools  # only tool-related lines
    shadow logs --since 1h         # lines from the last hour
    shadow logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_shadow_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when SHADOW runs — try 'shadow chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_shadow_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_shadow_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_shadow_home()}/logs/")
        return

    print(f"Log files in {display_shadow_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'shadow chat' to generate logs)")
