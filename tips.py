...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or shadow -c.",
    ...
    "Ctrl+Z suspends SHADOW to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "shadow -c resumes your most recent CLI session. shadow -c \"project name\" resumes by title.",
    "shadow -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "shadow -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "shadow chat -t web,terminal enables only specific toolsets for a focused session.",
    "shadow chat -s github-pr-workflow preloads a skill at launch.",
    "shadow chat -q \"query\" runs a single non-interactive query and exits.",
    "shadow chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "shadow chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "shadow --yolo bypasses all dangerous command approval prompts for the entire session.",
    "shadow chat --source telegram tags the session for filtering in shadow sessions list.",
    "shadow -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "shadow doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "shadow dump outputs a compact setup summary — great for bug reports.",
    "shadow config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "shadow config edit opens config.yaml in your default editor.",
    "shadow config check scans for missing or stale configuration options.",
    "shadow sessions browse opens an interactive session picker with search.",
    "shadow sessions stats shows session counts by platform and database size.",
    "shadow sessions prune --older-than 30 cleans up old sessions.",
    "shadow skills search react --source skills-sh searches the skills.sh public directory.",
    "shadow skills check scans installed hub skills for upstream updates.",
    "shadow skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "shadow skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "shadow mcp add github --command npx adds MCP servers from the command line.",
    "shadow mcp serve runs SHADOW itself as an MCP server for other agents.",
    "shadow auth add lets you add multiple API keys for credential pool rotation.",
    "shadow completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "shadow logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "shadow backup creates a zip backup of your entire SHADOW home directory.",
    "shadow profile create coder creates an isolated profile that becomes its own command.",
    "shadow profile create work --clone copies your current config and keys to a new profile.",
    "shadow update syncs new bundled skills to ALL profiles automatically.",
    "shadow gateway install sets up SHADOW as a system service (systemd/launchd).",
    "shadow memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "shadow webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as shadow -w).",
    ...
    "SHADOW_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call SHADOW tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'shadow profile create coder' creates the 'coder' command.",
    "shadow profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, SHADOW prints a resume command with session ID and stats.",
    "shadow sessions export backup.jsonl exports all sessions for backup or analysis.",
    "shadow -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "shadow skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .shadow/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: shadow cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "SHADOW runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "shadow gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to SHADOW via a local macOS server.",
    ...
    "Context files (.shadow.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.shadow/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "SHADOW loads project context from .shadow.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — shadow tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.shadow/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "shadow chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "shadow plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "shadow acp runs SHADOW as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "BOOT.md at ~/.shadow/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.shadow/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make SHADOW your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.shadow/SOUL.md to customize.",
    ...
    "Before a gateway session reset, SHADOW auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.shadow/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at SHADOW_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "SHADOW_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in SHADOW_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.shadow/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'shadow profile create ls' would be rejected.",
    "shadow profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "shadow claw migrate --dry-run previews OpenClaw migration without writing anything.",
