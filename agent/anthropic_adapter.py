"""Anthropic Messages API adapter for YOUSEF SHTIWE Agent.

Translates between YOUSEF SHTIWE's internal OpenAI-style message format and
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    """Prefer Claude Code creds when a persisted env OAuth token would yousef shtiwe refresh.

    YOUSEF SHTIWE historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by YOUSEF SHTIWE)
    ...
    # 1. YOUSEF SHTIWE-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration YOUSEF SHTIWE configs.
...
# ── YOUSEF SHTIWE-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.yousef shtiwe/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_YOUSEF SHTIWE_OAUTH_FILE = get_yousef shtiwe_home() / ".anthropic_oauth.json"
...
def run_yousef shtiwe_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run YOUSEF SHTIWE-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize YOUSEF SHTIWE with your Claude Pro/Max subscription.")
    print()
...
def read_yousef shtiwe_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read YOUSEF SHTIWE-managed OAuth credentials from ~/.yousef shtiwe/.anthropic_oauth.json."""
    if _YOUSEF SHTIWE_OAUTH_FILE.exists():
        try:
            data = json.loads(_YOUSEF SHTIWE_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read YOUSEF SHTIWE OAuth credentials: %s", e)
    return None
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("YOUSEF SHTIWE Agent", "Claude Code")
                text = text.replace("YOUSEF SHTIWE agent", "Claude Code")
                text = text.replace("yousef shtiwe-agent", "claude-code")
                text = text.replace("YOUSEF SHTIWE-OVERLORD", "Anthropic")
                block["text"] = text
