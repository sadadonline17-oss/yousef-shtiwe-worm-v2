...
from yousef shtiwe_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``YOUSEF SHTIWE_PLATFORM`` or
            ``YOUSEF SHTIWE_SESSION_PLATFORM`` env vars.  Falls back to the
    ...
    resolved_platform = (
        platform
        or os.getenv("YOUSEF SHTIWE_PLATFORM")
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.yousef shtiwe/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.yousef shtiwe/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    yousef shtiwe = metadata.get("yousef shtiwe") or {}
    if not isinstance(yousef shtiwe, dict):
        yousef shtiwe = {}
    return {
        "fallback_for_toolsets": yousef shtiwe.get("fallback_for_toolsets", []),
        "requires_toolsets": yousef shtiwe.get("requires_toolsets", []),
        "fallback_for_tools": yousef shtiwe.get("fallback_for_tools", []),
        "requires_tools": yousef shtiwe.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    yousef shtiwe = metadata.get("yousef shtiwe")
    if not isinstance(yousef shtiwe, dict):
        return []
    raw = yousef shtiwe.get("config")
    if not raw:
        return []
