...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - shadow_home (str): The active SHADOW_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.shadow``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "shadow").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'shadow memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], shadow_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'shadow memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``shadow_home`` is the active SHADOW_HOME directory path.
